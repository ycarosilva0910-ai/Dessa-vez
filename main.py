...
# ============================================================
# DERROTE O GORDO DO PAI DO KAYK - V10 CINEMA
        ...
        if self.domain_active:
            ...
            if self.domain_timer <= 0:
                ...
                if self.variant == "father" and inside and self.domain_tick <= 0:
                    ...
                    game.damage_texts.append(DamageText("DISCIPLINA!", game.player.pos.copy(), RED))
                    ...
                    game.damage_texts.append(DamageText("SILENCIO", game.player.pos.copy(), GRAY))
...
class Player:
    ...
    def expand_domain(self, game):
        ...
        if self.character == "Ruan":
            ...
            game.damage_texts.append(DamageText("GUARDIAO INVOCADO!", self.pos.copy(), CYAN, 1.1))
        ...
        if self.character == "Pai do Kayk":
            ...
            if self.father_domain_refill:
                self.hp = self.max_hp
                self.stamina = self.max_stamina
            game.damage_texts.append(DamageText("SENTENCA ABSOLUTA", self.pos.copy(), WHITE, 1.1))
...
class Game:
    ...
    def spawn_ruan_servant(self, enemy):
        ...
        if isinstance(enemy, TrainingDummy):
            return
        servant = RuanServant(enemy.pos.copy(), enemy, self.player, guardian=False)
        self.servants.append(servant)
        self.damage_texts.append(DamageText("LEVANTE-SE!", enemy.pos.copy(), GREEN, 0.9))
    ...
    def update_playing(self, dt):
        ...
        if self.domain_active:
            ...
            self.domain_center = self.player.pos.copy()
            self.domain_timer -= dt
            if self.domain_timer <= 0:
                self.domain_active = False
                self.domain_timer = 0
                ...
                for e in list(self.enemies):
                    ...
                    if pr.pos.distance_to(e.pos) <= pr.radius + e.radius:
                        ...
                        if isinstance(pr, BoomerangProjectile) and pr.returning and pr.crown:
                            dmg *= 1.65
                            self.damage_texts.append(DamageText("COROA!", e.pos.copy(), YELLOW))
