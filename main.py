...
def _write_sans_trial_crash(context, exc):
    """Salva erros Python da Julgamento Sans sem depender do console do Android."""
    try:
        base=os.path.dirname(os.path.abspath(__file__))
        path=os.path.join(base,"sans_trial_crash.log")
        with open(path,"a",encoding="utf-8") as f:
            f.write("\n=== SANS TRIAL | "+str(context)+" ===\n")
            f.write("Erro: "+repr(exc)+"\n")
...
# ============================================================
# DERROTE O GORDO DO PAI DO KAYK - V15.15 SANS BOSS + FINAL TOUCHES
...
MAHORAGA_MODULE_KEY = "Mahoraga Módulo"
SUKUNA_KEY = "Sukuna"
...
SANS_BONE_IMAGE = None
SANS_BONE_LARGE_IMAGE = None
SANS_BONE_BLUE_IMAGE = None
SANS_BONE_ORANGE_IMAGE = None
SANS_BONES_ASSETS_LOADED = False
SANS_BLASTER_FRAMES = []
SANS_PLAYER_FRAMES = {}
...
_HANK_TARGET_CACHE = {}
_SANS_BONE_CACHE = {}
_SANS_BONE_LARGE_CACHE = {}
_SANS_BONE_BLUE_CACHE = {}
_SANS_BONE_ORANGE_CACHE = {}
_SANS_BLASTER_FRAME_CACHE = {}
_SANS_PLAYER_FRAME_CACHE = {}
    ...
    for path in candidates:
        try:
            if os.path.exists(path):
                ...
                # Android/Pydroid: os PNGs da Lira sao grandes. Mantemos uma copia-base
                # reduzida para evitar smoothscale pesado no PRIMEIRO uso durante a luta.
                ow, oh = img.get_size()
                longest = max(ow, oh)
                if longest > 640:
                    scale = 640.0 / longest
                    img = pygame.transform.smoothscale(img, (max(8,int(ow*scale)), max(8,int(oh*scale))))
                return img
        except Exception:
            pass
    return None
...
def _scaled_image(image, cache, size):
    if image is None:
        return None
    # Quantiza tamanhos para reduzir a quantidade de superficies diferentes no cache.
    size=max(8,int(round(float(size)/8.0)*8))
    if size not in cache:
        try:
            ow,oh=image.get_size(); ratio=oh/max(1,ow)
            # scale normal e muito mais barato que smoothscale por frame/tamanho no Android.
            cache[size]=pygame.transform.scale(image,(size,max(6,int(size*ratio))))
        except Exception:
            return None
...
def _load_png_asset_exact(filename, max_base=None):
    """Carrega um PNG especifico sem fallback cruzado entre os ossos do Sans."""
    base = os.path.dirname(os.path.abspath(__file__))
    for folder in (os.path.join(base, "assets"), base):
        ...
        try:
            ...
            if max_base:
                ...
                longest = max(ow, oh)
                if longest > max_base:
                    scale = max_base / float(longest)
                    img = pygame.transform.smoothscale(img, (max(8, int(ow*scale)), max(8, int(oh*scale))))
            return img
        except Exception as exc:
            print("Falha ao carregar asset do Sans:", p, exc)
    return None


def _ensure_sans_bone_images():
    """Carrega cada PNG de osso explicitamente; nenhum tipo usa o asset do outro."""
    global SANS_BONE_IMAGE, SANS_BONE_LARGE_IMAGE, SANS_BONE_BLUE_IMAGE, SANS_BONE_ORANGE_IMAGE, SANS_BONES_ASSETS_LOADED
    if SANS_BONES_ASSETS_LOADED:
        return
    SANS_BONES_ASSETS_LOADED = True
    SANS_BONE_IMAGE = _load_png_asset_exact("sans_bone.png", 320)
    SANS_BONE_LARGE_IMAGE = _load_png_asset_exact("sans_bone_large.png", 360)
    SANS_BONE_BLUE_IMAGE = _load_png_asset_exact("sans_bone_blue.png", 360)
    SANS_BONE_ORANGE_IMAGE = _load_png_asset_exact("sans_bone_orange.png", 360)


def _cache_put_limited(cache, key, value, limit=80):
    cache[key] = value
    if len(cache) > limit:
        try:
...
def sans_bone_small_surface(length, thickness, direction):
    """Osso NORMAL: usa apenas sans_bone.png. O PNG base e vertical.
    O eixo longo fica perpendicular ao movimento, estilo Undertale.
    """
    _ensure_sans_bone_images()
    if SANS_BONE_IMAGE is None:
        return None
    d = pygame.Vector2(direction)
    if d.length_squared() <= 0.0001:
        d = pygame.Vector2(1, 0)
    d = d.normalize()
    angle = int(round(math.degrees(math.atan2(d.y, d.x)) / 15.0) * 15) % 360
    length_step=max(8,S(14)); thick_step=max(4,S(4))
    length=max(S(84), int(round(float(length)/length_step)*length_step))
    thickness=max(S(24), int(round(float(thickness)/thick_step)*thick_step))
    key=(length,thickness,angle)
    if key not in _SANS_BONE_CACHE:
        try:
            # Base vertical: largura=espessura, altura=comprimento.
            img=pygame.transform.smoothscale(SANS_BONE_IMAGE,(thickness,length))
            # moving right => stays vertical; moving down => rotates horizontal.
            if angle:
                img=pygame.transform.rotate(img,-angle)
            _cache_put_limited(_SANS_BONE_CACHE,key,img)
        except Exception:
            return None
    return _SANS_BONE_CACHE.get(key)


def sans_bone_large_surface(length, thickness, direction):
    """Osso GRANDE: usa apenas sans_bone_large.png. Nunca cai para sans_bone.png."""
    _ensure_sans_bone_images()
    if SANS_BONE_LARGE_IMAGE is None:
        return None
    d=pygame.Vector2(direction)
    if d.length_squared() <= 0.0001:
        d=pygame.Vector2(1,0)
    d=d.normalize()
    angle=int(round(math.degrees(math.atan2(d.y,d.x))/15.0)*15)%360
    length_step=max(8,S(18)); thick_step=max(4,S(5))
    length=max(S(120),int(round(float(length)/length_step)*length_step))
    thickness=max(S(34),int(round(float(thickness)/thick_step)*thick_step))
    key=(length,thickness,angle)
    if key not in _SANS_BONE_LARGE_CACHE:
        try:
            # Base horizontal: largura=comprimento, altura=espessura.
            img=pygame.transform.smoothscale(SANS_BONE_LARGE_IMAGE,(length,thickness))
            if angle:
                img=pygame.transform.rotate(img,-angle)
            _cache_put_limited(_SANS_BONE_LARGE_CACHE,key,img)
        except Exception:
            return None
    return _SANS_BONE_LARGE_CACHE.get(key)


def sans_bone_special_surface(mode, length, thickness, direction):
    """Ossos AZUL/LARANJA do Sans Boss; usam os sprites enviados pelo usuario."""
    _ensure_sans_bone_images()
    if mode == "blue":
        image, cache = SANS_BONE_BLUE_IMAGE, _SANS_BONE_BLUE_CACHE
    elif mode == "orange":
        image, cache = SANS_BONE_ORANGE_IMAGE, _SANS_BONE_ORANGE_CACHE
    else:
        return None
    if image is None:
        return None
    ...
    if key not in cache:
        try:
            img=pygame.transform.smoothscale(image,(length,thickness))
            if angle: img=pygame.transform.rotate(img,-angle)
            _cache_put_limited(cache,key,img)
        except Exception:
            return None
    return cache.get(key)

def _surface_has_visible_pixels(surface, threshold=18):
    try:
        ...
        for y in range(0, h, step_y):
            for x in range(0, w, step_x):
                ...
                if c.a > 0 and (c.r > threshold or c.g > threshold or c.b > threshold):
                    return True
    except Exception:
        return True
    return False
...
def _trim_black_surface(surface, threshold=12, pad=6):
    w, h = surface.get_size()
    minx = w
    miny = h
    maxx = -1
    ...
    if maxx < minx or maxy < miny:
        return surface.copy()
    rect = pygame.Rect(max(0, minx-pad), max(0, miny-pad), min(w, maxx-minx+1+pad*2), min(h, maxy-miny+1+pad*2))
...
def _load_sans_blaster_frames():
    """V15.8: carrega SOMENTE os quatro PNGs recortados pelo usuario, em ordem fixa."""
    global SANS_BLASTER_FRAMES
    if SANS_BLASTER_FRAMES:
        return
    base = os.path.dirname(os.path.abspath(__file__))
    folders = (
        os.path.join(base, 'assets', 'blaster_frames'),
        os.path.join(base, 'blaster_frames'),
        os.path.join(base, 'Blasters'),
    )
    names = (
        'Sans_blaster_1.png',
        'Sans_blaster_2.png',
        'Sans_blaster_3.png',
        'Sans_blaster_final.png',
    )
    frames = []
    for folder in folders:
        ...
        for name in names:
            ...
            if not os.path.isfile(path):
                ...
                break
            try:
                candidate.append(pygame.image.load(path).convert_alpha())
            except Exception:
                ok = False
                break
        if ok and len(candidate) == len(names):
            frames = candidate
            break
    SANS_BLASTER_FRAMES = frames


_load_sans_blaster_frames()


def sans_blaster_frame_surface(frame_idx, size, angle_deg=0.0):
    if not SANS_BLASTER_FRAMES:
        return None
    idx = int(frame_idx) % len(SANS_BLASTER_FRAMES)
    size = max(24, int(round(float(size) / 4.0) * 4))
    ang = int(round(float(angle_deg) / 5.0) * 5)
    key = (idx, size, ang)
    if key not in _SANS_BLASTER_FRAME_CACHE:
        img = SANS_BLASTER_FRAMES[idx]
        ow, oh = img.get_size()
        ratio = oh / max(1, ow)
        scaled = pygame.transform.scale(img, (size, max(16, int(size * ratio))))
        if ang:
            scaled = pygame.transform.rotozoom(scaled, -ang, 1.0)
        _SANS_BLASTER_FRAME_CACHE[key] = scaled
    return _SANS_BLASTER_FRAME_CACHE.get(key)


def draw_sans_blaster_plant(screen, plant, offset):
    pos = plant['pos'] + offset
    d = pygame.Vector2(plant.get('dir', (0, 1)))
    if d.length_squared() <= 0.0001:
        d = pygame.Vector2(0, 1)
    else:
        d = d.normalize()
    timer = max(0.0, float(plant.get('timer', 0.0)))
    total = max(0.001, float(plant.get('total', 0.9)))
    progress = 1.0 - timer / total
    frame_count = max(1, len(SANS_BLASTER_FRAMES))
    # 0.00-0.30s: frame 1 | 0.30-0.60s: frame 2 | 0.60-0.82s: frame 3
    # ultimos ~0.08s: Sans_blaster_final.png (boca aberta), exatamente antes do laser.
    if frame_count >= 4:
        if progress < (1.0/3.0):
            frame_idx = 0
        ...
        frame_idx = min(frame_count - 1, int(progress * frame_count))
    angle = math.degrees(math.atan2(d.y, d.x)) - 90.0
    scale_boost = 1.0 + 0.18 * progress + (0.10 if timer < 0.18 else 0.0)
    size = max(S(96), int(S(152 if plant.get('auto') else 176) * scale_boost))
    img = sans_blaster_frame_surface(frame_idx, size, angle)
    if img is not None:
        rect = img.get_rect(center=(int(pos.x), int(pos.y)))
        screen.blit(img, rect)
    else:
        nose = pos + d * S(44)
        wing = pygame.Vector2(-d.y, d.x)
        pts = [
            (int((pos + d * S(56)).x), int((pos + d * S(56)).y)),
            (int((pos - d * S(48) + wing * S(34)).x), int((pos - d * S(48) + wing * S(34)).y)),
            (int((pos - d * S(48) - wing * S(34)).x), int((pos - d * S(48) - wing * S(34)).y)),
        ]
        pygame.draw.polygon(screen, WHITE, pts)
        pygame.draw.circle(screen, CYAN if plant.get('auto') else WHITE, (int(nose.x), int(nose.y)), S(14))
    if timer < 0.20:
        glow_r = max(S(20), int(S(42) * (0.20 - timer) / 0.20))
        pygame.draw.circle(screen, CYAN, (int(pos.x), int(pos.y)), glow_r, max(2, S(3)))



def _make_bg_transparent(surface, bg=(212, 212, 212), tol=26):
    out = surface.copy().convert_alpha()
    w, h = out.get_size()
    for y in range(h):
        for x in range(w):
            c = out.get_at((x, y))
            if abs(c.r-bg[0]) <= tol and abs(c.g-bg[1]) <= tol and abs(c.b-bg[2]) <= tol:
                out.set_at((x, y), (0, 0, 0, 0))
    return out


def _crop_alpha_surface(surface, pad=2):
    w, h = surface.get_size()
    minx = w
    miny = h
    maxx = -1
...
def _crop_nonblack_surface(surface, threshold=14, pad=3):
    w, h = surface.get_size()
    minx = w
    miny = h
    maxx = -1
...
def _load_sans_player_frames():
    """V15.8: sprites do Sans ja chegam recortados; nao existe mais slicing de sheet."""
    global SANS_PLAYER_FRAMES
    if SANS_PLAYER_FRAMES:
        return
    base = os.path.dirname(os.path.abspath(__file__))
    folders = (
        os.path.join(base, 'assets', 'sans_frames'),
        os.path.join(base, 'sans_frames'),
        os.path.join(base, 'Sans'),
    )
    frames = {'down': [], 'up': [], 'left': [], 'right': []}
    for folder in folders:
        if not os.path.isdir(folder):
            continue
        candidate = {'down': [], 'up': [], 'left': [], 'right': []}
        ok = True
        for direction in ('down', 'up', 'left', 'right'):
            for number in range(1, 5):
                name = f'Sans_sprite_{direction}{number}.png'
                path = os.path.join(folder, name)
                if not os.path.isfile(path):
                    ok = False
                    break
                try:
                    candidate[direction].append(pygame.image.load(path).convert_alpha())
                except Exception:
                    ok = False
                    break
            if not ok:
        ...
        if ok:
            frames = candidate
            break
    SANS_PLAYER_FRAMES = frames


_load_sans_player_frames()


def sans_player_surface(direction, frame_idx, height):
    if not SANS_PLAYER_FRAMES:
        return None
    frames = SANS_PLAYER_FRAMES.get(direction) or SANS_PLAYER_FRAMES.get('down') or []
    if not frames:
        return None
    idx = int(frame_idx) % len(frames)
    height = max(36, int(round(float(height) / 4.0) * 4))
    key = (direction, idx, height)
    if key not in _SANS_PLAYER_FRAME_CACHE:
        img = frames[idx]
        ow, oh = img.get_size()
        ratio = ow / max(1, oh)
        scaled = pygame.transform.scale(img, (max(16, int(height * ratio)), height))
        _SANS_PLAYER_FRAME_CACHE[key] = scaled
    return _SANS_PLAYER_FRAME_CACHE.get(key)
...
# ============================================================
GAME_MODE_MAIN = ["normal", "infinite", "boss_rush", "arena_survival", "extinction", "evolution", "mahoraga_trial", "sans_trial"]
...
GAME_MODES = {
    ...
    "mahoraga_trial": {"name":"PROVAÇÃO — MAHORAGA", "color":YELLOW, "desc":"Desafio do General Divino. No FACIL nao ha vitoria registrada nem recompensas."},
    "sans_trial": {"name":"JULGAMENTO — SANS", "color":CYAN, "desc":"Bossfight de resistencia. No FACIL nao ha vitoria registrada nem recompensas."},
...
class AudioManager:
    ...
    def __init__(self, settings):
        ...
        self.audio_dir = os.path.join(self.script_dir, "assets", "audio")

        def pick_audio(*filenames):
            # Durante testes no Pydroid, aceita o arquivo ao lado do .py.
            # No APK, continua priorizando assets/audio/.
            for folder in (self.audio_dir, self.script_dir):
                for filename in filenames:
                    path = os.path.join(folder, filename)
                    if os.path.isfile(path):
                        return path
            # Mantem um caminho previsivel para o fallback seguro do AudioManager.
            return os.path.join(self.audio_dir, filenames[0])

        self.external_music_files = {
            "menu": pick_audio("menu.ogg"),
            "battle": pick_audio("batalha.ogg"),
            "mahoraga_summon": pick_audio("mahoraga_summon.ogg"),
            "mahoraga_battle": pick_audio("mahoraga_battle.ogg"),
            "sukuna_ult": pick_audio("ultsukuna.ogg"),
            "sukuna_destruction": pick_audio("destruicaosukuna.ogg"),
            "sukuna_domain": pick_audio("sukuna_domain.ogg"),
            "sans_theme": pick_audio("sans_theme.ogg", "MEGALOVANIA - Toby Fox.mp3"),
            "sans_blaster": pick_audio("sans_blaster.ogg", "[Undertale] Gaster Blaster Sound Effect.mp3"),
            "sans_dodge": pick_audio("sans_dodge_boost.wav", "sans_dodge.ogg", "Undertale SLASH Sound Effect.mp3"),
            "sans_soul": pick_audio("sans_soul.wav", "sans_soul.ogg", "Sans Blue Attack (No music)_1.mp3"),
            "sans_teleport": pick_audio("sans_teleport.ogg", "Sans Teleport sound effect.mp3"),
            "mahoraga_wheel": pick_audio("mahoraga_wheel_spin.mp3", "Mahoraga Wheel Spin  Adaption Sound Effect.mp3"),
            "mahoraga_player_wheel": pick_audio("mahoraga_player_wheel.mp3", "Mahoraga Wheel Sound.mp3"),
        }
        self.external_music_failed = set()
        self.external_sfx_cache = {}
        try:
    ...
    def play_external_sfx(self, role, volume=1.0, cooldown_ms=0):
        """Toca um OGG curto sem interromper a musica. Se faltar/der erro, retorna False."""
        if not self.enabled or not self.sfx_on:
            return False
        path=self.external_music_files.get(role)
        if not path or not os.path.isfile(path):
            return False
        now=pygame.time.get_ticks(); last=self.last_play.get("ext:"+role,-999999)
        if cooldown_ms and now-last<cooldown_ms:
            return False
        try:
            snd=self.external_sfx_cache.get(path)
            if snd is None:
                snd=pygame.mixer.Sound(path); self.external_sfx_cache[path]=snd
            ch=pygame.mixer.find_channel(True); ch.set_volume(clamp(self.sfx_volume*volume,0.0,1.0)); ch.play(snd)
            self.last_play["ext:"+role]=now
            return True
        except Exception as exc:
            print("Falha ao carregar SFX externo:",path,exc)
            return False
        ...
        if in_battle:
            ...
            if maho is not None and not getattr(maho, "dead", True):
                if self._try_external_music("mahoraga_battle", vol):
                    return
            # O Julgamento do Sans SEMPRE usa MEGALOVANIA/tema do Sans, independente do personagem do jogador.
            if getattr(game,"game_mode",None)=="sans_trial":
                if self._try_external_music("sans_theme", vol):
                    return
            # Sans jogavel tambem possui tema proprio. Mahoraga continua tendo prioridade acima.
            if getattr(getattr(game,"player",None),"character",None)=="Sans":
                if self._try_external_music("sans_theme", vol):
                    return
...
CHARACTER_CONFIG = {
    ...
    "Sans": {
        ...
        "desc": "1 HP | Esquiva por Estamina | Ossos a distancia | Blaster dedicado",
    ...
    MAHORAGA_MODULE_KEY: {
        "hp": 210, "stamina": 175, "weapon": 20, "weapon_name": "ESPADA DE EXORCIZACAO", "damage": 24.0, "speed": 300,
        "regen": 10.5, "color": YELLOW, "domain": "SANTUARIO DA ADAPTACAO",
        "desc": "Recompensa da Provação Mahoraga | Espada de Exorcização | Adaptação progressiva",
        "domain_desc": "Santuário da Adaptação: dura 20s. A roda gira a cada 1s e concede +10% de adaptação temporária até 80%. Quanto maior a adaptação, maior a resistência, dano, velocidade e recuperação de cooldown. Ao terminar, volta ao valor persistente anterior. A carga demora 4x mais que uma expansão comum.",
    },
    "Vasco": {
    ...
    HANK_KEY: {
        "hp": 240, "stamina": 150, "weapon": 16, "weapon_name": "CANHAO PESADO", "damage": 24.0, "speed": 246,
        "regen": 9.0, "color": (92, 92, 92), "domain": "SEJA BEM VINDO (OU NAO)",
        "desc": "Firme 1.5s uma vez | Enquanto parado atira sem cooldown | Tanque de artilharia",
        "domain_desc": "Segure DOM 3s: prende inimigos, 2x velocidade, regenera HP/Estamina, fica imortal e troca o canhao por facas duplas.",
    },
    SUKUNA_KEY: {
...
# ============================================================
CHARACTER_ORDER = ["Ana", "Kevyn", "Ycaro", "Kayk", "Pedro", "Ruan", "Vasco", LIRA_KEY, HANK_KEY, "Sans", "Potential Man", MAHORAGA_MODULE_KEY, "Pai do Kayk", "Strikada Egoísta", "Glonk 100% Power", "Vinicius 13", "Glonk"]
BETA_CHARACTER_ORDER = [SUKUNA_KEY, RIP_INDRA_KEY, HISOKA_KEY]
...
CHARACTER_BASE_RATINGS = {
    ...
    MAHORAGA_MODULE_KEY: {"strength":9,"speed":8,"resistance":9,"hp":9,"stamina":8,"parry":8},
...
CHARACTER_TAGS = {
    ...
    "Pai do Kayk": {"MELEE", "AOE", "DOMAIN", "PARRY"},
    "Sans": {"PROJECTILE", "DASH", "DOMAIN"},
    ...
    MAHORAGA_MODULE_KEY: {"MELEE", "ADAPT", "DASH", "DOMAIN"},
...
CHARACTER_GUIDE = {
    "Ana": ("INVOQUE Mini-Anas e pressione o mapa com explosoes.", "Acumule invocacoes antes de waves perigosas; elas sao melhores em grupos."),
    "Kevyn": ("Lute de perto com a espada e use PARRY para devolver projeteis.", "Aproxime-se com seguranca; o kit recompensa timing e combate corpo a corpo."),
    "Ycaro": ("Shotgun: aproxime para concentrar pellets ou mantenha distancia para cobrir area.", "Chefes derretem quando varios pellets acertam ao mesmo tempo."),
    "Kayk": ("Duas pistolas de alma com alto ritmo e boa mobilidade.", "Mantenha pressao constante e use o Dominio para saturar a arena."),
    "Pedro": ("Abacaxi bumerangue causa dano na ida e na volta.", "Posicione-se para o retorno atravessar o maior numero de inimigos."),
    "Ruan": ("Dash ofensivo recruta inimigos derrotados como Amigos.", "Mate com o proprio Dash para montar exercito; proteja Amigos valiosos."),
    "Vasco": ("Apostas alteram seu dano; 100% dentro do Dominio ativa JACKPOT.", "Construa a aposta antes de entrar em perigo e aproveite a invulnerabilidade do Jackpot."),
    LIRA_KEY: ("Projeteis censuram atributos; completar censuras desarma inimigos temporariamente.", "Espalhe debuffs antes do Dominio e escolha as censuras que counteram a wave."),
    HANK_KEY: ("Fique parado 1,5s para firmar o canhao; depois pode mirar e disparar livremente ate se mover.", "Escolha um ponto seguro e use o PARRY no layout normal enquanto segura a posicao."),
    "Potential Man": ("Toque invoca Sapos; segure ATK para Caes Divinos; ULT invoca Mahoraga.", "Use os Caes como linha de frente e so provoque Mahoraga quando estiver preparado."),
    "Pai do Kayk": ("Golpe circular de grande area e Dominio de sentenca.", "Entre no meio da horda antes de atacar para aproveitar o alcance total."),
    "Strikada Egoísta": ("Chute uma bola que procura novos alvos a cada quique.", "Priorize corredores cheios; cada quique aumenta muito o valor do ataque."),
    "Glonk 100% Power": ("1 HP, lingua larga e dano propositalmente ridiculo.", "Sobreviver e o desafio. Abuse de posicionamento, PARRY e Power Ups absurdos."),
    "Vinicius 13": ("Mire manualmente com o segundo analogico. Cada acerto gera Sucata; as torretas sao sua principal fonte de poder.", "O projetil so faz uma pequena correcao de rota se passar perto do alvo: a mira ainda e sua responsabilidade."),
    MAHORAGA_MODULE_KEY: ("Ataque com a Espada de Exorcizacao para cortar inimigos e rebater projeteis. ADAPTAR deixa Mahoraga parado e invulneravel por 4s.", "Se for atacado durante ADAPTAR, ele ganha +5% de resistencia geral permanente, ate 70%. O Dominio leva a adaptacao temporariamente ate 80%."),
    "Glonk": ("Glonk quase nao luta e inevitavelmente perde HP.", "Personagem-desafio. O objetivo e sobreviver ao absurdo pelo maior tempo possivel."),
    "Sans": ("Ossos no segundo analogico; BLAST planta o Gaster Blaster. O DASH e um teleporte de longo alcance com 0,5s de tela apagada.", "Durante BAD TIME, BLAST vira ALMA. Sans e desbloqueado ao vencer seu JULGAMENTO no NORMAL ou superior."),
    SUKUNA_KEY: ("BETA: cortes a distancia; toque ULT entra na mira do Fuuga, toque na arena para escolher o alvo; segure ULT abre Santuário.", "Misture Fuuga e Dominio. Dedos de Boss aumentam dano e HP durante a run."),
    RIP_INDRA_KEY: ("BETA: toque = True Triple Yoru larga; segure = tres ondas carregadas.", "Carregar de perto faz as tres ondas sobreporem e causa dano enorme."),
    HISOKA_KEY: ("BETA: cartas bumerangue; segure para carta paralisante; revive uma vez.", "Depois do Bombeia-Gum, use a Aranha para puxar grupos inteiros para perto."),
}

def character_unlock_text(name):
    if name=="Kayk": return "Derrote o boss Pai do Kayk em uma run real."
    if name=="Pai do Kayk": return "Derrote o Pai do Kayk com Ana, Kevyn, Ycaro, Kayk, Pedro e Ruan."
    if name==HANK_KEY: return "Derrote A Sentinela com Ana, Kevyn, Ycaro, Kayk, Pedro e Ruan."
    if name=="Strikada Egoísta": return "Alcance a Wave 30 no PADRAO em dificuldade NORMAL ou superior."
    if name=="Glonk 100% Power": return f"Morra 100 vezes usando Glonk em runs reais. Progresso: {int(SAVE.get('glonk_deaths',0) if 'SAVE' in globals() else 0)}/100."
    if name==MAHORAGA_MODULE_KEY: return "Vença a PROVAÇÃO do Mahoraga no NORMAL ou superior e resgate a recompensa única."
    if name=="Sans": return "Venca o JULGAMENTO do Sans no NORMAL ou superior."
    if name==RIP_INDRA_KEY:
        if 'SAVE' not in globals(): return "Complete 50% das conquistas."
        normals=[a[0] for a in ACHIEVEMENTS if a[0]!="meta_all"]; done=sum(1 for k in normals if k in set(SAVE.get("unlocked_achievements",[])))
        return f"Complete 50% das conquistas. Progresso: {done}/{len(normals)}."
    if name==HISOKA_KEY:
        n=len(set(SAVE.get("hisoka_impossible_wave30_chars",[]))) if 'SAVE' in globals() else 0
        return f"Wave 30 no IMPOSSIVEL com 3 personagens normais nao-principais. Beta nao conta. Progresso: {n}/3."
    if name in BETA_CHARACTER_ORDER: return "Disponivel para TESTE BETA. Beta nao concede recompensas/progresso permanente."
...
CHARACTER_UPGRADES = {
    ...
    "Pai do Kayk": [
        ...
        ("epic", "PALMADA REINCIDENTE", "Sobreviventes recebem um segundo impacto menor 1s depois", "father_repeat_slap"),
        ("legendary", "AQUI QUEM MANDA SOU EU", "Atingidos ficam mais lentos e causam menos dano", "father_i_rule_here"),
        ("divine", "SENTENCA PATERNA", "Matar 3+ inimigos no mesmo golpe cria uma segunda onda de choque", "father_paternal_sentence"),
    ],
    "Vinicius 13": [
    ...
    HANK_KEY: [
        ...
        ("epic", "CICLO DE FERROLHO", "Tempo para firmar o canhao cai para 1,1s", "hank_bolt_cycle"),
...
FORGE_CHARACTER_ORDER = ["Ana", "Kevyn", "Ycaro", "Kayk", "Pedro", "Ruan", "Vasco", LIRA_KEY, "Pai do Kayk", "Glonk", "Sans", "Strikada Egoísta", "Glonk 100% Power", "Potential Man"]
FORGE_WEAPONS = [
    # KEVYN
...
PERMANENT_UPGRADES = [
    ("vitality_level", "VITALIDADE", "+5 HP e +3 Estamina por nivel efetivo", 110),
    ("damage_level", "DANO BRUTO", "+2,5% dano inicial por nivel efetivo", 150),
    ("speed_level", "PASSOS RAPIDOS", "+0,75% velocidade por nivel efetivo (teto +30%)", 140),
    ("stamina_level", "RESERVA EXTRA", "+6 Estamina maxima por nivel efetivo", 110),
    ("regen_level", "PULMAO TREINADO", "+2% regeneracao de Estamina por nivel efetivo", 125),
    ("armor_level", "PELE DE FERRO", "-1% dano recebido por nivel efetivo (ate 35%)", 175),
    ("healing_level", "CURA EFICIENTE", "+2% cura recebida por nivel efetivo", 130),
    ("domain_level", "RITUAL ACELERADO", "+2% carga de Dominio recebida por nivel efetivo", 160),
    ("domain_range_level", "ALCANCE DO RITUAL", "+1% alcance de Dominio/ULT por nivel efetivo", 220),
    ("domain_duration_level", "RITUAL PROLONGADO", "+0,75% duracao de Dominio/ULT por nivel efetivo", 240),
    ("orb_level", "IMA DE ORBES", "+1% chance de drops por nivel efetivo", 120),
    ("coin_level", "GANANCIA SAUDAVEL", "+2% moedas obtidas por nivel efetivo", 180),
    ("dash_level", "IMPULSO", "-1% cooldown do Dash por nivel efetivo (ate 35%)", 150),
    ("crit_level", "MIRA NERVOSA", "+0,5% chance critica por nivel efetivo (ate 18%)", 190),
]
PERMANENT_UPGRADE_CAP = 60

def permanent_effective_level(key_or_level):
    """Retorno decrescente: LV1-20 = 100%, 21-40 = 50%, 41-60 = 25%."""
    if isinstance(key_or_level, str):
        lvl=max(0,min(PERMANENT_UPGRADE_CAP,int(SAVE.get(key_or_level,0) or 0))) if 'SAVE' in globals() else 0
    else:
        lvl=max(0,min(PERMANENT_UPGRADE_CAP,int(key_or_level or 0)))
    return min(lvl,20) + max(0,min(lvl-20,20))*0.50 + max(0,min(lvl-40,20))*0.25

def permanent_upgrade_cost(base, level):
    # Curva progressiva: +14% por nivel. Com ~400K moedas, da para montar uma boa conta
    # (cerca de LV25 medio na loja inteira), mas maximizar LV60 continua sendo projeto de longo prazo.
    return max(int(base), int(round(float(base) * (1.14 ** max(0,int(level))))))

def format_big_money(value):
    value=int(value)
    for div,suf in ((10**15,'Q'),(10**12,'T'),(10**9,'B'),(10**6,'M'),(10**3,'K')):
        if value>=div:
...
WHATS_NEW_V15 = [
    'V15.4: Sans dispara ossos continuamente ao segurar o analogico de mira; Blasters da BAD TIME agora travam automaticamente nos inimigos.',
    'V15.18: Provacao Mahoraga e Julgamento Sans exigem NORMAL+ para vitoria/recompensa; Dominio do boss 30% maior, prende quem entra e Corte do Mundo 2x mais frequente.',
    'V15.19: Corrige HUD/carga do Dominio do Mahoraga jogavel, ADAPT toca a roda durante os 4s, Santuário da Adaptacao ganha visual da Roda Eterna, carga 4x mais lenta e duracao 20s.',
    'V15.3: Provacao Mahoraga ganhou escudos de fase, fuga inteligente, Parry reativo, Dominio soberano e recompensa unica completa.',
    'V15.1: novo modo MAHORAGA traz um boss com IA adaptativa, 99% de adaptacao maxima e counters aprendidos durante a luta.',
    'V15.1: Hank firma o canhao uma unica vez enquanto estiver parado; depois pode mirar/disparar livremente, recebeu mais defesa e PARRY reposicionado.',
    'V15.2: Loja Permanente vai ate LV.60, sobe 14% de preco por nivel, tem retorno decrescente e upgrades de alcance/duracao de ULT.',
    'V15.1: novo Catalogo de Personagens mostra kit, como usar, dicas, tags visiveis e condicoes de desbloqueio.',
    'V15 PATCH: Pecados ganharam escala propria e dano reduzido; Luxuria nao herda mais combat wave 700.',
    'V15 PATCH: Mahoraga e Caes Divinos escalam melhor; hitboxes de Rip_Indra, Sukuna e Hank foram corrigidas.',
    'V15 PATCH: FPS aparece no canto da tela e a interface recebeu melhorias para elencos maiores.',
...
DEFAULT_SAVE = {
    ...
    "dash_level": 0,
    "crit_level": 0,
    "domain_range_level": 0,
    "domain_duration_level": 0,
    "best_wave": 0,
    "best_kills": 0,
    "mode_records": {},
    "mahoraga_trial_wins": 0,
    "mahoraga_module_unlocked": False,
    "mahoraga_reward_claimed": False,
    "mahoraga_divine_choice_pending": False,
    "sans_trial_wins": 0,
    "sans_unlocked": False,
    "sans_reward_claimed": False,
    "kayk_unlocked": False,
    "father_victories": [],
    ...
    "cheat_zero_cooldown": False,
    "cheat_enemy_start_hp": -1,
    "cheat_enemy_start_stamina": -1,
...
def passive_set_slot(character, key, slot):
    ...
    cleared=None
    # Duplicata EXATA nao pode apagar/mover a copia que ja estava equipada.
    if other == key:
        return False, None
    # A passiva NOVA tem prioridade apenas em conflitos de FAMILIA/raridade diferentes.
    if other:
...
SAVE = load_save()
# V15.1: limite real de 60 niveis para upgrades permanentes, com retorno decrescente.
...
def _draw_bone_fallback(center, direction, length, width, color=WHITE):
    c=pygame.Vector2(center); d=pygame.Vector2(direction)
    if d.length_squared()<=0.0001: d=pygame.Vector2(1,0)
    d=d.normalize(); p=pygame.Vector2(-d.y,d.x); hl=length*0.5; hw=width*0.5
    pts=[c-d*hl-p*hw,c+d*hl-p*hw,c+d*hl+p*hw,c-d*hl+p*hw]
    outer=[c-d*(hl+S(2))-p*(hw+S(2)),c+d*(hl+S(2))-p*(hw+S(2)),c+d*(hl+S(2))+p*(hw+S(2)),c-d*(hl+S(2))+p*(hw+S(2))]
    pygame.draw.polygon(SCREEN,DARK,outer); pygame.draw.polygon(SCREEN,color,pts)


def draw_sans_bone_small(center, direction, length, width, color=WHITE):
    img=sans_bone_small_surface(length,width,direction)
    c=pygame.Vector2(center)
    if img is not None:
        SCREEN.blit(img,img.get_rect(center=(int(c.x),int(c.y))))
        return
    _draw_bone_fallback(center,direction,length,width,color)


def draw_sans_bone_large(center, direction, length, width, color=WHITE):
    img=sans_bone_large_surface(length,width,direction)
    c=pygame.Vector2(center)
    if img is not None:
        SCREEN.blit(img,img.get_rect(center=(int(c.x),int(c.y))))
        return
    # Se o asset grande faltar, usa desenho procedural, NUNCA o PNG pequeno.
    _draw_bone_fallback(center,direction,length,width,color)
...
def sans_bone_projectile_hits(pr, point, target_radius):
    """Hitbox do osso NORMAL acompanha a haste visual inteira, perpendicular ao movimento."""
    v=pygame.Vector2(getattr(pr,"vel",(1,0)))
    if v.length_squared() <= 0.0001: v=pygame.Vector2(1,0)
    move=v.normalize(); side=pygame.Vector2(-move.y,move.x)
    current=pygame.Vector2(pr.pos); previous=pygame.Vector2(getattr(pr,"prev_pos",current))
    travel=current-previous; travel_len=travel.length()
    center=(previous+current)*0.5
    half_travel=travel_len*0.5 + S(16) + float(target_radius)
    half_bone=S(92) + float(target_radius)  # visual total ~= 184px
    rel=pygame.Vector2(point)-center
    along=abs(rel.dot(move)); across=abs(rel.dot(side))
    return along <= half_travel and across <= half_bone
...
class Projectile:
    ...
    def draw(self, offset):
        ...
        if kind in ("sans_bone", "sans_bad_time_bone"):
            ...
            draw_sans_bone_small(self.pos + offset, d, S(184 if kind == "sans_bone" else 196), S(30), WHITE)
            return
...
class RuanServant:
    ...
    def update(self, dt, game):
        ...
        if getattr(game,"game_mode","normal")=="mahoraga_trial" and getattr(game,"mahoraga",None) is not None and not game.mahoraga.dead:
            targets.append(game.mahoraga)
        if self.guardian:
        ...
        if combat_kind == "shooter":
            ...
            if dist > self.radius + target.radius + S(12):
                ...
                if isinstance(target,MahoragaSummon): target.take_damage(self.damage*attack_mult,game,self.pos,from_player=True)
                else: target.damage(self.damage*attack_mult, game, self.pos)
                game.current_damage_kind = old
...
class SansBoneObstacle:
    ...
    def draw(self, offset):
        ...
        draw_sans_bone_large(self.pos + offset, d, self.length, max(S(38), self.width*2.6), WHITE)



class SansSoulWallBone:
    """Osso retangular criado pelo impacto da ALMA na parede."""
    NEXT_ID = 5000000
    def __init__(self, pos, direction, damage, length=None):
        self.id = SansSoulWallBone.NEXT_ID; SansSoulWallBone.NEXT_ID += 1
        self.pos = pygame.Vector2(pos)
        self.direction = pygame.Vector2(direction)
        if self.direction.length_squared() <= 0.001:
            self.direction = pygame.Vector2(1,0)
        self.direction = self.direction.normalize()
        self.length = float(length if length is not None else S(145))
        self.width = S(16)
    ...
    def endpoints(self):
        d=self.direction*(self.length*0.5)
        return self.pos-d, self.pos+d
    ...
    def update(self, dt, game):
        ...
        self.hit_cd = {k:max(0.0,v-dt) for k,v in self.hit_cd.items() if v-dt>0}
        a,b=self.endpoints()
        old=getattr(game,"current_damage_kind",None); game.current_damage_kind="sans_soul_bone"
        try:
            for e in list(getattr(game,"enemies",[])):
                try:
                    if getattr(e,"dead",False) or self.hit_cd.get(getattr(e,"id",id(e)),0)>0:
                        continue
                    radius=getattr(e,"radius",S(20))
                    radius=float(radius) if isinstance(radius,(int,float)) else float(S(20))
                    if point_segment_distance(pygame.Vector2(e.pos),a,b) <= self.width+radius:
                        try:
                            e.damage(self.damage,game,self.pos,minimal_fx=True)
                        except TypeError:
                            e.damage(self.damage,game,self.pos)
                        self.hit_cd[getattr(e,"id",id(e))]=0.62
                except Exception as exc:
                    print("ALMA osso ignorou alvo:",exc)
        finally:
            game.current_damage_kind=old
        return self.life>0

    def draw(self, offset):
        draw_sans_bone_large(self.pos+offset,self.direction,self.length,max(S(38),self.width*2.6),WHITE)
    ...
    def update(self,dt,game):
        ...
        if self.pos.distance_to(self.target_pos)<=S(45) or self.life<=0:
            ...
            if target is not None and not getattr(target,"dead",True):
                impact=min(target.hp,target.max_hp*0.50)
                if isinstance(target,MahoragaSummon): target.take_damage(impact,game,center,from_player=True)
                else: target.damage(impact,game,center)
...
class Enemy:
    ...
    def die(self, game):
        ...
        reward = int(self.coin_value * game.coin_multiplier * reward_mult * (1.0 + 0.02 * permanent_effective_level("coin_level")) * max(0.10,1.0+getattr(game.player,"passive_coin_bonus",0.0)))
...
class SinBoss(Boss):
    """Pecados e Arcebispos com escala propria, separada da compressao geral das waves."""
    def __init__(self, pos, wave, sin_cfg, archbishop=False):
        # Antes, Luxuria na wave 100 herdava combat wave 700 e DEPOIS recebia buffs de Pecado.
        # Agora a ordem do pecado define uma referencia bem mais suave; a wave real fica so para agenda/UI.
        sin_rank = next((i for i,cfg in enumerate(SIN_SEQUENCE) if cfg["key"]==sin_cfg["key"]), 0)
        reference_wave = 10 + sin_rank*2 + (1 if not archbishop else 0)
        super().__init__(pos, reference_wave)
        self.encounter_wave = wave
        self.display_wave = wave
        self.sin_rank = sin_rank
        ...
        # Arcebispo = demonstracao; Pecado verdadeiro ainda e boss especial,
        # mas sem multiplicar novamente o endgame comprimido.
        mult = 1.22 if archbishop else 1.85
        self.max_hp *= mult
        self.hp = self.max_hp
        self.speed *= 0.92 if archbishop else 1.05
        self.boss_damage_scale *= 0.68 if archbishop else 0.82
        ...
        if self.sin_key == "gula" and not archbishop:
            ...
            self.boss_damage_scale *= 1.0+min(0.80,0.08*hunger)
    ...
    def update(self, dt, game):
        ...
        if self.sin_key == "wrath" and self.sin_tick <= 0:
            ...
            for i in range(shots):
                ...
                game.projectiles.append(Projectile(self.pos,d*S(330), (8+self.wave*0.22)*self.boss_damage_scale*self.lira_outgoing_mult(game)*strength*difficulty_cfg(getattr(game,"difficulty","easy"))["damage"],"enemy",S(9),RED,2.5))
            ...
            if to.length_squared()>0:
                ...
                for off in (-0.24,0,0.24):
                    game.projectiles.append(Projectile(self.pos,vec_from_angle(a+off)*S(390),(9+self.wave*0.20)*self.boss_damage_scale*self.lira_outgoing_mult(game)*strength*difficulty_cfg(getattr(game,"difficulty","easy"))["damage"],"enemy",S(10),GREEN,3.0))
            ...
            for i in range(6 if self.archbishop else 10):
                ...
                game.projectiles.append(Projectile(self.pos,d*S(205),(7+self.wave*0.16)*self.boss_damage_scale*self.lira_outgoing_mult(game)*strength*difficulty_cfg(getattr(game,"difficulty","easy"))["damage"],"enemy",S(13),BLUE,4.0))
            ...
            if to.length_squared()>0:
                ...
                for off in (-0.42,-0.21,0,0.21,0.42):
                    game.projectiles.append(Projectile(self.pos,vec_from_angle(a+off)*S(360),(8+self.wave*0.18)*self.boss_damage_scale*self.lira_outgoing_mult(game)*strength*difficulty_cfg(getattr(game,"difficulty","easy"))["damage"],"enemy",S(9),PINK,3.0))
...
class DivineDog:
    ...
    def __init__(self, pos, player, side=1):
        ...
        self.side = side
        self.radius = S(24)
        # V15 PATCH: cada Cao possui 80% do HP maximo atual do Potential Man.
        # O piso de 105 preserva o balanceamento no inicio da progressao.
        self.max_hp = max(105.0, float(player.max_hp) * 0.80)
        self.hp = self.max_hp
...
class MahoragaSummon:
    ...
    def __init__(self, pos, player, wave):
        ...
        self.player_ref=player
        self.radius=S(72)
        # Mahoraga deve sobreviver ao ambiente conforme a run escala.
        self.max_hp=max(1600.0 + max(0,wave-1)*55.0, float(player.max_hp)*6.0)
        self.hp=self.max_hp
        self.speed=S(170)
    ...
    def take_damage(self, amount, game, source_pos=None, from_player=False):
        ...
        # Adaptacao maxima = o ambiente comum ja nao consegue finalizar Mahoraga.
        # O proprio jogador ainda consegue derrota-lo normalmente.
        if not from_player and self.resistance >= 0.80:
            dealt = min(dealt, max(0.0, self.hp-1.0))
        self.hp -= dealt
    ...
    def update(self, dt, game):
        ...
        if self.resistance > 0:
            # A roda tambem melhora a recuperacao: no cap, ~2% do HP por segundo.
            self.hp=min(self.max_hp,self.hp+self.max_hp*(0.025*self.resistance)*dt)
...
class AdaptiveMahoragaBoss(MahoragaSummon):
    """Boss exclusivo do modo MAHORAGA: observa, aprende, sobrevive e fecha a luta em fases."""
    def __init__(self,pos,player,difficulty="easy"):
        super().__init__(pos,player,1)
        dcfg=difficulty_cfg(difficulty)
        self.radius=S(88)
        diff_hp={"easy":0.80,"normal":0.95,"hard":1.10,"monarch":1.40,"impossible":1.65}.get(difficulty,1.0)
        # V15.3: HP inicial baixo; as tres barreiras de fase impedem burst de pular a luta inteira.
        self.max_hp=max(2400.0*diff_hp, float(player.max_hp)*10.0)
        self.hp=self.max_hp
        self.base_speed=S(155)*dcfg["speed"]
        self.speed=self.base_speed
        self.base_damage=max(24.0,player.max_hp*0.065)*dcfg["damage"]
        self.damage=self.base_damage
        self.adaptation_pct=0
        self.next_adapt_gain=50
        self.adapt_timer=10.0
        self.elapsed=0.0
        self.attack_cd=0.85
        self.special_cd=4.0
        self.parry_cd=3.5
        self.parry_window=0.0
        self.learned_parry=False
        self.projectile_threats_seen=set()
        self.learned_predictive_dash=False
        self.learned_stationary_counter=False
        self.stationary_observe=0.0
        self.focus_servants=False
        self.focus_turrets=False
        self.focus_dogs=False
        self.dash_observations=0
        self._dash_was_active=False
        self.start_parries=0
        self.dash_cd=2.2
        self.dash_time=0.0
        self.dash_dir=pygame.Vector2(1,0)
        self.name="MAHORAGA — IA ADAPTATIVA"
        self.resistance=0.0
        self.player_damage_adaptation=0.0
        # Escudos de fase: qualquer dano que cruzaria o limiar para exatamente nele e ativa 2s de invulnerabilidade.
        self.phase_shields=(0.80,0.50,0.20)
        self.phase_shields_used=set()
        self.shield_timer=0.0
        # Sobrevivencia: com HP baixo e adaptacao insuficiente, ele para de atacar e compra tempo fugindo.
        self.fleeing=False
        self.flee_announced=False
        # 八握剣・輪廻神域 — Santuário da Roda Eterna.
        # Barreira fechada e soberana: sem Clash. Dura 15s quando o jogador e capturado.
        self.domain_used=False
        self.domain_active=False
        self.domain_duration=15.0
        self.domain_timer=0.0
        self.domain_center=pygame.Vector2(self.pos)
        self.domain_radius=S(731.25)  # V15.18: 30% maior que 562.5
        self.domain_player_trapped=False
        self.domain_break_timer=-1.0  # legado; o dominio nao quebra mais por estar vazio
        self.domain_wheel_timer=3.0
        self.domain_world_cut_timer=1.5
        self.domain_cut_kind="world"
        self.domain_spin_count=0
        self.domain_visual_angle=0.0
        self.domain_recent_damage={}
        self.domain_adapt_category=None
        self.domain_adapt_stacks=0
        self.domain_adapt_resistance=0.0
        self.domain_adaptation_complete=False
        self.domain_cut_pending=False
        self.domain_cut_timer=0.0
        self.domain_cut_start=pygame.Vector2()
        self.domain_cut_end=pygame.Vector2()
        self.domain_analysis_started=False
        self.domain_final_wave_delay=0.0
        self.domain_final_wave_active=False
        self.domain_final_wave_launched=False
        self.domain_final_wave_radius=0.0
        self.domain_final_wave_prev_radius=0.0
        self.domain_final_wave_hit=False

    def notify(self,game,msg,color=YELLOW):
        game.add_mahoraga_ai_message(msg,color)

    def _damage_category(self,game,kind=None):
        raw=str(kind if kind is not None else getattr(game,"current_damage_kind","") or "").lower()
        if any(k in raw for k in ("servant","guardian","dog","summon","friend","turret","miniana","orbital")):
            return "INVOCACAO"
        if any(k in raw for k in ("explosion","artillery","fuuga","blast","bomb","detonation")):
            return "EXPLOSAO"
        if any(k in raw for k in ("domain","magic","staff","reality","soul","purple","censor","sukuna","curse")):
            return "MAGIA"
        if any(k in raw for k in ("projectile","shot","bolt","shell","card","bone","pineapple","ball","arrow","bullet")):
            return "PROJETEIS"
        # Quando o caller nao marcou o tipo, usa o estilo dominante do personagem como fallback.
        char=getattr(getattr(game,"player",None),"character","")
        if char in ("Ana","Kayk","Lira Solis, A C#nsur#"):
            return "MAGIA"
        if char in ("Ruan",):
            return "INVOCACAO"
        return "FISICO"

    def _play_wheel_spin(self,game,volume=0.92):
        if not AUDIO.play_external_sfx("mahoraga_wheel",volume,120):
            AUDIO.play("domain",0.62,120)

    def activate_sovereign_domain(self,game):
        if self.domain_used or self.dead:
            return False
        self.domain_used=True
        self.domain_active=True
        self.domain_timer=self.domain_duration
        self.domain_center=pygame.Vector2(self.pos)
        self.domain_radius=S(731.25)  # 30% maior
        trapped_radius=max(S(40),self.domain_radius-getattr(game.player,"radius",S(20))-S(8))
        self.domain_player_trapped=game.player.pos.distance_to(self.domain_center)<=trapped_radius
        self.domain_break_timer=-1.0
        self.domain_wheel_timer=3.0
        self.domain_world_cut_timer=1.5
        self.domain_cut_kind="world"
        self.domain_spin_count=0
        ...
        game.domain_clash_boss=None
        self.notify(game,"八握剣・輪廻神域 — SANTUARIO DA RODA ETERNA",YELLOW)
        self.notify(game,"A roda gira a cada 3s e adapta UMA categoria por vez.",CYAN)
        if not self.domain_player_trapped:
            self.notify(game,"A barreira permanece ativa. Se o jogador entrar, nao podera mais sair.",GRAY)
        game.boss_domain_message="SANTUARIO DA RODA ETERNA"
        game.boss_domain_message_timer=2.6
        game.shake=max(game.shake,S(20)); game.flash_screen=max(game.flash_screen,0.10)
        AUDIO.play("domain",1.0,250)
        return True

    def _keep_inside_domain(self,entity):
        if not self.domain_active or not hasattr(entity,"pos"):
            return
        delta=pygame.Vector2(entity.pos)-self.domain_center
        radius=max(S(40),self.domain_radius-getattr(entity,"radius",S(20))-S(8))
        if delta.length_squared()>radius*radius:
            if delta.length_squared()<1e-6:
                delta=pygame.Vector2(1,0)
            entity.pos=self.domain_center+delta.normalize()*radius

    def _end_eternal_wheel_domain(self,game):
        ...
        self.domain_final_wave_launched=False
        self.domain_analysis_started=False
        self.notify(game,"A Roda Eterna parou. As adaptacoes do Dominio foram apagadas.",GRAY)

    def _prepare_extinction_cut(self,game,kind="world"):
        # A linha nasce mirando a posicao ATUAL do jogador; os 0,8s de aviso permitem sair dela.
        self.domain_cut_kind=str(kind or "world")
        ang=random.uniform(0.0,math.tau)
        d=vec_from_angle(ang)
        center=pygame.Vector2(game.player.pos)
        reach=max(W,H)*1.25
        self.domain_cut_start=center-d*reach
        self.domain_cut_end=center+d*reach
        self.domain_cut_timer=0.80
        self.domain_cut_pending=True

    def _domain_wheel_spin(self,game):
        self.domain_spin_count+=1
        self._play_wheel_spin(game,1.0 if self.domain_spin_count>=4 else 0.88)
        category=None
        if self.domain_recent_damage:
            category=max(self.domain_recent_damage,key=lambda k:self.domain_recent_damage.get(k,0.0))
        if category and self.domain_recent_damage.get(category,0.0)>0:
            if category==self.domain_adapt_category:
                self.domain_adapt_stacks=min(4,self.domain_adapt_stacks+1)
            else:
                self.domain_adapt_category=category
                self.domain_adapt_stacks=1
                self.domain_adaptation_complete=False
            levels=(0.15,0.30,0.50,0.90)
            self.domain_adapt_resistance=levels[self.domain_adapt_stacks-1]
            if self.domain_adapt_stacks>=4:
                self.domain_adaptation_complete=True
                self.domain_adapt_resistance=0.90
                game.boss_domain_message="「 ADAPTACAO CONCLUIDA 」"
                game.boss_domain_message_timer=1.8
                self.notify(game,f"ADAPTACAO CONCLUIDA: {category} — 90% MENOS DANO",RED)
                game.flash_screen=max(game.flash_screen,0.11); game.shake=max(game.shake,S(14))
            else:
                self.notify(game,f"RODA {self.domain_adapt_stacks}/4: {category} — -{int(self.domain_adapt_resistance*100)}% dano",YELLOW)
        else:
            self.notify(game,"A roda girou, mas nao encontrou dano suficiente para analisar.",GRAY)
        self.domain_recent_damage={}

    def _start_final_analysis(self,game):
        if self.domain_analysis_started:
            return
        self.domain_analysis_started=True
        self.domain_final_wave_delay=1.05
        self.domain_final_wave_active=False
        self.domain_final_wave_launched=False
        self.domain_final_wave_radius=S(72)
        self.domain_final_wave_prev_radius=self.domain_final_wave_radius
        self.domain_final_wave_hit=False
        game.boss_domain_message="O General Divino terminou sua analise."
        game.boss_domain_message_timer=2.4
        self.notify(game,"O General Divino terminou sua analise.",WHITE)
        game.shake=max(game.shake,S(8))

    def _update_domain_mechanics(self,dt,game):
        if not self.domain_active:
            return
        self.domain_timer=max(0.0,self.domain_timer-dt)
        fast_spin=self.domain_analysis_started or self.domain_timer<=3.0
        self.domain_visual_angle=(self.domain_visual_angle+dt*(8.5 if fast_spin else 0.65))%math.tau
        self._keep_inside_domain(self)
        # V15.18: o dominio nao quebra se estiver vazio. Quem entrar depois fica preso ate o fim.
        if not self.domain_player_trapped:
            trap_radius=max(S(40),self.domain_radius-getattr(game.player,"radius",S(20))-S(8))
            if game.player.pos.distance_to(self.domain_center)<=trap_radius:
                self.domain_player_trapped=True
                self.notify(game,"A barreira reconheceu o jogador. Agora ele nao pode mais sair.",YELLOW)
                game.damage_texts.append(DamageText("PRESO NO SANTUARIO",pygame.Vector2(game.player.pos),YELLOW,0.8))
        if self.domain_player_trapped:
            self._keep_inside_domain(game.player)
            # A barreira e fechada: aliados/invocacoes que estao dentro tambem nao conseguem sair.
            for group_name in ("servants","potential_summons","vinicius_turrets"):
                for ent in list(getattr(game,group_name,[])):
                    try:
                        if not getattr(ent,"dead",False) and pygame.Vector2(ent.pos).distance_to(self.domain_center)<=self.domain_radius+getattr(ent,"radius",0):
                            self._keep_inside_domain(ent)
                    except Exception:
                        pass

        self.domain_wheel_timer-=dt
        while self.domain_wheel_timer<=0 and self.domain_spin_count<4:
            self.domain_wheel_timer+=3.0
            self._domain_wheel_spin(game)

        # V15.18: Corte do Mundo aparece a cada 1,5s — o dobro da frequencia antiga (3s).
        if not self.domain_analysis_started:
            self.domain_world_cut_timer-=dt
            if self.domain_world_cut_timer<=0:
                self.domain_world_cut_timer+=1.5
                if not self.domain_cut_pending:
                    self._prepare_extinction_cut(game,"world")

        if self.domain_cut_pending:
            self.domain_cut_timer-=dt
            if self.domain_cut_timer<=0:
                self.domain_cut_pending=False
                if point_segment_distance(game.player.pos,self.domain_cut_start,self.domain_cut_end)<=S(30)+game.player.radius:
                    game.player.take_damage(game.player.max_hp*0.35,game)
                    game.damage_texts.append(DamageText("CORTE DO MUNDO -35%",pygame.Vector2(game.player.pos),RED,0.9))
                game.shake=max(game.shake,S(14)); AUDIO.play("kevyn_sword",0.95,70)

        if self.domain_timer<=3.0:
            self._start_final_analysis(game)

        if self.domain_analysis_started:
            self.domain_final_wave_delay-=dt
            if self.domain_final_wave_delay<=0 and not self.domain_final_wave_active and not self.domain_final_wave_launched:
                self.domain_final_wave_active=True
                self.domain_final_wave_launched=True
                self.domain_final_wave_radius=S(72)
                self.domain_final_wave_prev_radius=self.domain_final_wave_radius
                self.domain_final_wave_hit=False
                self.notify(game,"八握剣 — JULGAMENTO DO GENERAL DIVINO",YELLOW)
                game.shake=max(game.shake,S(12))
            if self.domain_final_wave_active:
                self.domain_final_wave_prev_radius=self.domain_final_wave_radius
                wave_speed=max(S(260),self.domain_radius/1.15)
                self.domain_final_wave_radius+=wave_speed*dt
                pd=game.player.pos.distance_to(self.domain_center)
                safe_radius=S(92)
                crossed=(self.domain_final_wave_prev_radius-S(18) <= pd <= self.domain_final_wave_radius+S(28))
                if crossed and not self.domain_final_wave_hit:
                    self.domain_final_wave_hit=True
                    if pd<=safe_radius:
                        game.damage_texts.append(DamageText("ZONA SEGURA",pygame.Vector2(game.player.pos),CYAN,0.8))
                    elif game.player.dash_time>0:
                        game.damage_texts.append(DamageText("ATRAVESSOU A ONDA!",pygame.Vector2(game.player.pos),CYAN,0.8))
                    else:
                        game.player.take_damage(game.player.max_hp*0.50,game)
                        game.damage_texts.append(DamageText("JULGAMENTO -50%",pygame.Vector2(game.player.pos),RED,1.0))
                if self.domain_final_wave_radius>=self.domain_radius+S(60):
                    self.domain_final_wave_active=False

        if self.domain_timer<=0:
            self._end_eternal_wheel_domain(game)

    def _domain_hit_player(self,game,damage):
        # A nova Expansao nao possui mais HITKILL. Ataques normais continuam normais.
        game.player.take_damage(damage,game)
        return False

    def _adapt_step(self,game):
        gain=min(self.next_adapt_gain,99-self.adaptation_pct)
        if gain<=0:return
        self.adaptation_pct+=gain
        self.next_adapt_gain=max(1,self.next_adapt_gain//2)
        ratio=self.adaptation_pct/100.0
        self.resistance=min(0.94,0.95*ratio)
        self.player_damage_adaptation=2.0*ratio
        self.speed=self.base_speed*(1.0+1.05*ratio)
        self.notify(game,f"Mahoraga se adaptou: {self.adaptation_pct}% | menos dano, mais dano e mais velocidade.")
        if self.adaptation_pct>=75 and not self.domain_used:
            self.activate_sovereign_domain(game)
        if self.adaptation_pct>=99:
            game.start_mahoraga_final_countdown()
        game.shake=max(game.shake,S(10)); self._play_wheel_spin(game,0.78)

    def _activate_phase_shield(self,game,threshold):
        self.phase_shields_used.add(threshold)
        self.hp=max(self.hp,self.max_hp*threshold)
        self.shield_timer=2.0
        pct=int(threshold*100)
        self.notify(game,f"ESCUDO DE ADAPTACAO: {pct}% HP — INVULNERAVEL POR 2s",CYAN)
        game.damage_texts.append(DamageText(f"ESCUDO {pct}%",pygame.Vector2(self.pos),CYAN,1.0))
        game.spawn_particles(self.pos,CYAN,24); game.shake=max(game.shake,S(12)); AUDIO.play("shield",0.95,120)

    def take_damage(self,amount,game,source_pos=None,from_player=False):
        if self.dead:return
        if self.shield_timer>0:
            game.damage_texts.append(DamageText("INVULNERAVEL",pygame.Vector2(self.pos),CYAN,0.42))
            return
        if from_player and self.learned_parry and self.parry_window>0:
            self.parry_window=0.0
            self.parry_cd=max(self.parry_cd,1.6)
            game.damage_texts.append(DamageText("PARRY!",pygame.Vector2(self.pos),YELLOW,0.7))
            game.spawn_particles(self.pos,YELLOW,10)
            d=game.player.pos-self.pos
            if d.length_squared()>0:
                d=d.normalize(); pr=Projectile(self.pos+d*S(55),d*S(880),max(12.0,self.base_damage*0.75),"enemy",S(10),YELLOW,1.5); pr.kind="mahoraga_parry_counter"; game.projectiles.append(pr)
            return
        category=self._damage_category(game)
        if self.domain_active:
            resist=self.domain_adapt_resistance if category==self.domain_adapt_category else 0.0
        else:
            resist=self.resistance
        dealt=max(0.0,amount)*(1.0-clamp(resist,0.0,0.95))
        old_hp=self.hp
        proposed=self.hp-dealt
        # Nao deixa dano explosivo atravessar as tres fases de escudo de uma vez.
        for threshold in self.phase_shields:
            if threshold in self.phase_shields_used:
                continue
            line=self.max_hp*threshold
            if old_hp>line and proposed<=line:
                self.hp=line
                self._activate_phase_shield(game,threshold)
                dealt=max(0.0,old_hp-self.hp)
                break
        else:
            self.hp=proposed
        if from_player and dealt>0:
            if self.domain_active:
                self.domain_recent_damage[category]=self.domain_recent_damage.get(category,0.0)+dealt
            domain_raw=getattr(game,"current_domain_charge_raw_override",None); game.current_domain_charge_raw_override=None
            if domain_raw is not None and domain_raw>0: game.add_domain_charge(domain_raw)
        if dealt>0: game.spawn_particles(self.pos,self.color,4)
        if self.hp<=0:
            self.hp=0; self.dead=True
            if self.domain_active:self._end_eternal_wheel_domain(game)
            game.damage_texts.append(DamageText("MAHORAGA FOI EXORCIZADO",pygame.Vector2(self.pos),YELLOW,1.2))

    def _pick_target(self,game):
        if self.focus_turrets:
            ts=[t for t in game.vinicius_turrets if not t.dead]
            if ts:return min(ts,key=lambda x:x.pos.distance_to(self.pos))
        if self.focus_servants:
            ss=[s for s in game.servants if not s.dead]
            if ss:return min(ss,key=lambda x:x.pos.distance_to(self.pos))
        if self.focus_dogs:
            ds=[d for d in game.potential_summons if isinstance(d,DivineDog) and not d.dead]
            if ds:return min(ds,key=lambda x:x.pos.distance_to(self.pos))
        return game.player

    def _hit_target(self,target,damage,game):
        if self.domain_active:
            damage *= (1.0 + 0.10*min(4,int(getattr(self,"domain_spin_count",0))))
        if target is game.player:
            self._domain_hit_player(game,damage)
        elif hasattr(target,"take_damage"):
            target.take_damage(damage,game)

    def _incoming_projectile(self,game):
        best=None; best_t=999.0
        for pr in game.projectiles:
            if getattr(pr,"owner",None)!="player" or getattr(pr,"hit_mahoraga",False):
                continue
            vel=pygame.Vector2(getattr(pr,"vel",(0,0)))
            speed=vel.length()
            if speed<1.0:
                continue
            to_me=self.pos-pr.pos; dist=to_me.length()
            if dist>S(760) or dist<1.0:
                continue
            approach=vel.normalize().dot(to_me/dist)
            if approach<0.74:
                continue
            lateral=abs(to_me.cross(vel.normalize()))
            if lateral>self.radius+getattr(pr,"radius",S(8))+S(70):
                continue
            t=dist/speed
            if t<best_t:
                best,best_t=pr,t
    ...
    def reflect_projectile(self,pr,game):
        """Parry real: devolve o mesmo projetil ao jogador, como o Parry jogavel."""
        if self.parry_window<=0 or getattr(pr,"owner",None)!="player":
            return False
        self.parry_window=0.0
        self.parry_cd=max(self.parry_cd,1.6)
        pr.owner="enemy"
        ...
        game.damage_texts.append(DamageText("PARRY! REFLETIDO",pygame.Vector2(self.pos),YELLOW,0.7))
        game.spawn_particles(self.pos,YELLOW,12)
        game.shake=max(game.shake,S(9))
        AUDIO.play("parry",0.95,80)
        return True

    def _learn(self,game,dt):
        if game.player.character=="Ruan" and not self.focus_servants and any(not s.dead for s in game.servants):
            self.focus_servants=True; self.notify(game,"Mahoraga entendeu o Ruan: vai derrotar os Amigos primeiro.",PINK)
        if game.player.character=="Vinicius 13" and not self.focus_turrets and any(not t.dead for t in game.vinicius_turrets):
            self.focus_turrets=True; self.notify(game,"Mahoraga entendeu o Vinicius: as torretas agora sao prioridade.",ORANGE)
        if game.player.character=="Potential Man" and not self.focus_dogs and any(isinstance(d,DivineDog) and not d.dead for d in game.potential_summons):
            self.focus_dogs=True; self.notify(game,"Mahoraga reconheceu os Caes Divinos e vai elimina-los primeiro.",CYAN)
        active=game.player.dash_time>0
        if active and not self._dash_was_active:self.dash_observations+=1
        self._dash_was_active=active
        if not self.learned_predictive_dash and self.dash_observations>=2:
            self.learned_predictive_dash=True; self.notify(game,f"Mahoraga aprendeu como {game.player.character} usa o Dash. Agora ele prediz a rota.",CYAN)
        threat,ttc=self._incoming_projectile(game)
        if threat is not None:
            pid=id(threat)
            if pid not in self.projectile_threats_seen:
                self.projectile_threats_seen.add(pid)
                if len(self.projectile_threats_seen)>48:
                    self.projectile_threats_seen=set(list(self.projectile_threats_seen)[-24:])
            if not self.learned_parry and (len(self.projectile_threats_seen)>=3 or self.adaptation_pct>=50):
                self.learned_parry=True
                self.notify(game,"Mahoraga identificou projeteis em rota de colisao. PARRY REATIVO aprendido.",YELLOW)
        if not self.learned_parry and game.parries-self.start_parries>=2:
            self.learned_parry=True; self.notify(game,f"Mahoraga aprendeu o PARRY de {game.player.character}. Agora ele tambem pode bloquear ataques.",YELLOW)
        if game.player.character==HANK_KEY and not self.learned_stationary_counter:
            if game.player.still_timer>=1.5:self.stationary_observe+=dt
            else:self.stationary_observe=max(0.0,self.stationary_observe-dt*0.5)
            if self.stationary_observe>=2.25:
                self.learned_stationary_counter=True; self.notify(game,"Mahoraga entendeu a Artilharia do Hank: ficar parado agora atrai investidas e pressao.",ORANGE)
        return threat,ttc

    def update(self,dt,game):
        if self.dead:return False
        self.elapsed+=dt
        if not self.domain_active:self.adapt_timer-=dt
        self.attack_cd=max(0.0,self.attack_cd-dt); self.special_cd=max(0.0,self.special_cd-dt)
        self.parry_cd=max(0.0,self.parry_cd-dt); self.parry_window=max(0.0,self.parry_window-dt); self.dash_cd=max(0.0,self.dash_cd-dt)
        self.shield_timer=max(0.0,self.shield_timer-dt); self.slash_fx=max(0.0,self.slash_fx-dt)
        if self.domain_active:
            self._update_domain_mechanics(dt,game)
            if not self.domain_active:
                pass
            elif self.domain_analysis_started:
                # Nos 3s finais Mahoraga para de atacar e apenas conclui a analise.
                self.pos.x=clamp(self.pos.x,self.radius,W-self.radius); self.pos.y=clamp(self.pos.y,S(175)+self.radius,H-S(340)-self.radius)
                return not self.dead
        while (not self.domain_active) and self.adapt_timer<=0 and self.adaptation_pct<99:
            self.adapt_timer+=10.0; self._adapt_step(game)
        threat,ttc=self._learn(game,dt)
        ratio=self.adaptation_pct/100.0
        hp_ratio=self.hp/max(1.0,self.max_hp)
        # Parry so abre quando um projetil REALMENTE esta vindo na direcao dele.
        if self.learned_parry and self.parry_cd<=0 and threat is not None and ttc<=0.70:
            self.parry_window=clamp(ttc+0.13,0.20,0.52)
            self.parry_cd=max(1.25,2.8-1.1*ratio)
            game.damage_texts.append(DamageText("PARRY REATIVO",pygame.Vector2(self.pos),YELLOW,0.50))
        # HP baixo + pouca adaptacao: para de atacar e foge para comprar segundos de observacao.
        self.fleeing=(hp_ratio<=0.35 and self.adaptation_pct<75 and not self.domain_active)
        if self.fleeing:
            if not self.flee_announced:
                self.flee_announced=True; self.notify(game,"HP baixo e adaptacao insuficiente: Mahoraga esta GANHANDO TEMPO.",ORANGE)
            away=self.pos-game.player.pos
            if away.length_squared()<1.0:away=vec_from_angle(random.random()*math.tau)
            away=away.normalize(); self.facing=-away
            if self.dash_cd<=0:
                self.dash_dir=away; self.dash_time=0.20; self.dash_cd=max(0.8,1.7-0.6*ratio)
            if self.dash_time>0:
                self.dash_time-=dt; self.pos+=self.dash_dir*self.speed*4.7*dt
            else:
                self.pos+=away*self.speed*1.35*dt
            self.pos.x=clamp(self.pos.x,self.radius,W-self.radius); self.pos.y=clamp(self.pos.y,S(175)+self.radius,H-S(340)-self.radius)
            return not self.dead
        elif self.flee_announced and (hp_ratio>0.35 or self.adaptation_pct>=75):
            self.flee_announced=False
        target=self._pick_target(game)
        delta=target.pos-self.pos; dist=max(1.0,delta.length()); d=delta/dist; self.facing=pygame.Vector2(d)
        if self.learned_stationary_counter and target is game.player and game.player.still_timer>=1.5 and not self.domain_active:
            self.dash_cd=min(self.dash_cd,0.30); self.attack_cd=min(self.attack_cd,0.20)
        if (self.learned_predictive_dash or self.learned_stationary_counter) and self.dash_cd<=0 and target is game.player:
            mv=game.get_move_dir(); predicted=game.player.pos+(mv.normalize()*S(190) if mv.length_squared()>0 else pygame.Vector2())
            dd=predicted-self.pos
            if dd.length_squared()>0:self.dash_dir=dd.normalize(); self.dash_time=0.24; self.dash_cd=max(0.75,2.25-1.2*ratio)
        if self.dash_time>0:
            self.dash_time-=dt; self.pos+=self.dash_dir*self.speed*(4.2+1.8*ratio)*dt
            if self.pos.distance_to(target.pos)<=self.radius+getattr(target,"radius",S(25))+S(12):
                self._hit_target(target,self.base_damage*(1.0+2.0*ratio)*1.15,game); self.dash_time=0
        else:
            desired=S(135 if target is not game.player else 170)
            if dist>desired:self.pos+=d*self.speed*dt
        if self.attack_cd<=0 and game.state=="playing":
            atk=self.base_damage*(1.0+2.0*ratio)
            if dist<=S(205)+getattr(target,"radius",0):
                self._hit_target(target,atk,game); self.slash_fx=0.16; AUDIO.play("kevyn_sword",0.72,90)
            elif target is game.player:
                pr=Projectile(self.pos+d*S(60),d*S(620+500*ratio),atk*0.82,"enemy",S(12),self.color,2.2)
                pr.kind="mahoraga_adaptive_slash"; pr.mahoraga_owner=self; game.projectiles.append(pr)
            self.attack_cd=max(0.18,0.88*(1.0-0.72*ratio))
        if self.adaptation_pct>=75 and self.special_cd<=0 and game.state=="playing":
            self.special_cd=max(1.8,4.8-2.2*ratio)
            radius=S(255); game.spawn_particles(self.pos,YELLOW,22); game.shake=max(game.shake,S(10))
            if game.player.pos.distance_to(self.pos)<=radius+game.player.radius:self._hit_target(game.player,self.base_damage*(0.75+ratio),game)
            for ss in list(game.servants):
                if not ss.dead and ss.pos.distance_to(self.pos)<=radius+ss.radius:ss.take_damage(self.base_damage*(0.9+ratio),game)
            for t in list(game.vinicius_turrets):
                if not t.dead and t.pos.distance_to(self.pos)<=radius+t.radius:t.take_damage(self.base_damage*(0.9+ratio),game)
        self.pos.x=clamp(self.pos.x,self.radius,W-self.radius); self.pos.y=clamp(self.pos.y,S(175)+self.radius,H-S(340)-self.radius)
        if self.domain_active:
            self._keep_inside_domain(self)
            if self.domain_player_trapped:
    ...
    def draw(self,offset):
        super().draw(offset)
        p=self.pos+offset
        draw_text(f"IA {self.adaptation_pct}%",FONT_S,YELLOW,(p.x,p.y+self.radius+S(36)),True)
        if self.parry_window>0:pygame.draw.circle(SCREEN,YELLOW,(int(p.x),int(p.y)),self.radius+S(18),max(2,S(5)))
        if self.shield_timer>0:
            pygame.draw.circle(SCREEN,CYAN,(int(p.x),int(p.y)),self.radius+S(24),max(2,S(8)))
            draw_text(f"ESCUDO {self.shield_timer:.1f}s",FONT_S,CYAN,(p.x,p.y-self.radius-S(82)),True)


class SansBossBoneHazard:
    def __init__(self,pos,direction,length,damage,mode="white",life=7.0,motion="vertical",amplitude=None,move_speed=None):
        self.base_pos=pygame.Vector2(pos); self.pos=pygame.Vector2(pos); self.direction=pygame.Vector2(direction)
        if self.direction.length_squared()<0.001:self.direction=pygame.Vector2(1,0)
        self.direction=self.direction.normalize(); self.length=float(length); self.width=S(15)
        self.damage=float(damage); self.mode=mode; self.life=float(life); self.hit_cd=0.0
        self.motion=motion; self.motion_time=random.random()*math.tau
        self.amplitude=float(S(70) if amplitude is None else amplitude)
        self.move_speed=float(random.uniform(1.4,2.15) if move_speed is None else move_speed)
        self.spin_speed=random.choice((-1,1))*random.uniform(80.0,135.0)
    def endpoints(self):
        d=self.direction*(self.length*0.5); return self.pos-d,self.pos+d
    def update(self,dt,game):
        self.life-=dt; self.hit_cd=max(0.0,self.hit_cd-dt); self.motion_time+=dt*self.move_speed
        if self.life<=0:return False
        if self.motion=="vertical":
            self.pos=self.base_pos+pygame.Vector2(0,math.sin(self.motion_time)*self.amplitude)
        elif self.motion=="horizontal":
            self.pos=self.base_pos+pygame.Vector2(math.sin(self.motion_time)*self.amplitude,0)
        elif self.motion=="spin":
            self.pos=pygame.Vector2(self.base_pos)
            self.direction=self.direction.rotate(self.spin_speed*dt)
        p=game.player; a,b=self.endpoints()
        if self.hit_cd<=0 and point_segment_distance(p.pos,a,b)<=self.width+p.radius:
            moving=game.get_move_dir().length_squared()>0.02 or p.dash_time>0
            active=(self.mode=="white") or (self.mode=="blue" and moving) or (self.mode=="orange" and not moving)
            if active:
                p.take_damage(self.damage,game); self.hit_cd=0.55
        return True
    def draw(self,offset):
        p=self.pos+offset; thickness=max(S(30),int(self.width*2.15))
        img = sans_bone_special_surface(self.mode,self.length,thickness,self.direction) if self.mode in ("blue","orange") else sans_bone_large_surface(self.length,thickness,self.direction)
        if img is not None:
            SCREEN.blit(img,img.get_rect(center=(int(p.x),int(p.y))))
        else:
            d=pygame.Vector2(self.direction)
            if d.length_squared()<=0.001:d=pygame.Vector2(1,0)
            d=d.normalize(); a=p-d*(self.length*0.5); b=p+d*(self.length*0.5)
            col=CYAN if self.mode=="blue" else (ORANGE if self.mode=="orange" else WHITE)
            pygame.draw.line(SCREEN,col,(int(a.x),int(a.y)),(int(b.x),int(b.y)),max(S(8),thickness))


class SansBossBlasterHazard:
    def __init__(self,pos,target,damage,scale=1.0):
        self.pos=pygame.Vector2(pos); self.target=pygame.Vector2(target); self.damage=float(damage); self.scale=float(scale)
        d=self.target-self.pos; self.direction=d.normalize() if d.length_squared()>0.001 else pygame.Vector2(1,0)
        self.timer=0.62; self.total=0.62; self.beam_timer=0.0; self.fired=False; self.dead=False; self.sound_played=False
    def update(self,dt,game):
        if self.dead:return False
        if not self.sound_played:
            self.sound_played=True
            if not AUDIO.play_external_sfx("sans_blaster",0.82,45): AUDIO.play("boss",0.52,45)
        if not self.fired:
            d=game.player.pos-self.pos
            if d.length_squared()>0.001 and self.timer>0.08:self.direction=d.normalize()
            self.timer-=dt
            if self.timer<=0:
                self.fired=True; self.beam_timer=0.20
                d=self.direction; start=self.pos+d*S(58); end=ray_to_arena_edge(start,d)
                if beam_hits_point_fast(game.player.pos,start,d,max(1.0,(end-start).length()),S(40)*self.scale+game.player.radius):
                    game.player.take_damage(self.damage,game)
                game.shake=max(game.shake,S(8))
        else:
            self.beam_timer-=dt
            if self.beam_timer<=0:self.dead=True
        return not self.dead
    def draw(self,offset):
        p=self.pos+offset; d=self.direction
        angle=math.degrees(math.atan2(d.y,d.x))-90.0
        if not self.fired:
            progress=clamp(1.0-max(0.0,self.timer)/self.total,0,1)
            idx=0 if progress<.28 else (1 if progress<.56 else (2 if progress<.86 else 3))
        else: idx=3
        img=sans_blaster_frame_surface(idx,int(S(185)*self.scale),angle)
        if img is not None: SCREEN.blit(img,img.get_rect(center=(int(p.x),int(p.y))))
        if self.fired and self.beam_timer>0:
            start=p+d*S(58); end=ray_to_arena_edge(start,d)
            pygame.draw.line(SCREEN,CYAN,start,end,max(S(16),int(S(50)*self.scale)))


class SansTrialBoss(Enemy):
    """Boss de resistencia: Sans sobrevive gastando Estamina para esquivar."""
    def __init__(self,pos,game):
        super().__init__(pos,"kiter",1,None)
        self.kind="sans_trial_boss"; self.radius=S(34); self.max_hp=1.0; self.hp=1.0; self.speed=0.0; self.contact_damage=0.0
        self.color=CYAN; self.name="SANS — BOSSFIGHT"
        # V15.15.4: valor fixo pedido para a provacao.
        self.max_stamina=1000.0
        self.stamina=self.max_stamina
        self.dodge_cost=max(24.0,self.max_stamina/25.0)
        self.time_since_attack=0.0; self.regen_tick=0.0; self.wave_index=0
        self.final_judgement=False; self.relocate_cd=3.4
        self.soul_push_timer=0.0; self.soul_push_dir=pygame.Vector2()
        self.soul_cd=random.uniform(2.2,3.0)
        self.flash=0; self.lira_censor={"strength":0.0,"speed":0.0,"resistance":0.0,"intelligence":0.0}; self.lira_domain_stats=set(); self.lira_fully_censored=False
        self.wave_started=False; self.intro_timer=0.40
        # Fase final de desespero.
        self.desperation_active=False
        self.desperation_timer=0.0
        self.desperation_blasts_left=0
        self.desperation_total_blasts=12
        self.exhausted=False
        self.teleport_far(game,play_effect=False)

    def _arena_bounds(self):
        return S(80),W-S(80),S(215),H-S(385)

    def _teleport_effect(self,game):
        game.sans_teleport_blackout=max(getattr(game,"sans_teleport_blackout",0.0),0.50)
        if not AUDIO.play_external_sfx("sans_teleport",1.0,70):
            AUDIO.play("dash",0.82,70)

    def teleport_far(self,game,play_effect=True):
        left,right,top,bottom=self._arena_bounds()
        spots=[pygame.Vector2(left,top),pygame.Vector2(right,top),pygame.Vector2(left,bottom),pygame.Vector2(right,bottom)]
        self.pos=max(spots,key=lambda q:q.distance_to(game.player.pos))
        if play_effect:
            self._teleport_effect(game)

    def _hazard_damage(self,game,ratio=.05):
        return max(5.0,game.player.max_hp*ratio)*difficulty_cfg(getattr(game,"difficulty","easy"))["damage"]

    def _spread_positions(self,game,count):
        """Distribui slots pela arena inteira, sem formar escada/corredor."""
        left,right,top,bottom=self._arena_bounds()
        cols=7 if count>=21 else 6
        rows=4 if count>=21 else 3
        cell_w=(right-left)/float(cols)
        cell_h=(bottom-top)/float(rows)
        cells=[]
        for ry in range(rows):
            for cx in range(cols):
                x=left+(cx+0.5)*cell_w
                y=top+(ry+0.5)*cell_h
                x+=random.uniform(-cell_w*0.22,cell_w*0.22)
                y+=random.uniform(-cell_h*0.20,cell_h*0.20)
                q=pygame.Vector2(x,y)
                # Evita nascer exatamente em cima do jogador/Sans.
                if q.distance_to(game.player.pos)<S(105):
                    q+=pygame.Vector2(random.choice((-1,1))*S(90),random.choice((-1,1))*S(55))
                if q.distance_to(self.pos)<S(95):
                    q+=pygame.Vector2(random.choice((-1,1))*S(70),random.choice((-1,1))*S(45))
                q.x=clamp(q.x,left,right); q.y=clamp(q.y,top,bottom)
                cells.append(q)
        random.shuffle(cells)
        return cells[:min(count,len(cells))]

    def start_wave(self,game,initial=False):
        if self.exhausted or self.desperation_active:
            return
        game.sans_boss_bones=[]; game.sans_boss_blasters=[]
        if not initial:self.wave_index+=1
        pattern=self.wave_index%4

        # Ossos realmente espalhados por TODA a arena.
        total_bones=18 if not self.final_judgement else 28
        positions=self._spread_positions(game,total_bones)
        special_count=(6 if pattern==1 else 3) if not self.final_judgement else 13
        for i,center in enumerate(positions):
            mode="white"
            if i<special_count:
                mode="blue" if i%2==0 else "orange"
            direction=random.choice((pygame.Vector2(1,0),pygame.Vector2(0,1)))
            if self.final_judgement:
                motion=random.choice(("vertical","horizontal","spin"))
                amp=S(random.randint(55,95))
            else:
                motion="vertical"
                amp=S(random.randint(50,78))
            game.sans_boss_bones.append(
                SansBossBoneHazard(center,direction,S(random.randint(135,175)),self._hazard_damage(game,.04 if mode=="white" else .035),mode,6.6,motion=motion,amplitude=amp)
            )

        # Blasters em toda onda e mais frequentes na Verdadeira Batalha.
        left,right,top,bottom=self._arena_bounds()
        edge_spots=[
            pygame.Vector2(left,top),pygame.Vector2(right,top),pygame.Vector2(left,bottom),pygame.Vector2(right,bottom),
            pygame.Vector2((left+right)/2,top),pygame.Vector2((left+right)/2,bottom),pygame.Vector2(left,(top+bottom)/2),pygame.Vector2(right,(top+bottom)/2)
        ]
        random.shuffle(edge_spots)
        count=2 if not self.final_judgement else 4
        if pattern==2: count+=1
        for q in edge_spots[:count]:
            game.sans_boss_blasters.append(SansBossBlasterHazard(q,game.player.pos,self._hazard_damage(game,.075),1.50 if self.final_judgement else 1.20))

        # Ondas de ALMA extras, agora instantaneas.
        if pattern==3:
            self.trigger_soul_blast(game)

    def _spawn_soul_wall(self,game):
        p=pygame.Vector2(game.player.pos); d=pygame.Vector2(self.soul_push_dir)
        if d.length_squared()<0.001:return
        tangent=pygame.Vector2(-d.y,d.x); damage=self._hazard_damage(game,.04)
        for i in range(10):
            off=(i-4.5)*S(25); center=p+tangent*off-d*S(55)
            motion="vertical" if not self.final_judgement else ("spin" if i%3==0 else "horizontal")
            game.sans_boss_bones.append(SansBossBoneHazard(center,-d,S(150),damage,"blue" if i%2==0 else "orange",4.8,motion=motion,amplitude=S(42)))

    def trigger_soul_blast(self,game,desperation=False):
        """ALMA instantanea: explosao que joga o jogador direto numa parede."""
        p=game.player
        dirs=[pygame.Vector2(1,0),pygame.Vector2(-1,0),pygame.Vector2(0,1),pygame.Vector2(0,-1)]
        d=random.choice(dirs)
        self.soul_push_dir=pygame.Vector2(d)
        left,right,top,bottom=S(25)+p.radius,W-S(25)-p.radius,S(175)+p.radius,H-S(345)-p.radius
        if d.x>0:
            p.pos.x=right; p.pos.y=clamp(p.pos.y+random.uniform(-S(90),S(90)),top,bottom)
        elif d.x<0:
            p.pos.x=left; p.pos.y=clamp(p.pos.y+random.uniform(-S(90),S(90)),top,bottom)
        elif d.y>0:
            p.pos.y=bottom; p.pos.x=clamp(p.pos.x+random.uniform(-S(130),S(130)),left,right)
        else:
            p.pos.y=top; p.pos.x=clamp(p.pos.x+random.uniform(-S(130),S(130)),left,right)
        p.invuln=max(getattr(p,"invuln",0.0),0.12)
        if not AUDIO.play_external_sfx("sans_soul",0.95,120):
            AUDIO.play("domain",0.72,120)
        game.shake=max(game.shake,S(18 if desperation else 13))
        game.flash_screen=max(getattr(game,"flash_screen",0.0),0.04)
        if not desperation:
            self._spawn_soul_wall(game)
            game.damage_texts.append(DamageText("ALMA!",pygame.Vector2(p.pos),CYAN,0.45))

    def _start_desperation(self,game):
        self.desperation_active=True
        self.exhausted=False
        self.desperation_blasts_left=self.desperation_total_blasts
        self.desperation_timer=0.12
        self.stamina=0.0
        game.sans_boss_bones=[]; game.sans_boss_blasters=[]
        game.damage_texts.append(DamageText("...",pygame.Vector2(self.pos),WHITE,0.8))
        game.shake=max(game.shake,S(12))

    def _finish_desperation(self,game):
        self.desperation_active=False
        self.exhausted=True
        self.stamina=0.0
        game.sans_boss_bones=[]; game.sans_boss_blasters=[]
        game.player.hp=max(1.0,min(game.player.hp,1.0))
        game.damage_texts.append(DamageText("EXAUSTO",pygame.Vector2(self.pos),RED,2.0))
        game.damage_texts.append(DamageText("...AGORA.",pygame.Vector2(game.player.pos),WHITE,1.3))
        game.shake=max(game.shake,S(8))
    ...
    def damage(self,amount,game,source_pos=None,minimal_fx=False):
        ...
        if self.exhausted:
            self.hp=0; self.dead=True
            game.damage_texts.append(DamageText("SANS FOI ATINGIDO",pygame.Vector2(self.pos),YELLOW,1.2))
            game.sans_trial_victory_pending=True
            return
        if not self.wave_started:
            self.wave_started=True; self.intro_timer=0.0; self.start_wave(game,initial=True)
        self.time_since_attack=0.0; self.regen_tick=0.0
        if self.stamina>0:
            self.hp=1.0; self.dead=False
            self.stamina=max(0.0,self.stamina-self.dodge_cost)
            game.damage_texts.append(DamageText("MISS",pygame.Vector2(self.pos),WHITE,0.65))
            if not AUDIO.play_external_sfx("sans_dodge",1.0,55):AUDIO.play("perfect",0.8,55)
            if self.stamina<=self.max_stamina*0.50 and not self.final_judgement:
                self.final_judgement=True; self.relocate_cd=2.5; self.soul_cd=1.35
                game.damage_texts.append(DamageText("A VERDADEIRA BATALHA COMEÇA.",pygame.Vector2(self.pos),RED,1.8)); game.shake=max(game.shake,S(18))
            if self.stamina<=0:
                # O golpe que esgota a barra inicia a ultima tentativa desesperada.
                self._start_desperation(game)
                return
            # Toda esquiva/troca de lugar usa o MESMO efeito do teleporte jogavel.
            self.teleport_far(game,play_effect=True)
            self.start_wave(game)
            return

    def die(self,game):
        self.damage(999999,game)
    ...
    def update(self,dt,game):
        ...
        if not self.wave_started:
            ...
            if self.intro_timer<=0:
                self.wave_started=True; self.start_wave(game,initial=True)
            return True

        # Ultimo desespero: ALMA para todos os lados e HP do jogador nunca passa de 1 para baixo.
        if self.desperation_active:
            game.sans_boss_bones=[]; game.sans_boss_blasters=[]
            self.desperation_timer-=dt
            if self.desperation_timer<=0:
                remaining=max(1,self.desperation_blasts_left)
                self.trigger_soul_blast(game,desperation=True)
                p=game.player
                if self.desperation_blasts_left<=1:
                    p.hp=1.0
                else:
                    # Cada explosao parece fatal, mas a sequencia e estritamente nao letal.
                    chunk=max(p.max_hp*0.11,(p.hp-1.0)/remaining)
                    p.hp=max(1.0,p.hp-chunk)
                p.invuln=max(getattr(p,"invuln",0.0),0.16)
                self.desperation_blasts_left-=1
                self.desperation_timer=0.22
                if self.desperation_blasts_left<=0:
                    self._finish_desperation(game)
            return True
        ...
        self.time_since_attack+=dt; self.flash=max(0.0,self.flash-dt)
        # V15.15.4: Sans so recupera depois de 10s sem o jogador atacar.
        if self.time_since_attack>=10.0 and self.stamina<self.max_stamina:
            self.regen_tick+=dt
            if self.regen_tick>=1.0:
                self.regen_tick-=1.0; gain=self.max_stamina*0.015; self.stamina=min(self.max_stamina,self.stamina+gain)
                game.damage_texts.append(DamageText(f"SANS RECUPEROU +{int(gain)} STA",pygame.Vector2(self.pos),CYAN,0.8))

        # ALMA muito mais frequente e instantanea.
        self.soul_cd-=dt
        if self.soul_cd<=0:
            self.trigger_soul_blast(game)
            self.soul_cd=random.uniform(1.25,1.9) if self.final_judgement else random.uniform(2.0,2.8)

        if self.final_judgement:
            self.relocate_cd-=dt
            if self.relocate_cd<=0:
                self.relocate_cd=random.uniform(2.3,3.6)
                if self.pos.distance_to(game.player.pos)>S(210) and random.random()<0.62:
                    self.teleport_far(game,play_effect=True)
                    self.start_wave(game)
                    game.damage_texts.append(DamageText("AINDA NAO.",pygame.Vector2(self.pos),WHITE,0.65))
        return True

    def draw(self,offset):
        p=self.pos+offset
        img=sans_player_surface("down",0,max(S(118),int(self.radius*5.7)))
        if img is not None:SCREEN.blit(img,img.get_rect(midbottom=(int(p.x),int(p.y+self.radius+S(8)))))
        else:pygame.draw.circle(SCREEN,WHITE,(int(p.x),int(p.y)),self.radius)
        if self.exhausted:
            draw_text("EXAUSTO",FONT_S,RED,(p.x,p.y-self.radius-S(42)),True)
        elif self.desperation_active:
            draw_text("!!!",FONT_M,RED,(p.x,p.y-self.radius-S(42)),True)
        elif self.final_judgement:
            pygame.draw.circle(SCREEN,RED,(int(p.x),int(p.y)),self.radius+S(20),max(2,S(4)))
...
class Player:
    def __init__(self, game, character="Ycaro"):
        ...
        self.base_speed = S(cfg["speed"]) * (1 + min(0.30,0.0075 * permanent_effective_level("speed_level"))) * (1 + 0.015 * evo.get("speed", 0))
        self.base_speed *= max(0.10, 1.0 + self.passive_static.get("move",0.0))
        self.speed_mult = 1.0
        self.speed_buff = 0
        self.still_timer = 0.0

        self.max_hp = cfg["hp"] + permanent_effective_level("vitality_level") * 5 + evo.get("hp", 0) * 6
        if self.character not in ("Sans", "Glonk 100% Power"):
            self.max_hp *= max(0.10, 1.0 + self.passive_static.get("hp",0.0))
        self.hp = self.max_hp
        self.max_stamina = cfg["stamina"] + permanent_effective_level("vitality_level") * 3 + permanent_effective_level("stamina_level") * 6 + evo.get("stamina", 0) * 5
        ...
        self.stamina = self.max_stamina
        self.stamina_regen = cfg["regen"] * (1 + 0.02 * permanent_effective_level("regen_level")) * (1 + 0.015 * evo.get("stamina", 0))
        self.stamina_regen_delay = 0.0

        self.base_damage = cfg["damage"] * (1 + 0.025 * permanent_effective_level("damage_level")) * (1 + 0.03 * evo.get("strength", 0))
        ...
        self.crit_chance = min(0.18, 0.005 * permanent_effective_level("crit_level")) + self.passive_static.get("crit",0.0)
        self.crit_damage_mult = self.passive_static.get("crit_mult",1.5)
        self.drop_bonus = 0.01 * permanent_effective_level("orb_level")
        ...
        self.sans_anim_time = 0.0
        self.sans_walk_frame = 0
        self.sans_sprite_dir = "down"
        # Strikada Egoista
        ...
        # Mahoraga Modulo jogavel - Adaptar + Santuário da Adaptacao
        self.mahoraga_adaptation_pct = 0.0
        self.mahoraga_adapt_cd = 0.0
        self.mahoraga_adapt_timer = 0.0
        self.mahoraga_adapt_was_hit = False
        self.mahoraga_adapt_hit_count = 0
        self.mahoraga_domain_adapt_pct = 0.0
        self.mahoraga_domain_wheel_cd = 1.0
        self.mahoraga_domain_visual_angle = 0.0
        self.mahoraga_wheel_fx = 0.0
    ...
    def use_hank_artillery(self, game):
        ...
        for i in range(10):
            ...
            game.hank_artillery_queue.append({"pos": pos, "timer": 0.45 + 0.2*i, "radius": S(155)*(1.0+0.01*permanent_effective_level("domain_range_level")), "done": False})
        return True
    ...
    def spend_stamina(self, amount):
        if SAVE.get("cheat_infinite_stamina", False):
            return True
    ...
    def heal(self, amount, game=None):
        if game and (getattr(game, "healing_locked", False) or getattr(game,"game_mode","normal")=="extinction"):
            return
        old = self.hp
        amount *= 1.0 + 0.02 * permanent_effective_level("healing_level")
    ...
    def activate_sukuna_domain(self, game):
        ...
        game.sukuna_fuuga_aiming = False
        AUDIO.play("domain", 1.0, 250)
        ...
        game.domain_radius = math.hypot(W, H) * 1.15 * (1.0+0.01*permanent_effective_level("domain_range_level"))
        game.domain_duration = 8.0 * (1.0+0.0075*permanent_effective_level("domain_duration_level"))
    ...
    def effective_damage_mult(self, game, enemy=None, source="generic", distance=None):
        ...
        if self.character == MAHORAGA_MODULE_KEY:
            ar=self.mahoraga_effective_adaptation_pct(game)/100.0
            m *= 1.0 + 0.75*ar
        return m
    ...
    def take_damage(self, amount, game):
        ...
        if SAVE.get("cheat_immortal", False):
            ...
            return False
        if self.character == MAHORAGA_MODULE_KEY and self.mahoraga_adapt_timer > 0:
            self.mahoraga_adapt_was_hit=True
            self.mahoraga_adapt_hit_count+=1
            msgs=("Mahoraga Entendeu","Mahoraga Reconheceu","Mahoraga Aprendeu","Mahoraga Adaptou.")
            msg=msgs[min(self.mahoraga_adapt_hit_count-1,len(msgs)-1)]
            game.damage_texts.append(DamageText(msg,pygame.Vector2(self.pos),YELLOW,0.85))
            self.invuln=max(self.invuln,0.20)
            return False
        ...
        if self.character == "Sans":
            ...
            if not self.sans_exhausted and self.stamina > 0:
                ...
                if not AUDIO.play_external_sfx("sans_dodge", 1.0, 55):
                    AUDIO.play("perfect", 0.75, 110)
        ...
        if self.character == "Kevyn" and self.in_domain(game):
            ...
            return False
        if self.character == HANK_KEY and self.in_domain(game):
            game.damage_texts.append(DamageText("MADNESS - IMORTAL", pygame.Vector2(self.pos), RED, 0.45))
            return False
        if self.invuln > 0 or self.dash_time > 0:
            return False
        if self.shield > 0:
        ...
        if impossible_hit:
            ...
            if self.no_brakes:
                amount *= 1.25
            amount *= max(0.65, 1.0 - 0.01 * permanent_effective_level("armor_level"))
            amount *= max(0.625, 1.0 - 0.015 * self.evo_resistance)
            if self.character == HANK_KEY:
                amount *= 0.78  # blindagem pesada: 22% de resistencia natural fora do Dominio
        ...
        if self.character == LIRA_KEY and game.domain_active and "resistance" in self.lira_domain_buffs:
            amount *= 0.50
        if self.character == MAHORAGA_MODULE_KEY:
            amount *= max(0.20,1.0-self.mahoraga_effective_adaptation_pct(game)/100.0)
        if self.character == "Vasco":
    ...
    def _auto_aim(self, game, domain_only=False, max_range=None):
        candidates = [e for e in game.enemies if not e.dead]
        maho=getattr(game,"mahoraga",None)
        if getattr(game,"game_mode","normal")=="mahoraga_trial" and maho is not None and not maho.dead:
            candidates.append(maho)
        if domain_only:
    ...
    def rip_click_attack(self,game):
        ...
        rng=S(205)*self.rip_range_mult; hit_any=False
        d=pygame.Vector2(self.facing) if self.facing.length_squared() else pygame.Vector2(1,0)
        d=d.normalize(); a=self.pos+d*S(18); b=self.pos+d*rng
        half_width=S(82)*self.rip_range_mult
        for e in list(game.enemies):
            if e.dead: continue
            dist=e.pos.distance_to(self.pos)
            if point_segment_distance(e.pos,a,b) <= half_width + e.radius:
    ...
    def attack(self, game):
        ...
        # Mahoraga Modulo: Espada de Exorcizacao com o mesmo arco visual do Kevyn.
        # Alem de causar dano, a lamina rebate/destrói projeteis hostis dentro do arco.
        if self.character == MAHORAGA_MODULE_KEY:
            if self.mahoraga_adapt_timer > 0 or self.attack_cd > 0 or not self.spend_stamina(5):
                return
            adapt_ratio=self.mahoraga_effective_adaptation_pct(game)/100.0
            self.attack_cd=0.38
            rng=S(175); angle_limit=math.cos(math.radians(78)); hit_any=False; parried=False
            facing=self.facing.normalize() if self.facing.length_squared()>0 else pygame.Vector2(1,0)
            for pr in list(game.projectiles):
                if getattr(pr,"owner",None)!="enemy": continue
                to=pr.pos-self.pos; dist=to.length()
                in_arc=dist<=rng+getattr(pr,"radius",S(8)) and (dist<=self.radius+getattr(pr,"radius",S(8))+S(34) or (dist>0.001 and (to/dist).dot(facing)>=angle_limit))
                if not in_arc: continue
                if getattr(pr,"vel",pygame.Vector2()).length_squared()>0.001:
                    pr.owner="player"; pr.vel*=-1.35; pr.damage*=1.25; pr.color=YELLOW; pr.parry_reflected=True; parried=True
                else:
                    try: game.projectiles.remove(pr)
                    except ValueError: pass
                    parried=True
            old=game.current_damage_kind; game.current_damage_kind="mahoraga_module_sword"
            for e in list(game.enemies):
                if e.dead: continue
                to=e.pos-self.pos; dist=to.length()
                if dist<=rng+e.radius and (dist<=self.radius+e.radius+S(34) or (dist>0.001 and (to/dist).dot(facing)>=angle_limit)):
                    e.damage(self.base_damage*self.effective_damage_mult(game,e,"sword",dist),game,self.pos); hit_any=True
            maho=getattr(game,"mahoraga",None)
            if maho is not None and not maho.dead:
                to=maho.pos-self.pos; dist=to.length()
                if dist<=rng+maho.radius and (dist<=self.radius+maho.radius+S(34) or (dist>0.001 and (to/dist).dot(facing)>=angle_limit)):
                    maho.take_damage(self.base_damage*self.effective_damage_mult(game,None,"sword",dist),game,self.pos,from_player=True); hit_any=True
            game.current_damage_kind=old
            game.slash_fx=0.15
            game.spawn_particles(self.pos,YELLOW,10); AUDIO.play("kevyn_sword",0.82,70)
            if parried:
                AUDIO.play("parry",0.85,70)
                game.damage_texts.append(DamageText("PARRY DA LAMINA",pygame.Vector2(self.pos),YELLOW,0.45))
            if hit_any: game.register_combo_hit(); game.domain_charge_primary_hit(20.0)
            return

        # Sans beta: somente ossos a distancia. Direcao vem do segundo analogico de mira.
        # Blaster continua dedicado ao botao BLAST (antigo PARRY).
        if self.character == "Sans":
            if self.attack_cd > 0 or not self.spend_stamina(4):
                return
            ...
            game.projectiles.append(pr)
            self.attack_cd=0.56
            AUDIO.play("kevyn_sword",0.38,70)
            return
        ...
        if self.character == HANK_KEY:
            ...
            if not self.spend_stamina(6):
                return
            # Sem cooldown artificial depois do tiro: o limitador e posicionar/firmar o canhao novamente.
            self.attack_cd = 0.0
            d = game.attack_direction() if hasattr(game, "attack_direction") else pygame.Vector2(self.facing)
            if d.length_squared() == 0:
                d = pygame.Vector2(1,0)
        ...
        # Vinicius 13: mira manual. O tiro pode fazer UMA pequena correcao se passar perto do alvo, mas nao persegue.
        if self.character == "Vinicius 13":
            ...
            pr.kind="vinicius_bolt"; pr.origin=pygame.Vector2(self.pos); pr.vinicius_child=False; pr.vinicius_correction_used=False; pr.domain_charge_raw=20.0; pr.domain_charge_uses_left=1; game.projectiles.append(pr)
    ...
    def fire_sans_blaster(self, game, auto=False):
        ...
        spawn = None
        target_hint = None
        if auto and getattr(game, "bad_time_active", False):
            targets=[e for e in game.enemies if not getattr(e,"dead",False)]
            maho=getattr(game,"mahoraga",None)
            if maho is not None and not getattr(maho,"dead",True):
                targets.append(maho)
            if targets:
                target=min(targets,key=lambda e:e.pos.distance_to(self.pos))
                target_hint=pygame.Vector2(target.pos)
            # V15.9: Blasters automaticos da BAD TIME SEMPRE nascem dentro da arena.
            # A margem considera o tamanho do sprite para a cabeca inteira continuar visivel.
            margin_x=max(S(105), int(W*0.045))
            arena_top=S(165)+S(105)
            arena_bottom=H-S(335)-S(105)
            if arena_bottom <= arena_top:
                arena_top, arena_bottom = S(205), H-S(360)
            spawn=pygame.Vector2(
                random.uniform(margin_x, max(margin_x+1, W-margin_x)),
                random.uniform(arena_top, max(arena_top+1, arena_bottom))
            )
            if target_hint is None:
                target_hint=pygame.Vector2(self.pos)+pygame.Vector2(self.facing if pygame.Vector2(self.facing).length_squared()>0 else (1,0))*S(400)
            toward=target_hint-spawn
            d=toward.normalize() if toward.length_squared()>0.001 else pygame.Vector2(0,1)
        if d is None:
            d=game.attack_direction() if hasattr(game,"attack_direction") else pygame.Vector2(self.facing)
            if d.length_squared()==0:
                d=pygame.Vector2(1,0)
            d=d.normalize()
            spawn=pygame.Vector2(self.pos)+d*S(145)
        if not auto:
            self.facing=pygame.Vector2(d)
        game.sans_blaster_plants.append({
            "pos": pygame.Vector2(spawn),
            "dir": pygame.Vector2(d),
            "timer": 0.90,
            "total": 0.90,
            "auto": bool(auto and getattr(game, "bad_time_active", False)),
            "target_hint": pygame.Vector2(target_hint) if target_hint is not None else None,
            "width": S(72 if (auto and getattr(game, "bad_time_active", False)) else 96),
            "mult": 4.5 if (auto and getattr(game, "bad_time_active", False)) else 12.0,
            "mouth_offset": S(58),
        })
        self.sans_blaster_cd = 2.0 if game.bad_time_active else 15.0
        if not AUDIO.play_external_sfx("sans_blaster",0.95 if not auto else 0.65,180):
            AUDIO.play("domain" if auto else "boss",0.45 if auto else 0.62,180)
        return True

    def release_sans_blaster(self, game, plant):
        auto = bool(plant.get("auto", False) and getattr(game, "bad_time_active", False))
        d = pygame.Vector2(plant.get("dir", (1, 0)))
        start_anchor = pygame.Vector2(plant.get("pos", self.pos))
        if auto:
            targets=[e for e in game.enemies if not getattr(e,"dead",False)]
            maho=getattr(game,"mahoraga",None)
            if maho is not None and not getattr(maho,"dead",True):
                targets.append(maho)
            if targets:
                target=min(targets,key=lambda e:e.pos.distance_to(start_anchor))
                lock=pygame.Vector2(target.pos)-start_anchor
                if lock.length_squared()>0.001:
                    d=lock.normalize()
        if d.length_squared()==0:
            d=pygame.Vector2(1,0)
        d=d.normalize()
        start = start_anchor + d * float(plant.get("mouth_offset", S(58)))
        end = ray_to_arena_edge(start, d)
        beam_length = max(1.0, (end-start).length())
        width = float(plant.get("width", S(72 if auto else 96)))
        mult = float(plant.get("mult", 4.5 if auto else 12.0))
        old = game.current_damage_kind
        ...
        try:
            for e in list(game.enemies):
                ...
                if beam_hits_point_fast(e.pos, start, d, beam_length, width+e.radius):
                    e.damage(self.base_damage*mult*self.effective_damage_mult(game,e,"sans_blaster",e.pos.distance_to(start)),game,start, minimal_fx=True)
                    hit_any = True
        finally:
        ...
        if len(game.sans_blaster_fx) > 3:
            game.sans_blaster_fx = game.sans_blaster_fx[-3:]
        game.shake=max(game.shake,S(5 if auto else 8))
        game.flash_screen=max(game.flash_screen,0.03 if auto else 0.05)
        if hit_any:
            game.register_combo_hit()
            game.domain_charge_primary_hit(20.0)
        return True

    def use_sans_soul(self, game):
        """BAD TIME: arremessa ate os 3 inimigos mais fortes na direcao do Sans.

        V15.9.2: implementacao defensiva para Android. Nenhum alvo quebrado,
        atributo ausente ou erro isolado desta habilidade deve encerrar a partida.
        """
        if self.character != "Sans" or not getattr(game,"bad_time_active",False) or self.sans_exhausted:
            return False
        try:
            cost=max(1.0,float(self.max_stamina)*0.10)
            if float(self.stamina) < cost:
                game.damage_texts.append(DamageText("ALMA: SEM ESTAMINA",pygame.Vector2(self.pos),RED,0.65))
                AUDIO.play("hurt",0.35,120)
                return False

            if not hasattr(game,"sans_soul_throws") or not isinstance(game.sans_soul_throws,list):
                game.sans_soul_throws=[]
            if not hasattr(game,"sans_soul_wall_bones") or not isinstance(game.sans_soul_wall_bones,list):
                game.sans_soul_wall_bones=[]

            candidates=[]
            for enemy in list(getattr(game,"enemies",[])):
                if enemy is None or getattr(enemy,"dead",False):
                    continue
                try:
                    pygame.Vector2(getattr(enemy,"pos",self.pos))
                except Exception:
                    continue
            ...
            def soul_strength(enemy):
                try:
                    hp=getattr(enemy,"max_hp",0.0)
                    hp=float(hp) if isinstance(hp,(int,float)) else 0.0
                except Exception:
                    hp=0.0
                attack=getattr(enemy,"contact_damage",0.0)
                if not isinstance(attack,(int,float)):
                    attack=getattr(enemy,"base_damage",0.0)
                if not isinstance(attack,(int,float)):
                    attack=0.0
                return hp + float(attack)*18.0

            candidates.sort(key=soul_strength,reverse=True)
            targets=candidates[:3]
            d=pygame.Vector2(getattr(self,"facing",(1,0)))
            if d.length_squared()<=0.001:
                d=pygame.Vector2(1,0)
            d=d.normalize()

            self.stamina=max(0.0,float(self.stamina)-cost)
            self.stamina_regen_delay=max(float(getattr(self,"stamina_regen_delay",0.0)),0.65)

            # O MP3 original podia ser instavel no mixer Android. O ZIP V15.9.2
            # inclui sans_soul.wav e ele tem prioridade aqui.
            try:
                if not AUDIO.play_external_sfx("sans_soul",1.0,80):
                    AUDIO.play("domain",0.72,80)
            except Exception as exc:
                print("ALMA audio ignorado:",exc)

            throw_speed=float(S(2200))
            for enemy in targets:
                try:
                    game.sans_soul_throws=[t for t in game.sans_soul_throws if t.get("enemy") is not enemy]
                    game.sans_soul_throws.append({
                        "enemy":enemy,
                        "vel":pygame.Vector2(d)*throw_speed,
                        "life":1.45,
                        "dir":pygame.Vector2(d),
                    })
                    game.damage_texts.append(DamageText("ALMA!",pygame.Vector2(enemy.pos),CYAN,0.55))
                except Exception as exc:
                    print("ALMA alvo ignorado:",exc)
                    continue
            game.shake=max(float(getattr(game,"shake",0.0)),S(8))
            return bool(targets)
        except Exception as exc:
            # Ultima barreira: o botao ALMA nunca deve derrubar o APK.
            print("ERRO ALMA:",repr(exc))
            try:
                game.damage_texts.append(DamageText("ALMA: ERRO EVITADO",pygame.Vector2(self.pos),RED,0.8))
            except Exception:
                pass
            return False
    ...
    def dash(self, game, move_dir):
        ...
        if self.character == "Sans":
            dash_cost *= 3
            if self.sans_blue_shortcut:
                dash_cost = max(3, dash_cost - 3)
        ...
        # V15.13: Teleporte do Sans com alcance bem maior, cooldown normal e alto custo de estamina.
        if self.character == "Sans":
            old_dash_time = 0.18
            dash_base = min(self.base_speed, S(560))
            old_distance = dash_base * 3.2 * old_dash_time
            teleport_distance = old_distance * 6.0
            target = pygame.Vector2(self.pos) + self.dash_dir * teleport_distance
            target.x = clamp(target.x, self.radius, W-self.radius)
            target.y = clamp(target.y, S(205)+self.radius, H-S(335)-self.radius)
            self.pos = target
            self.facing = pygame.Vector2(self.dash_dir)
            self.dash_time = 0.0
            base_cd = 0.72 * max(0.65, 1.0 - 0.01 * permanent_effective_level("dash_level")) * max(0.75,1.0-self.passive_dash_recharge)
            if self.sans_blue_shortcut:
                base_cd *= 0.70
            self.dash_cd = base_cd
            self.invuln = max(self.invuln, 0.55)
            game.sans_teleport_blackout = max(getattr(game,"sans_teleport_blackout",0.0), 0.50)
            if not AUDIO.play_external_sfx("sans_teleport", 1.0, 80):
                AUDIO.play("dash", 0.85, 80)
            return
        ...
        self.dash_cd = dash_cd_base * max(0.65, 1.0 - 0.01 * permanent_effective_level("dash_level")) * max(0.75,1.0-self.passive_dash_recharge)
    ...
    def mahoraga_effective_adaptation_pct(self, game=None):
        if self.character != MAHORAGA_MODULE_KEY:
            return 0.0
        base = clamp(float(getattr(self, "mahoraga_adaptation_pct", 0.0)), 0.0, 70.0)
        if game is not None and getattr(game, "domain_active", False) and getattr(game, "domain_name", "") == "SANTUARIO DA ADAPTACAO":
            return max(base, clamp(float(getattr(self, "mahoraga_domain_adapt_pct", 0.0)), 0.0, 80.0))
        return base

    def start_mahoraga_adapt(self, game):
        if self.character != MAHORAGA_MODULE_KEY or game.state != "playing":
            return False
        if self.mahoraga_adapt_timer > 0 or self.mahoraga_adapt_cd > 0:
            return False
        self.mahoraga_adapt_timer = 4.0
        self.mahoraga_adapt_cd = 10.0
        self.mahoraga_adapt_was_hit = False
        self.mahoraga_adapt_hit_count = 0
        self.invuln = max(self.invuln, 4.05)
        # V15.19: a Roda do Dharma começa a soar NO CLIQUE e gira durante todo o período parado.
        # O som não é mais disparado depois que a adaptação já terminou.
        self.mahoraga_wheel_fx = max(self.mahoraga_wheel_fx, 4.05)
        AUDIO.play_external_sfx("mahoraga_player_wheel", 1.0, 0)
        game.attack_held = False
        game.dash_held = False
        game.domain_held = False
        game.damage_texts.append(DamageText("ADAPTANDO...", pygame.Vector2(self.pos), YELLOW, 0.9))
        return True

    def parry(self, game):
        if self.character == MAHORAGA_MODULE_KEY:
            self.start_mahoraga_adapt(game)
            return
    ...
    def expand_domain(self, game):
        ...
        if self.character == "Potential Man":
            ...
            return
        if self.character == MAHORAGA_MODULE_KEY:
            if self.mahoraga_adapt_timer > 0 or game.domain_active or game.domain_charge < 100:
                return
            game.domain_charge=0
            game.domain_center=pygame.Vector2(self.pos)
            game.domain_radius=S(562.5)*(1.0+0.01*permanent_effective_level("domain_range_level"))
            # V15.19: Santuário da Adaptação dura 20 segundos.
            game.domain_duration=20.0
            game.domain_timer=game.domain_duration
            game.domain_name="SANTUARIO DA ADAPTACAO"
            game.domain_active=True
            game.domain_message_timer=1.65
            # O Dominio parte da adaptacao que Mahoraga ja conquistou e cada giro soma +10%,
            # ate o teto temporario de 80%. Ao acabar, o valor-base continua intacto.
            self.mahoraga_domain_adapt_pct=float(self.mahoraga_adaptation_pct)
            self.mahoraga_domain_wheel_cd=1.0
            self.mahoraga_domain_visual_angle=0.0
            self.mahoraga_wheel_fx=0.8
            AUDIO.play("domain",1.0,180)
            game.shake=max(game.shake,S(16)); game.flash_screen=max(game.flash_screen,0.10)
            return
        ...
        base_radius = S(562.5) * (1.0 + 0.01*permanent_effective_level("domain_range_level"))
        if self.character == "Pai do Kayk":
            base_radius *= self.father_domain_radius_mult
        if self.character == RIP_INDRA_KEY:
            base_radius *= 1.45
        game.domain_radius = base_radius
        game.domain_duration = (8.0 + (2.0 if self.domain_evolution else 0.0)) * (1.0+self.passive_domain_duration) * (1.0+0.0075*permanent_effective_level("domain_duration_level"))
        if self.hybrid_resonant_domain:
            game.domain_duration += 1.5
        if self.character == "Pai do Kayk":
            game.domain_duration = (5.5 + (1.5 if self.domain_evolution else 0.0)) * (1.0+0.0075*permanent_effective_level("domain_duration_level"))
        ...
        if self.character == HISOKA_KEY:
            ...
            maho=getattr(game,"mahoraga",None)
            if getattr(game,"game_mode","normal")=="mahoraga_trial" and maho is not None and not maho.dead and game.is_in_domain(maho.pos): targets.append(maho)
            for e in targets:
                d=e.pos-self.pos
                if d.length_squared()==0: d=vec_from_angle(random.random()*math.tau)
                boss=isinstance(e,(Boss,MahoragaSummon))
        ...
        if self.character == "Pai do Kayk":
            ...
            for e in list(game.enemies):
                if not e.dead and game.is_in_domain(e.pos):
                    ...
                    e.die(game)
            maho=getattr(game,"mahoraga",None)
            if getattr(game,"game_mode","normal")=="mahoraga_trial" and maho is not None and not maho.dead and game.is_in_domain(maho.pos):
                sentence=max(self.base_damage*8.0,maho.max_hp*0.12)
                maho.take_damage(sentence,game,self.pos,from_player=True)
                game.damage_texts.append(DamageText("SENTENCA: -12% HP BASE",pygame.Vector2(maho.pos),WHITE,0.9))
            game.current_damage_kind = old
    ...
    def update(self, dt, game, move_dir):
        ...
        maho_ar=self.mahoraga_effective_adaptation_pct(game)/100.0 if self.character==MAHORAGA_MODULE_KEY else 0.0
        maho_cd_rate=1.0+0.625*maho_ar
        self.attack_cd = max(0, self.attack_cd - dt*lira_cooldown_rate*passive_attack_rate*maho_cd_rate)
        self.dash_cd = max(0, self.dash_cd - dt*lira_cooldown_rate*maho_cd_rate)
        self.passive_dash_buff_timer=max(0.0,self.passive_dash_buff_timer-dt)
        self.parry_cd = max(0, self.parry_cd - dt*lira_cooldown_rate*maho_cd_rate)
        ...
        if (self.divine_ruan or self.divine_kayk or self.divine_pedro or self.divine_kevyn) and getattr(game,"dash_held",False):
            ...
            if self.divine_dash_hold>=1.0 and not self.divine_dash_triggered:
                if self.divine_kevyn:
                    ...
                    self.divine_dash_triggered=True
        if self.character == MAHORAGA_MODULE_KEY:
            self.mahoraga_adapt_cd=max(0.0,self.mahoraga_adapt_cd-dt)
            self.mahoraga_wheel_fx=max(0.0,self.mahoraga_wheel_fx-dt)
            if self.mahoraga_adapt_timer>0:
                before=self.mahoraga_adapt_timer
                self.mahoraga_adapt_timer=max(0.0,self.mahoraga_adapt_timer-dt)
                move_dir=pygame.Vector2(0,0)
                self.invuln=max(self.invuln,0.18)
                if before>0 and self.mahoraga_adapt_timer<=0:
                    if self.mahoraga_adapt_was_hit:
                        old_pct=self.mahoraga_adaptation_pct
                        self.mahoraga_adaptation_pct=min(70.0,self.mahoraga_adaptation_pct+5.0)
                        game.damage_texts.append(DamageText(f"ADAPTACAO {int(self.mahoraga_adaptation_pct)}%",pygame.Vector2(self.pos),YELLOW,1.0))
                    else:
                        game.damage_texts.append(DamageText("NADA A ADAPTAR",pygame.Vector2(self.pos),GRAY,0.7))
                    self.mahoraga_adapt_was_hit=False; self.mahoraga_adapt_hit_count=0
        if self.character == "Sans":
            ...
            visual = pygame.Vector2(move_dir) if move_dir.length_squared() > 0.01 else pygame.Vector2(self.facing)
            if visual.length_squared() > 0.01:
                if abs(visual.x) > abs(visual.y):
                    self.sans_sprite_dir = "right" if visual.x > 0 else "left"
                else:
                    self.sans_sprite_dir = "down" if visual.y > 0 else "up"
            if self.is_moving:
                self.sans_anim_time = (self.sans_anim_time + dt * 8.0) % 4.0
                self.sans_walk_frame = int(self.sans_anim_time) % 4
            else:
                self.sans_anim_time = 0.0
                self.sans_walk_frame = 0
            if self.stamina <= 0.001:
        ...
        if SAVE.get("cheat_zero_cooldown", False):
            self.attack_cd = 0.0
            self.dash_cd = 0.0
            self.parry_cd = 0.0
            self.sans_blaster_cd = 0.0
        ...
        if self.character == HANK_KEY and game.domain_active:
            speed_mult *= 2.0
        if self.character == MAHORAGA_MODULE_KEY:
            speed_mult *= 1.0 + 0.50*(self.mahoraga_effective_adaptation_pct(game)/100.0)
        ...
        if self.character == HANK_KEY and game.domain_active:
            regen *= 2.4
            # Dominio do Hank agora tambem regenera vida enquanto ele permanece dentro.
            if self.in_domain(game) and getattr(game,"game_mode","normal") != "extinction":
                self.hp = min(self.max_hp, self.hp + self.max_hp*0.075*dt)
    ...
    def draw(self, offset, game):
        ...
        if self.character == "Sans":
            spr = sans_player_surface(getattr(self, "sans_sprite_dir", "down"), getattr(self, "sans_walk_frame", 0) if getattr(self, "is_moving", False) else 0, max(S(118), int(self.radius * 5.6)))
            if spr is not None:
                rect = spr.get_rect(midbottom=(int(p.x), int(p.y + self.radius + S(8))))
                SCREEN.blit(spr, rect)
                if self.invuln > 0:
                    pygame.draw.rect(SCREEN, WHITE, rect.inflate(S(10), S(10)), max(1, S(2)), border_radius=S(8))
            else:
                pygame.draw.circle(SCREEN, color, (int(p.x), int(p.y)), self.radius)
        else:
            pygame.draw.circle(SCREEN, color, (int(p.x), int(p.y)), self.radius)
        ...
        if self.character == HANK_KEY:
            ...
            pygame.draw.circle(SCREEN,DARK,(int(p.x+ex),int(ey)),rr,max(1,S(1)))
        if self.character == MAHORAGA_MODULE_KEY:
            wheel_c=pygame.Vector2(p.x,p.y-self.radius-S(30)); wr=S(20)
            ang=(pygame.time.get_ticks()/220.0 if self.mahoraga_wheel_fx>0 or (game.domain_active and game.domain_name=="SANTUARIO DA ADAPTACAO") else 0.0)
            pygame.draw.circle(SCREEN,YELLOW,(int(wheel_c.x),int(wheel_c.y)),wr,max(2,S(3)))
            for i in range(8):
                d=vec_from_angle(ang+i*math.tau/8); pygame.draw.line(SCREEN,YELLOW,wheel_c+d*S(5),wheel_c+d*wr,max(1,S(2)))
            draw_text(f"{int(self.mahoraga_effective_adaptation_pct(game))}%",FONT_S,WHITE,(p.x,p.y-self.radius-S(60)),True)
        if self.character != "Sans":
        ...
        if self.character == "Sans":
            if self.sans_melee_fx > 0:
                ...
                draw_sans_bone_small(bone_center, d, S(150), S(30), WHITE)
            if self.sans_exhausted:
        ...
        if game.slash_fx > 0 and (self.weapon == 0 or self.character in (HANK_KEY, MAHORAGA_MODULE_KEY)):
...
class CharacterRival(Boss):
    ...
    def _move(self,dt,game,d,dist):
        ...
        if c in ("Kevyn","Ruan","Pai do Kayk","Glonk 100% Power"):
    ...
    def _update_rival_domain(self, dt, game):
        if self.domain_active:
            ...
            elif c=="Pai do Kayk" and inside and self.domain_tick<=0:
                if game.player.character not in ("Glonk", "Glonk 100% Power"):
    ...
    def draw(self,offset):
        ...
        if self.rival_fx>0:
            ...
            elif self.rival_character=="Pai do Kayk": pygame.draw.circle(SCREEN,WHITE,(int(p.x),int(p.y)),S(270),max(2,S(6)))
...
class Game:
    def __init__(self):
        ...
        self.easy_trial_warning_character = None
        self.easy_trial_warning_confirm_rect = pygame.Rect(0,0,0,0)
        self.easy_trial_warning_cancel_rect = pygame.Rect(0,0,0,0)
        ...
        self.character_catalog_page = 0
        self.character_catalog_selected = "Ycaro"
        self.character_catalog_rects = []
        self.catalog_launch_character = None
        self.bestiary_return_state = "menu"
        ...
        self.sans_bones = []
        self.sans_blaster_plants = []
        self.sans_blaster_fx = []
        self.sans_soul_throws = []
        self.sans_soul_wall_bones = []
        self.sans_teleport_blackout = 0.0
        ...
        self.mahoraga_ai_messages = []
        self.mahoraga_final_countdown = -1.0
        self.mahoraga_final_countdown_total = 12.0
        self.mahoraga_reward_page = 0
        self.mahoraga_reward_selected = None
        self.mahoraga_reward_rects = []
        self.mahoraga_reward_confirm_rect = pygame.Rect(0,0,0,0)
        self.mahoraga_reward_prev_rect = pygame.Rect(0,0,0,0)
        self.mahoraga_reward_next_rect = pygame.Rect(0,0,0,0)
        self.sans_trial_boss=None
        self.sans_boss_bones=[]
        self.sans_boss_blasters=[]
        self.sans_trial_victory_pending=False
        self.sans_trial_error=""
        ...
        self.sukuna_fuuga_aiming = False
        self.sukuna_ult_center = pygame.Vector2(W/2, H/2)
        self.sukuna_ult_radius = S(562.5) * (1.0+0.01*permanent_effective_level("domain_range_level"))
        ...
        if SAVE.get("mahoraga_divine_choice_pending",False):
            self.state="mahoraga_reward"

    def add_mahoraga_ai_message(self,text,color=YELLOW):
        self.mahoraga_ai_messages.insert(0,[str(text),4.2,color])
        self.mahoraga_ai_messages=self.mahoraga_ai_messages[:5]

    def start_mahoraga_final_countdown(self):
        if self.game_mode!="mahoraga_trial" or self.mahoraga_final_countdown>0:
            return
        self.mahoraga_final_countdown=self.mahoraga_final_countdown_total
        self.add_mahoraga_ai_message(f"ADAPTACAO 99%: DERROTE MAHORAGA EM {int(self.mahoraga_final_countdown_total)}s OU A PARTIDA ACABA.",RED)
        self.shake=max(self.shake,S(18)); AUDIO.play("boss",1.0,200)

    def grant_mahoraga_trial_base_rewards(self):
        if SAVE.get("mahoraga_reward_claimed",False) or SAVE.get("mahoraga_divine_choice_pending",False):
            return False
        SAVE["mahoraga_module_unlocked"]=True
        SAVE["coins"]=int(SAVE.get("coins",0) or 0)+100000
        prof=evolution_profile(MAHORAGA_MODULE_KEY)
        prof["points"]=int(prof.get("points",0) or 0)+150
        prof["earned"]=int(prof.get("earned",0) or 0)+150
        SAVE["mahoraga_divine_choice_pending"]=True
        save_data(SAVE)
        return True

    def grant_sans_trial_rewards(self):
        if SAVE.get("sans_reward_claimed",False):
            return False
        SAVE["sans_unlocked"]=True
        SAVE["coins"]=int(SAVE.get("coins",0) or 0)+1
        prof=evolution_profile("Sans")
        prof["points"]=int(prof.get("points",0) or 0)+150
        prof["earned"]=int(prof.get("earned",0) or 0)+150
        SAVE["sans_reward_claimed"]=True
        save_data(SAVE)
        return True

    def mahoraga_reward_weapons(self):
        # So oferece Divinas realmente utilizaveis: previews sem implementacao nao podem consumir a recompensa unica.
        return [w for w in FORGE_WEAPONS if w.get("rarity")=="divine" and (not w.get("preview",False) or w.get("testable",False))]

    def claim_mahoraga_divine_reward(self,key):
        if not SAVE.get("mahoraga_divine_choice_pending",False): return False
        recipe=FORGE_WEAPON_BY_KEY.get(key)
        if not recipe or recipe.get("rarity")!="divine" or (recipe.get("preview",False) and not recipe.get("testable",False)): return False
        crafted=set(SAVE.get("crafted_weapons",[]))
        if key in crafted: return False
        crafted.add(key); SAVE["crafted_weapons"]=sorted(crafted)
        SAVE["mahoraga_divine_choice_pending"]=False
        SAVE["mahoraga_reward_claimed"]=True
        save_data(SAVE)
        self.mahoraga_reward_selected=None
        self.mode_complete_title="RECOMPENSA RESGATADA"
        self.mode_complete_subtitle=f"{recipe['name']} | Mahoraga Modulo + 100K + 150 Pontos"
        self.state="mode_complete"
    ...
    def mark_sukuna_ult(self):
        if self.player.character != SUKUNA_KEY or self.domain_charge < 100:
            return False
        self.sukuna_fuuga_aiming=True
        self.sukuna_ult_marked=False
        self.damage_texts.append(DamageText("FUUGA: TOQUE NA ARENA",pygame.Vector2(self.player.pos),SUKUNA_FIRE,1.0))
        AUDIO.play("click",0.8,80)
        return True

    def set_sukuna_ult_target(self,pos):
        if self.player.character!=SUKUNA_KEY or not getattr(self,"sukuna_fuuga_aiming",False):
            return False
        c=pygame.Vector2(pos)
        top=S(165); bottom=H-S(335)
        c.x=clamp(c.x,S(70),W-S(70)); c.y=clamp(c.y,top+S(55),bottom-S(55))
        self.sukuna_ult_center=c
        self.sukuna_ult_marked=True
        self.sukuna_fuuga_aiming=False
        self.damage_texts.append(DamageText("FUUGA: AREA MARCADA",pygame.Vector2(c),SUKUNA_FIRE,0.9))
        AUDIO.play("click",0.8,80)
        return True
    ...
    def start_sukuna_cutscene(self):
        if self.player.character != SUKUNA_KEY or self.sukuna_cutscene_active or not self.sukuna_ult_marked:
            return False
        if not SAVE.get("cutscenes_enabled",True):
            self.trigger_sukuna_fuuga()
            self.sukuna_ult_marked=False
            self.sukuna_fuuga_aiming=False
            self.sukuna_destruction_active=True
            self.sukuna_destruction_timer=1.45
            if AUDIO.sfx_on:
                if not AUDIO.play_external_once("sukuna_destruction",AUDIO.sfx_volume): AUDIO.play("hank_blast",1.0,100)
            return True
    ...
    def start_mahoraga_cutscene(self):
        trial = getattr(self,"game_mode","normal")=="mahoraga_trial"
        if (not trial and self.player.character != "Potential Man") or self.mahoraga_cutscene_active:
            return False
        if not SAVE.get("cutscenes_enabled",True):
            self.mahoraga=None; self.mahoraga_hostile_to_dogs=False
    ...
    def _spawn_mahoraga_for_cutscene(self):
        ...
        if getattr(self,"game_mode","normal")=="mahoraga_trial":
            maho = AdaptiveMahoragaBoss(spawn, self.player, getattr(self,"difficulty","easy"))
            maho.start_parries=self.parries
        else:
        ...
        self.mahoraga = maho
        self.mahoraga_hostile_to_dogs = getattr(self,"game_mode","normal")=="mahoraga_trial"
        return maho
    ...
    def finish_mahoraga_cutscene(self):
        ...
        if getattr(self,"game_mode","normal")=="mahoraga_trial":
            self.add_mahoraga_ai_message("Mahoraga esta observando. A primeira adaptacao vem em 10 segundos.",WHITE)
        else:
    ...
    def mode_lock_text(self,key):
        ...
        if key=="one_vs_all": return "DESBLOQUEIE O PAI DO KAYK"
    ...
    def mode_record(self,key):
        if key=="mahoraga_trial": return int(SAVE.get("mahoraga_trial_wins",0) or 0)
        if key=="sans_trial": return int(SAVE.get("sans_trial_wins",0) or 0)
        rec=SAVE.get("mode_records",{})
        return int(rec.get(key,0) or 0)

    def update_mode_record(self):
        if self.mode!="arena" or is_beta_character(self.player.character) or self.game_mode in ("mahoraga_trial","sans_trial"): return
    ...
    def character_unlocked(self, name):
        ...
        if name == MAHORAGA_MODULE_KEY:
            return SAVE.get("mahoraga_module_unlocked", False)
        if name == "Sans":
            return SAVE.get("sans_unlocked", False)
        if name == RIP_INDRA_KEY:
        ...
        if name in (SUKUNA_KEY, "Potential Man"):
            return True
        return True
    ...
    def complete_mode(self,title,subtitle):
        maho_finish=(self.game_mode=="mahoraga_trial" and title=="MAHORAGA EXORCIZADO" and self.mode=="arena" and not is_beta_character(self.player.character))
        sans_finish=(self.game_mode=="sans_trial" and title=="SANS DERROTADO" and self.mode=="arena")
        trial_eligible=getattr(self,"difficulty","easy") in ("normal","hard","monarch","impossible")
        maho_victory=maho_finish and trial_eligible
        sans_victory=sans_finish and trial_eligible
        easy_trial_finish=(maho_finish or sans_finish) and not trial_eligible
        if maho_victory:
            SAVE["mahoraga_trial_wins"]=int(SAVE.get("mahoraga_trial_wins",0) or 0)+1
            self.grant_mahoraga_trial_base_rewards()
        if sans_victory:
            SAVE["sans_trial_wins"]=int(SAVE.get("sans_trial_wins",0) or 0)+1
            first=self.grant_sans_trial_rewards()
            if first: subtitle="SANS DESBLOQUEADO | +150 PONTOS DE EVOLUCAO | +1 MOEDA GLORIOSA"
        if easy_trial_finish:
            # Nao conta como vitoria, nao desbloqueia personagem e nao banca moedas da run.
            self.run_coins=0
            self.run_rewards_banked=True
            title="DEMONSTRACAO CONCLUIDA"
            subtitle="FACIL = 0 RECOMPENSAS | VITORIA NAO REGISTRADA"
        else:
            self.bank_run_rewards()
        self.mode_complete_title=title; self.mode_complete_subtitle=subtitle
        self.state="mahoraga_reward" if (maho_victory and SAVE.get("mahoraga_divine_choice_pending",False)) else "mode_complete"
        save_data(SAVE)
        if easy_trial_finish:
            AUDIO.play("click",0.75,60)
        else:
            AUDIO.play("unlock",1.0,100)
    ...
    def spawn_wave(self):
        ...
        if self.game_mode=="mahoraga_trial":
            self.enemies=[]
            self.potential_summons=[x for x in self.potential_summons if not isinstance(x,MahoragaSummon)]
            self.mahoraga=None; self.mahoraga_hostile_to_dogs=True
            self.mahoraga_final_countdown=-1.0
            self.wave_banner=0.0
            self.start_mahoraga_cutscene()
            return
        if self.game_mode=="sans_trial":
            try:
                self.enemies=[]; self.sans_boss_bones=[]; self.sans_boss_blasters=[]; self.sans_trial_victory_pending=False
                self.sans_trial_boss=SansTrialBoss((W-S(100),S(230)),self)
                self.enemies=[self.sans_trial_boss]; self.wave_banner=0.0
                self.damage_texts.append(DamageText("ALCANCE O SANS.",pygame.Vector2(self.player.pos),CYAN,1.2))
            except Exception as exc:
                _write_sans_trial_crash("spawn_wave",exc)
                self.sans_trial_error=repr(exc)
                self.enemies=[]; self.sans_boss_bones=[]; self.sans_boss_blasters=[]; self.sans_trial_boss=None
                self.state="mode_select"
                self.unlock_notice="ERRO SANS TRIAL SALVO EM sans_trial_crash.log"; self.unlock_notice_timer=8.0
            return
    ...
    def bank_run_rewards(self):
        ...
        if self.run_rewards_banked or self.mode != "arena" or is_beta_character(self.player.character):
            return False
        # FACIL em Provacao/Julgamento e somente demonstracao: nenhuma recompensa persistente.
        if self.game_mode in ("mahoraga_trial","sans_trial") and getattr(self,"difficulty","easy")=="easy":
            return False
    ...
    def start_domain_clash(self, boss):
        maho=getattr(self,"mahoraga",None)
        if self.game_mode=="mahoraga_trial" and maho is not None and not maho.dead and getattr(maho,"domain_active",False):
            return
    ...
    def update_bad_time(self, dt):
        ...
        # Durante BAD TIME o Blaster planta, toca o audio e dispara 0.9s depois.
        if self.player.sans_blaster_cd <= 0:
            self.player.fire_sans_blaster(self,auto=True)

    def update_sans_blaster_plants(self, dt):
        if not self.sans_blaster_plants:
            return
        remaining=[]
        for plant in self.sans_blaster_plants:
            plant["timer"] -= dt
            if plant["timer"] <= 0:
                try:
                    self.player.release_sans_blaster(self, plant)
                except Exception:
                    pass
            else:
                remaining.append(plant)
        self.sans_blaster_plants = remaining

    def spawn_sans_soul_wall_bones(self, impact, wall):
        """Cria uma fileira compacta de ossos saindo da parede atingida."""
        impact=pygame.Vector2(impact)
        if wall=="left": inward=pygame.Vector2(1,0); tangent=pygame.Vector2(0,1)
        elif wall=="right": inward=pygame.Vector2(-1,0); tangent=pygame.Vector2(0,1)
        elif wall=="top": inward=pygame.Vector2(0,1); tangent=pygame.Vector2(1,0)
        else: inward=pygame.Vector2(0,-1); tangent=pygame.Vector2(1,0)
        length=S(155)
        damage=self.player.base_damage*0.90
        count=10
        spacing=S(24)
        for i in range(count):
            off=(i-(count-1)/2.0)*spacing
            center=impact + tangent*off + inward*(length*0.46)
            self.sans_soul_wall_bones.append(SansSoulWallBone(center,inward,damage,length))
        self.spawn_particles(impact,WHITE,22)
        self.shake=max(self.shake,S(10))

    def update_sans_soul_throws(self, dt):
        throws=getattr(self,"sans_soul_throws",None)
        if not isinstance(throws,list) or not throws:
            return
        remaining=[]
        arena_top=float(S(165))
        arena_bottom=float(H-S(335))
        for throw in list(throws):
            try:
                enemy=throw.get("enemy") if isinstance(throw,dict) else None
                if enemy is None or getattr(enemy,"dead",False):
                    continue
                life=float(throw.get("life",0.0))-float(dt)
                vel=pygame.Vector2(throw.get("vel",(0,0)))
                pos=pygame.Vector2(getattr(enemy,"pos",(W*0.5,H*0.5))) + vel*float(dt)
                radius=getattr(enemy,"radius",S(20))
                radius=float(radius) if isinstance(radius,(int,float)) else float(S(20))
                radius=max(1.0,radius)
                left=radius; right=float(W)-radius
                top=arena_top+radius; bottom=arena_bottom-radius
                # Protege resolucoes/areas de arena muito pequenas.
                if bottom<top:
                    top=radius; bottom=float(H)-radius
                wall=None
                if pos.x<=left:
                    pos.x=left; wall="left"
                elif pos.x>=right:
                    pos.x=right; wall="right"
                elif pos.y<=top:
                    pos.y=top; wall="top"
                elif pos.y>=bottom:
                    pos.y=bottom; wall="bottom"
                enemy.pos=pygame.Vector2(pos)

                if wall is not None:
                    previous=getattr(self,"current_damage_kind",None)
                    self.current_damage_kind="sans_soul_slam"
                    try:
                        slam=float(getattr(self.player,"base_damage",1.0))*2.6
                        try:
                            enemy.damage(slam,self,pygame.Vector2(pos),minimal_fx=False)
                        except TypeError:
                            enemy.damage(slam,self,pygame.Vector2(pos))
                    except Exception as exc:
                        print("ALMA impacto sem dano:",exc)
                    finally:
                        self.current_damage_kind=previous
                    try:
                        self.damage_texts.append(DamageText("PAREDE!",pygame.Vector2(pos),CYAN,0.7))
                        self.spawn_sans_soul_wall_bones(pygame.Vector2(pos),wall)
                    except Exception as exc:
                        print("ALMA efeito de parede ignorado:",exc)
                    continue

                if life>0:
                    throw["life"]=life
                    throw["vel"]=vel*max(0.0,1.0-0.10*float(dt))
                    remaining.append(throw)
            except Exception as exc:
                # Um inimigo especial nao pode derrubar a run inteira.
                print("ALMA arremesso ignorado:",repr(exc))
                continue
        self.sans_soul_throws=remaining
    ...
    def add_domain_charge(self, amount):
        ...
        if self.domain_active:
            return
        amount *= (1.0 / 5.0)
        # V15.19: Mahoraga jogável leva 300% MAIS tempo para obter a Expansão.
        # +300% de tempo = 4x o tempo normal, então cada acerto dá somente 25% da carga usual.
        if self.player.character == MAHORAGA_MODULE_KEY:
            amount *= 0.25
        ...
        mult = (2.0 if self.player.devil_pact else 1.0) * (1.0 + 0.02 * permanent_effective_level("domain_level"))
    ...
    def handle_powerup_hit(self, enemy, amount, source_pos=None):
        ...
        if p.character=="Pai do Kayk" and p.father_i_rule_here and source=="father_wave":
    ...
    def award_passive_critical_kill(self, enemy):
        ...
        extra=max(1,int(coin_value*self.coin_multiplier*reward_mult*(1.0+0.02*permanent_effective_level("coin_level"))*max(.10,1.0+self.player.passive_coin_bonus)*self.player.passive_crit_kill_coins))
    ...
    def on_enemy_killed(self, enemy):
        ...
        if p.character == "Vinicius 13" and self.mode == "arena":
            # Sucata agora vem de ACERTOS. Kills continuam alimentando apenas a construcao das Mini-Torres.
            self.vinicius_mini_kills += 1
    ...
    def uses_aim_stick(self):
        ...
        if self.player.character == HANK_KEY and self.domain_active:
            return False
        return self.player.character in ("Kayk", "Ycaro", "Pedro", "Vasco", LIRA_KEY, HANK_KEY, "Vinicius 13", "Sans")
    ...
    def handle_touch_down(self, pos, finger_id=None):
        if self.state == "playing" and (getattr(self, "mahoraga_cutscene_active", False) or getattr(self, "sukuna_cutscene_active", False)):
            return
        if self.state == "playing":
            if getattr(self,"sukuna_fuuga_aiming",False) and S(165)<=pos[1]<=H-S(335):
                self.set_sukuna_ult_target(pos); return
            ...
            elif self.control_rect("parry").collidepoint(pos):
                if self.player.character == "Sans":
                    if self.bad_time_active: self.player.use_sans_soul(self)
                    else: self.player.fire_sans_blaster(self,auto=False)
                elif self.player.character == MAHORAGA_MODULE_KEY:
                    self.player.start_mahoraga_adapt(self)
    ...
    def apply_dev_room_start_stats(self):
        """Aplica overrides DEV uma unica vez por valor a cada inimigo que entra na sala."""
        if not SAVE.get("cheat_dev",False):
            return
        hp_raw=SAVE.get("cheat_enemy_start_hp",-1)
        try: hp_override=int(hp_raw)
        except Exception: hp_override=-1
        sta_raw=SAVE.get("cheat_enemy_start_stamina",-1)
        try: sta_override=int(sta_raw)
        except Exception: sta_override=-1
        targets=list(getattr(self,"enemies",[]) or [])
        maho=getattr(self,"mahoraga",None)
        if maho is not None and not getattr(maho,"dead",True) and maho not in targets:
            targets.append(maho)
        for enemy in targets:
            if enemy is None or getattr(enemy,"dead",False):
                continue
            if hp_override>=0 and getattr(enemy,"_dev_hp_override_value",None)!=hp_override:
                try:
                    max_hp=float(getattr(enemy,"max_hp",hp_override))
                    enemy.hp=max(0.0,min(float(hp_override),max_hp))
                    enemy._dev_hp_override_value=hp_override
                except Exception:
                    pass
            if sta_override>=0 and hasattr(enemy,"stamina") and getattr(enemy,"_dev_sta_override_value",None)!=sta_override:
                try:
                    max_sta=float(getattr(enemy,"max_stamina",sta_override))
                    enemy.stamina=max(0.0,min(float(sta_override),max_sta))
                    enemy._dev_sta_override_value=sta_override
                    if isinstance(enemy,SansTrialBoss):
                        if enemy.stamina<=0 and not enemy.desperation_active and not enemy.exhausted:
                            enemy._start_desperation(self)
                        elif enemy.stamina<=enemy.max_stamina*0.50 and not enemy.final_judgement:
                            enemy.final_judgement=True; enemy.relocate_cd=2.5; enemy.soul_cd=1.35
                            self.damage_texts.append(DamageText("A VERDADEIRA BATALHA COMECA.",pygame.Vector2(enemy.pos),RED,1.3))
                except Exception:
                    pass
    ...
    def update_playing(self, dt):
        ...
        self.apply_dev_room_start_stats()
        ...
        self.time += dt
        self.mahoraga_ai_messages=[[m,t-dt,c] for m,t,c in self.mahoraga_ai_messages if t-dt>0]
        ...
        if self.kevyn_sword_kill_timer <= 0:
            self.kevyn_sword_kills = 0
        if self.domain_active and self.player.character==MAHORAGA_MODULE_KEY and self.domain_name=="SANTUARIO DA ADAPTACAO":
            # Mesmo movimento visual da roda gigante do domínio do boss.
            self.player.mahoraga_domain_visual_angle=(self.player.mahoraga_domain_visual_angle+dt*0.65)%math.tau
            self.player.mahoraga_domain_wheel_cd-=dt
            spins=0
            while self.player.mahoraga_domain_wheel_cd<=0 and spins<2:
                self.player.mahoraga_domain_wheel_cd+=1.0
                self.player.mahoraga_domain_adapt_pct=min(80.0,self.player.mahoraga_domain_adapt_pct+10.0)
                self.player.mahoraga_wheel_fx=0.90
                AUDIO.play_external_sfx("mahoraga_player_wheel",0.95,0)
                self.damage_texts.append(DamageText(f"RODA +10%  |  {int(self.player.mahoraga_domain_adapt_pct)}%",pygame.Vector2(self.player.pos),YELLOW,0.65))
                spins+=1
        if self.domain_active:
            ...
            if self.domain_timer <= 0:
                ...
                if self.player.character==LIRA_KEY: self.player.lira_end_domain(self)
                if self.player.character==MAHORAGA_MODULE_KEY:
                    self.player.mahoraga_domain_adapt_pct=0.0
                    self.player.mahoraga_domain_wheel_cd=1.0
                    self.player.mahoraga_domain_visual_angle=0.0
                    self.player.mahoraga_wheel_fx=0.0
                    self.damage_texts.append(DamageText(f"ADAPTACAO RETORNA A {int(self.player.mahoraga_adaptation_pct)}%",pygame.Vector2(self.player.pos),YELLOW,0.8))
        ...
        if self.domain_active and self.player.character == SUKUNA_KEY:
            ...
            while self.sukuna_domain_tick_cd <= 0.0 and ticks < 3:
                ...
                maho=getattr(self,"mahoraga",None)
                if self.game_mode=="mahoraga_trial" and maho is not None and not maho.dead:
                    dmg=5.0*(1.25 if self.player.sukuna_king_curses else 1.0)*self.player.effective_damage_mult(self,None,"sukuna_domain",maho.pos.distance_to(self.player.pos))
                    maho.take_damage(dmg,self,self.player.pos,from_player=True)
        ...
        if self.domain_active and self.player.character == "Kayk" and self.player.in_domain(self):
            ...
            if self.kayk_domain_fire_cd <= 0:
                ...
                maho=getattr(self,"mahoraga",None)
                if self.game_mode=="mahoraga_trial" and maho is not None and not maho.dead and self.is_in_domain(maho.pos): targets.append(maho)
                ...
                for target in targets[:max_targets]:
                    ...
                    damage = self.player.base_damage * dmg_mult * self.player.effective_damage_mult(self, None if isinstance(target,MahoragaSummon) else target, "soul_shot", target.pos.distance_to(self.player.pos))
                    if isinstance(target,MahoragaSummon): target.take_damage(damage,self,self.player.pos,from_player=True)
                    else: target.damage(damage, self, self.player.pos)
        ...
        self.sans_teleport_blackout = max(0.0, getattr(self,"sans_teleport_blackout",0.0)-dt)
        ...
        self.update_sans_blaster_plants(dt)
        ...
        # V15.4 SANS FIX: segundo analogico funciona como gatilho continuo.
        # Enquanto o dedo estiver segurando/arrastando a MIRA, attack() e consultado a cada frame;
        # o cooldown interno do Sans controla a cadencia e gera novos ossos automaticamente.
        if (self.attack_held and self.player.character == "Sans" and self.aim_active
                and self.aim_vector.length_squared() > 0.01):
            self.player.attack(self)
        elif self.attack_held and self.player.character not in ("Sans", "Potential Man", HANK_KEY, RIP_INDRA_KEY, HISOKA_KEY) and not self.player.has_divine_attack_hold():
            self.player.attack(self)
        ...
        if self.game_mode=="mahoraga_trial" and self.mahoraga is not None and not self.mahoraga.dead and self.mahoraga_final_countdown>0 and self.state=="playing":
            self.mahoraga_final_countdown=max(0.0,self.mahoraga_final_countdown-dt)
            if self.mahoraga_final_countdown<=0:
                self.damage_texts.append(DamageText("99% — TEMPO ESGOTADO",pygame.Vector2(self.player.pos),RED,1.2))
                self.player.hp=0
                self.end_run()
                return
        if self.mahoraga is not None and self.mahoraga.dead:
            if self.game_mode=="mahoraga_trial" and self.state=="playing":
                self.complete_mode("MAHORAGA EXORCIZADO",f"ADAPTACAO FINAL: {getattr(self.mahoraga,'adaptation_pct',0)}%")
                return
            self.mahoraga=None
            self.mahoraga_hostile_to_dogs=False
        ...
        if self.game_mode=="sans_trial":
            try:
                self.sans_boss_bones=[b for b in self.sans_boss_bones if b.update(dt,self)]
                self.sans_boss_blasters=[b for b in self.sans_boss_blasters if b.update(dt,self)]
                if self.sans_trial_victory_pending and self.state=="playing":
                    self.sans_trial_victory_pending=False
                    self.complete_mode("SANS DERROTADO","A VERDADEIRA BATALHA TERMINOU")
                    return
            except Exception as exc:
                _write_sans_trial_crash("hazard_update",exc)
                self.sans_trial_error=repr(exc)
                self.sans_boss_bones=[]; self.sans_boss_blasters=[]
                self.damage_texts.append(DamageText("ERRO DE PADRAO IGNORADO",pygame.Vector2(self.player.pos),RED,1.0))
        # ALMA da BAD TIME: o deslocamento forcado acontece DEPOIS da IA inimiga,
        # para a IA nao cancelar o arremesso no mesmo frame.
        self.update_sans_soul_throws(dt)
        self.sans_soul_wall_bones=[b for b in self.sans_soul_wall_bones if b.update(dt,self)]
        self.enemies = [e for e in self.enemies if not e.dead]
        ...
        for pr in self.projectiles:
            ...
            # Vinicius 13: UMA correcao pequena (max 7 graus) se o tiro ja estiver passando perto do alvo.
            # Depois disso a rota fica reta; nao existe tracking/teleguiado continuo.
            if pr.owner == "player" and getattr(pr,"kind","") == "vinicius_bolt" and not getattr(pr,"vinicius_correction_used",False):
                targets=[e for e in self.enemies if not e.dead and e.id not in pr.hit_ids and e.pos.distance_to(pr.pos)<=S(520)]
                if targets and pr.vel.length_squared()>0:
                    target=min(targets,key=lambda e:e.pos.distance_to(pr.pos)); desired=target.pos-pr.pos
                    if desired.length_squared()>0:
                        cur=pr.vel.normalize(); want=desired.normalize(); angle=cur.angle_to(want)
                        if abs(angle)<=28.0:
                            turn=clamp(angle,-7.0,7.0); speed=pr.vel.length(); pr.vel=cur.rotate(turn)*speed; pr.vinicius_correction_used=True
            ...
            if pr.owner == "enemy":
                ...
                if pr.pos.distance_to(self.player.pos) <= pr.radius + self.player.radius:
                    if self.player.dash_time <= 0:
                        if getattr(pr, "gula_devour", False):
                            self.gula_devour()
                        elif (getattr(pr,"kind","").startswith("mahoraga_") and self.game_mode=="mahoraga_trial" and self.mahoraga is not None and not self.mahoraga.dead):
                            self.mahoraga._domain_hit_player(self,pr.damage)
                        else:
                ...
                if maho is not None and not maho.dead and not getattr(pr, "hit_mahoraga", False):
                    ...
                    swept = source in ("sukuna_slash","hank_shell","rip_wave") and hasattr(pr,"prev_pos")
                    maho_hit = sans_bone_projectile_hits(pr,maho.pos,maho.radius) if source=="sans_bone" else pr.pos.distance_to(maho.pos) <= pr.radius + maho.radius
                    if swept and not maho_hit:
                        maho_hit = point_segment_distance(maho.pos, pr.prev_pos, pr.pos) <= pr.radius + maho.radius + S(4)
                    if maho_hit:
                        if isinstance(maho,AdaptiveMahoragaBoss) and maho.reflect_projectile(pr,self):
                            new_projectiles.append(pr)
                            continue
                ...
                for e in list(self.enemies):
                    ...
                    source = getattr(pr, "kind", "projectile")
                    swept = source in ("sukuna_slash","hank_shell","rip_wave") and hasattr(pr,"prev_pos")
                    target_hit = sans_bone_projectile_hits(pr,e.pos,e.radius) if source=="sans_bone" else pr.pos.distance_to(e.pos) <= pr.radius + e.radius
                    if swept and not target_hit:
                        target_hit = point_segment_distance(e.pos, pr.prev_pos, pr.pos) <= pr.radius + e.radius + S(4)
                    if target_hit:
                        origin = getattr(pr, "origin", self.player.pos)
                        ...
                        self.register_combo_hit()
                        if self.player.character=="Vinicius 13" and source=="vinicius_bolt":
                            before_scrap=self.vinicius_scrap
                            self.vinicius_scrap=min(20,self.vinicius_scrap+1)
                            if self.vinicius_scrap!=before_scrap and (self.vinicius_scrap%5==0 or self.vinicius_scrap==20):
                                self.damage_texts.append(DamageText(f"SUCATA {self.vinicius_scrap}/20",pygame.Vector2(self.player.pos),ORANGE,0.45))
                        ...
                        if source == "vinicius_bolt" and not getattr(pr,"vinicius_child",False):
                            ...
                            for other in options[:3]:
                                d=other.pos-e.pos
                                if d.length_squared()>0:
                                    child=Projectile(e.pos+d.normalize()*S(18),d.normalize()*S(690),pr.damage*0.72,"player",S(7),RED,1.55,0); child.kind="vinicius_bolt"; child.origin=pygame.Vector2(e.pos); child.vinicius_child=True; child.vinicius_correction_used=True; child.hit_ids.add(e.id); child.domain_charge_raw=0.0; child.domain_charge_uses_left=0; new_projectiles.append(child)
        ...
        elif self.game_mode in ("mahoraga_trial","sans_trial"):
            pass
    ...
    def update(self, dt):
        ...
        elif self.state in ("menu", "mode_select", "character_select", "character_catalog_list", "character_catalog_detail", "difficulty_select", "easy_trial_warning", "curse_choice", "mode_complete", "shop", "upgrade", "death", "glonk_death", "upgrade_catalog", "passive_catalog", "passive_roll", "passive_auto_config", "passive_dev", "bestiary", "forge", "items", "weapons", "control_editor", "run_upgrades", "whats_new", "achievements", "missions", "cheats", "vinicius_mega"):
    ...
    def draw_bg(self):
        maho=getattr(self,"mahoraga",None)
        maho_boss_domain=(self.game_mode=="mahoraga_trial" and maho is not None and not maho.dead and getattr(maho,"domain_active",False))
        maho_player_domain=(self.domain_active and self.player.character==MAHORAGA_MODULE_KEY and self.domain_name=="SANTUARIO DA ADAPTACAO")
        maho_domain=(maho_boss_domain or maho_player_domain)
        if maho_domain:
        ...
        for x in range(0, W, grid):
            col=(42,42,45) if maho_domain else ((15,42,74) if self.bad_time_active else (24,26,34))
            pygame.draw.line(SCREEN,col,(x,map_top),(x,map_bottom),1)
        for y in range(map_top, map_bottom, grid):
            col=(42,42,45) if maho_domain else ((15,42,74) if self.bad_time_active else (24,26,34))
            pygame.draw.line(SCREEN,col,(0,y),(W,y),1)

        # V8: borda branca mostrando claramente o limite da arena.
        inset = max(2, S(4))
        arena_rect = pygame.Rect(inset, map_top, W - inset * 2, map_bottom - map_top)
        pygame.draw.rect(SCREEN, (190,190,195) if maho_domain else WHITE, arena_rect, width=max(2, S(5)))
    ...
    def draw_domain_zone(self, offset):
        ...
        # V15.19: o Mahoraga jogável usa o MESMO visual-base da Roda Eterna do boss:
        # marcas concêntricas no chão + roda gigante girando ao fundo + barreira dupla.
        if self.domain_active and self.player.character==MAHORAGA_MODULE_KEY and self.domain_name=="SANTUARIO DA ADAPTACAO":
            c=self.domain_center+offset
            r=int(self.domain_radius)
            ang0=float(getattr(self.player,"mahoraga_domain_visual_angle",0.0))
            for frac in (0.20,0.34,0.48,0.64,0.82):
                pygame.draw.circle(SCREEN,(92,92,96),(int(c.x),int(c.y)),max(1,int(r*frac)),max(1,S(2)))
            for i in range(8):
                a=ang0+i*math.tau/8
                mc=c+vec_from_angle(a)*r*0.58
                pygame.draw.circle(SCREEN,(108,108,112),(int(mc.x),int(mc.y)),S(34),max(1,S(3)))
            wheel_r=max(S(120),int(r*0.70))
            pygame.draw.circle(SCREEN,(150,150,154),(int(c.x),int(c.y)),wheel_r,max(2,S(5)))
            for i in range(8):
                a=ang0+i*math.tau/8
                d=vec_from_angle(a)
                a1=c+d*(wheel_r*0.34); b1=c+d*wheel_r
                pygame.draw.line(SCREEN,(165,165,170),(int(a1.x),int(a1.y)),(int(b1.x),int(b1.y)),max(2,S(5)))
                tip=c+d*wheel_r
                pygame.draw.circle(SCREEN,(205,205,208),(int(tip.x),int(tip.y)),S(8),max(1,S(2)))
            pygame.draw.circle(SCREEN,(210,210,214),(int(c.x),int(c.y)),r,max(2,S(8)))
            pygame.draw.circle(SCREEN,(95,95,100),(int(c.x),int(c.y)),max(1,r-S(10)),max(2,S(3)))
        for e in self.enemies:
        ...
        maho=getattr(self,"mahoraga",None)
        if self.game_mode=="mahoraga_trial" and maho is not None and not maho.dead and getattr(maho,"domain_active",False):
            c=maho.domain_center+offset
            r=int(maho.domain_radius)
            # Marcas circulares no chao.
            for frac in (0.20,0.34,0.48,0.64,0.82):
                pygame.draw.circle(SCREEN,(92,92,96),(int(c.x),int(c.y)),max(1,int(r*frac)),max(1,S(2)))
            for i in range(8):
                a=getattr(maho,"domain_visual_angle",0.0)+i*math.tau/8
                mc=c+vec_from_angle(a)*r*0.58
                pygame.draw.circle(SCREEN,(108,108,112),(int(mc.x),int(mc.y)),S(34),max(1,S(3)))
            # Roda gigante ao fundo, girando lentamente (ou muito rapido na analise final).
            wheel_r=max(S(120),int(r*0.70))
            pygame.draw.circle(SCREEN,(150,150,154),(int(c.x),int(c.y)),wheel_r,max(2,S(5)))
            for i in range(8):
                a=getattr(maho,"domain_visual_angle",0.0)+i*math.tau/8
            ...
            pygame.draw.circle(SCREEN,(210,210,214),(int(c.x),int(c.y)),r,max(2,S(8)))
            pygame.draw.circle(SCREEN,(95,95,100),(int(c.x),int(c.y)),max(1,r-S(10)),max(2,S(3)))
            # Telegraph do Corte do Mundo.
            if getattr(maho,"domain_cut_pending",False):
                a=maho.domain_cut_start+offset; b=maho.domain_cut_end+offset
                pulse=1.0+0.25*math.sin(pygame.time.get_ticks()*0.025)
                pygame.draw.line(SCREEN,WHITE,(int(a.x),int(a.y)),(int(b.x),int(b.y)),max(S(5),int(S(8)*pulse)))
            # Julgamento final: onda circular chegando.
            if getattr(maho,"domain_final_wave_active",False):
                wr=int(maho.domain_final_wave_radius)
                pygame.draw.circle(SCREEN,WHITE,(int(c.x),int(c.y)),wr,max(3,S(12)))
                pygame.draw.circle(SCREEN,(145,145,150),(int(c.x),int(c.y)),max(S(18),S(92)),max(2,S(4)))
    ...
    def draw_hud(self):
        ...
        if self.game_mode=="mahoraga_trial" and self.mahoraga is not None:
            mw=min(S(850),int(W*0.42)); mx=W/2-mw/2; my=S(118); mh=S(24)
            pygame.draw.rect(SCREEN,DARK,(mx,my,mw,mh),border_radius=S(8))
            pygame.draw.rect(SCREEN,YELLOW,(mx,my,mw*clamp(self.mahoraga.hp/max(1,self.mahoraga.max_hp),0,1),mh),border_radius=S(8))
            draw_text(f"MAHORAGA — IA {getattr(self.mahoraga,'adaptation_pct',0)}% | HP {int(self.mahoraga.hp)}/{int(self.mahoraga.max_hp)}",FONT_S,WHITE,(W/2,my+S(38)),True)
            status=[]
            if getattr(self.mahoraga,"shield_timer",0)>0: status.append(f"ESCUDO {self.mahoraga.shield_timer:.1f}s")
            if getattr(self.mahoraga,"domain_active",False):
                cat=getattr(self.mahoraga,"domain_adapt_category",None) or "SEM ALVO"
                res=int(getattr(self.mahoraga,"domain_adapt_resistance",0.0)*100)
                spins=getattr(self.mahoraga,"domain_adapt_stacks",0)
                status.append(f"RODA ETERNA {self.mahoraga.domain_timer:.1f}s | {cat} {spins}/4 -{res}%")
            if self.mahoraga_final_countdown>0: status.append(f"99%: DERROTE EM {self.mahoraga_final_countdown:.1f}s")
            if status: draw_text("  |  ".join(status),FONT_S,RED if self.mahoraga_final_countdown>0 else CYAN,(W/2,my+S(68)),True)

        if self.game_mode=="sans_trial" and self.sans_trial_error:
            draw_text("SANS TRIAL: ERRO SALVO EM sans_trial_crash.log",FONT_S,RED,(W/2,S(108)),True)
        if self.game_mode=="sans_trial" and self.sans_trial_boss is not None and not self.sans_trial_boss.dead:
            sb=self.sans_trial_boss; sw=min(S(850),int(W*0.42)); sx=W/2-sw/2; syb=S(118); sh=S(24)
            pygame.draw.rect(SCREEN,DARK,(sx,syb,sw,sh),border_radius=S(8))
            pygame.draw.rect(SCREEN,CYAN,(sx,syb,sw*clamp(sb.stamina/max(1,sb.max_stamina),0,1),sh),border_radius=S(8))
            pygame.draw.rect(SCREEN,WHITE,(sx,syb,sw,sh),max(1,S(2)),border_radius=S(8))
            title="SANS — VERDADEIRA BATALHA" if sb.final_judgement else "SANS — ESTAMINA"
            draw_text(f"{title} {int(sb.stamina)}/{int(sb.max_stamina)}",FONT_S,RED if sb.final_judgement else WHITE,(W/2,syb+S(38)),True)
            if sb.time_since_attack>=6.0: draw_text("VOCE DEMOROU: SANS ESTA RECUPERANDO ESTAMINA",FONT_S,CYAN,(W/2,syb+S(66)),True)
        ...
        if p.character in ("Glonk", "Glonk 100% Power", "Vinicius 13"):
            ...
            if self.bad_time_active:
                draw_text("ALMA: SEM COOLDOWN | CUSTO 10% STA",FONT_S,CYAN,(ux,uy+S(76)))
            else:
        ...
        elif p.character==MAHORAGA_MODULE_KEY:
            if self.domain_active and self.domain_name=="SANTUARIO DA ADAPTACAO":
                ratio=clamp(self.domain_timer/max(0.01,self.domain_duration),0,1)
                pygame.draw.rect(SCREEN,YELLOW,(ux,uy,uw*ratio,uh),border_radius=S(10))
                draw_text(f"SANTUARIO DA ADAPTACAO {self.domain_timer:.1f}s | RODA {int(p.mahoraga_domain_adapt_pct)}%",FONT_S,WHITE,(ux,uy+S(42)))
            else:
                pygame.draw.rect(SCREEN,YELLOW if self.domain_charge>=100 else PURPLE,(ux,uy,uw*(self.domain_charge/100),uh),border_radius=S(10))
                ready_txt="PRONTA" if self.domain_charge>=100 else f"{int(self.domain_charge)}%"
                draw_text(f"SANTUARIO DA ADAPTACAO {ready_txt} | CARGA 4x MAIS LENTA",FONT_S,WHITE,(ux,uy+S(42)))
        ...
        if p.character=="Sans" and self.bad_time_active:
            parry_label="ALMA"; parry_val=0.0
        elif p.character==MAHORAGA_MODULE_KEY:
            parry_label="ADAPT"; parry_val=p.mahoraga_adapt_cd
        else:
        ...
        if p.divine_ana:
            ...
            if self.domain_held:
                ...
                special_text="TOQUE ULT = MIRAR FUUGA | SEGURE 3s = SANTUARIO"
                special_color=SUKUNA_FIRE
        elif p.character==MAHORAGA_MODULE_KEY:
            eff=p.mahoraga_effective_adaptation_pct(self)
            if p.mahoraga_adapt_timer>0:
                special_text=f"ADAPTANDO {p.mahoraga_adapt_timer:.1f}s | BASE {int(p.mahoraga_adaptation_pct)}%"
            elif self.domain_active and self.domain_name=="SANTUARIO DA ADAPTACAO":
                special_text=f"RODA {int(p.mahoraga_domain_adapt_pct)}% | EFETIVA {int(eff)}%"
            else:
                special_text=f"ADAPTACAO {int(p.mahoraga_adaptation_pct)}% / 70%"
        ...
        if self.game_mode=="mahoraga_trial" and self.mahoraga_ai_messages:
            my=cool_y+S(72)
            for msg,timer,color in self.mahoraga_ai_messages[:4]:
                alpha_note=""  # timers only control lifetime; keep drawing cheap on Android
                draw_wrapped_text(msg,FONT_S,color,pygame.Rect(ux,my,uw,S(60)),2,False,2)
                my+=S(58)
    ...
    def control_rect(self, key):
        """Hitbox efetiva dos controles: respeita exatamente o layout salvo/editor."""
    ...
    def draw_controls(self):
        ...
        if self.player.character == "Sans":
            if self.bad_time_active:
                soul_cost=self.player.max_stamina*0.10
                soul_color=CYAN if self.player.stamina>=soul_cost and not self.player.sans_exhausted else PANEL2
                circle_button(self.control_rect("parry"), "ALMA", soul_color)
            else:
                blast_color = CYAN if self.player.sans_blaster_cd <= 0 and not self.player.sans_exhausted else PANEL2
                circle_button(self.control_rect("parry"), "BLAST", blast_color)
        elif self.player.character == MAHORAGA_MODULE_KEY:
            rr=self.control_rect("parry"); center=pygame.Vector2(rr.center); rad=min(rr.w,rr.h)//2
            ready=self.player.mahoraga_adapt_cd<=0 and self.player.mahoraga_adapt_timer<=0
            col=YELLOW if ready or self.player.mahoraga_adapt_timer>0 else PANEL2
            pygame.draw.circle(SCREEN,col,(int(center.x),int(center.y)),rad)
            pygame.draw.circle(SCREEN,WHITE,(int(center.x),int(center.y)),rad,max(2,S(3)))
            ang=pygame.time.get_ticks()/180.0 if self.player.mahoraga_adapt_timer>0 else 0.0
            wr=int(rad*0.72); pygame.draw.circle(SCREEN,DARK,(int(center.x),int(center.y)),wr,max(1,S(3)))
            for i in range(8):
                d=vec_from_angle(ang+i*math.tau/8); pygame.draw.line(SCREEN,DARK,center+d*max(2,S(4)),center+d*wr,max(1,S(3)))
            draw_text("ADAPT",FONT_S,DARK,(int(center.x),int(center.y)),True)
        else:
            circle_button(self.control_rect("parry"), "PARRY", YELLOW)
    ...
    def draw_playing(self):
        ...
        if getattr(self, "sukuna_cutscene_active", False):
            ...
            return
        # Teleporte do Sans: por 0,5s absolutamente nada do mundo/HUD aparece.
        # Somente os controles permanecem visiveis e clicaveis.
        if getattr(self,"sans_teleport_blackout",0.0) > 0.0:
            SCREEN.fill((0,0,0))
            self.draw_controls()
            return
        self.draw_bg()
        ...
        if self.player.character == SUKUNA_KEY and getattr(self,"sukuna_fuuga_aiming",False):
            draw_text("FUUGA: TOQUE NA ARENA PARA ESCOLHER O ALVO",FONT_S,SUKUNA_FIRE,(W/2,S(142)),True)
        if self.player.character == SUKUNA_KEY and getattr(self,"sukuna_ult_marked",False):
            ...
            pygame.draw.circle(SCREEN,SUKUNA_FIRE,(int(c.x),int(c.y)),r,max(2,S(4)))
            pygame.draw.line(SCREEN,SUKUNA_FIRE,(int(c.x-S(22)),int(c.y)),(int(c.x+S(22)),int(c.y)),max(1,S(3)))
            pygame.draw.line(SCREEN,SUKUNA_FIRE,(int(c.x),int(c.y-S(22))),(int(c.x),int(c.y+S(22))),max(1,S(3)))
        ...
        for bone in self.sans_bones:
            bone.draw(offset)
        for bone in self.sans_soul_wall_bones:
            bone.draw(offset)
        if self.game_mode=="sans_trial":
            try:
                for bone in self.sans_boss_bones:
                    bone.draw(offset)
                for blast in self.sans_boss_blasters:
                    blast.draw(offset)
            except Exception as exc:
                _write_sans_trial_crash("hazard_draw",exc)
                self.sans_trial_error=repr(exc)
                self.sans_boss_bones=[]; self.sans_boss_blasters=[]
        else:
            for bone in self.sans_boss_bones:
            ...
            for blast in self.sans_boss_blasters:
                blast.draw(offset)
        for plant in self.sans_blaster_plants:
            draw_sans_blaster_plant(SCREEN, plant, offset)
        for fx in self.sans_blaster_fx:
    ...
    def draw_menu(self):
        ...
        mid_gap=S(24); mid_w=min(S(560),int(W*0.34)); mid_y=int(H*0.68)
        characters_btn=pygame.Rect(W/2-mid_gap/2-mid_w,mid_y,mid_w,S(62))
        settings_btn=pygame.Rect(W/2+mid_gap/2,mid_y,mid_w,S(62))
        pygame.draw.rect(SCREEN, DIVINE_BLUE, characters_btn, border_radius=S(16)); pygame.draw.rect(SCREEN,WHITE,characters_btn,max(1,S(2)),border_radius=S(16))
        pygame.draw.rect(SCREEN, BLUE, settings_btn, border_radius=S(16)); pygame.draw.rect(SCREEN, WHITE, settings_btn, width=max(1,S(2)), border_radius=S(16))
        draw_text("PERSONAGENS",FONT_S,DARK,characters_btn.center,True)
        draw_text("CONFIGURACOES  |  AUDIO", FONT_S, WHITE, settings_btn.center, True)
        self.menu_characters_rect=characters_btn; self.menu_settings_rect = settings_btn
        ...
        if SAVE.get("hank_unlocked", False):
            ...
            draw_text(f"PAI DO KAYK DESBLOQUEADO   |   PROGRESSO HANK: {sentinel_progress}/{len(FATHER_UNLOCK_CHARACTERS)}", FONT_S, WHITE, (W/2, H*0.85), True)
    ...
    def character_catalog_names(self):
        return list(CHARACTER_ORDER)+list(BETA_CHARACTER_ORDER)

    def draw_character_catalog_list(self):
        SCREEN.fill(BG)
        draw_text("PERSONAGENS",FONT_L,WHITE,(W/2,S(55)),True)
        draw_text("ESCOLHA UM PERSONAGEM PARA ABRIR A FICHA COMPLETA",FONT_S,CYAN,(W/2,S(100)),True)
        names=self.character_catalog_names(); per=7; pages=max(1,math.ceil(len(names)/per)); self.character_catalog_page=int(clamp(self.character_catalog_page,0,pages-1))
        subset=names[self.character_catalog_page*per:(self.character_catalog_page+1)*per]
        self.character_catalog_rects=[]; gap=S(16); top=S(145); bottom=H-CONTROL_SAFE_Y-S(90)
        card_h=max(S(410),bottom-top); card_w=int((W-CONTROL_SAFE_X*2-gap*(per-1))/per)
        for i,name in enumerate(subset):
            r=pygame.Rect(CONTROL_SAFE_X+i*(card_w+gap),top,card_w,card_h)
            cfg=CHARACTER_CONFIG[name]; unlocked=self.character_unlocked(name); beta=is_beta_character(name); edge=cfg["color"] if unlocked else GRAY
            pygame.draw.rect(SCREEN,PANEL if unlocked else (22,22,27),r,border_radius=S(24)); pygame.draw.rect(SCREEN,edge,r,max(2,S(4)),border_radius=S(24))
            # Retrato vertical simples: circulo grande na cor do personagem.
            portrait=(r.centerx,r.y+S(145)); pr=max(S(42),min(S(72),card_w//3))
            pygame.draw.circle(SCREEN,edge,portrait,pr); pygame.draw.circle(SCREEN,WHITE,portrait,pr,max(2,S(3)))
            if beta: draw_text("BETA",FONT_S,CYAN,(r.centerx,r.y+S(36)),True)
            elif not unlocked: draw_text("BLOQUEADO",FONT_S,RED,(r.centerx,r.y+S(36)),True)
            draw_wrapped_text(name.upper(),FONT_M if len(name)<13 else FONT_S,edge,pygame.Rect(r.x+S(10),r.y+S(255),r.w-S(20),S(120)),3,True,3)
            draw_text("TOQUE PARA ABRIR",FONT_S,GRAY,(r.centerx,r.bottom-S(42)),True)
            self.character_catalog_rects.append((r,name))
        ...
        draw_text(f"PAGINA {self.character_catalog_page+1}/{pages}",FONT_S,GRAY,(W-S(160),H-S(45)),True)
        self.character_catalog_back_rect=back; self.character_catalog_prev_rect=prev; self.character_catalog_next_rect=nxt

    def draw_character_catalog_detail(self):
        SCREEN.fill(BG)
        name=self.character_catalog_selected if self.character_catalog_selected in CHARACTER_CONFIG else "Ycaro"; cfg=CHARACTER_CONFIG[name]; beta=is_beta_character(name); unlocked=self.character_unlocked(name)
        color=cfg["color"]
        draw_text(name.upper(),FONT_L,color,(W/2,S(55)),True)
        draw_text(("BETA | " if beta else "")+("DESBLOQUEADO" if unlocked else "BLOQUEADO"),FONT_S,GREEN if unlocked else RED,(W/2,S(100)),True)
        left=pygame.Rect(CONTROL_SAFE_X,S(140),W*0.47-CONTROL_SAFE_X,S(650)); right=pygame.Rect(W*0.51,S(140),W*0.46-CONTROL_SAFE_X,S(650))
        for r in (left,right): pygame.draw.rect(SCREEN,PANEL,r,border_radius=S(18)); pygame.draw.rect(SCREEN,color,r,max(2,S(3)),border_radius=S(18))
        draw_text("KIT",FONT_M,WHITE,(left.x+S(20),left.y+S(18)))
        draw_text(f"ARMA: {cfg['weapon_name']}",FONT_S,color,(left.x+S(20),left.y+S(66)))
        draw_text(f"ULT/DOM: {cfg['domain']}",FONT_S,PURPLE,(left.x+S(20),left.y+S(104)))
        draw_wrapped_text(cfg['desc'],FONT_S,WHITE,pygame.Rect(left.x+S(20),left.y+S(145),left.w-S(40),S(68)),3,False,3)
        draw_text("EXPANSAO / ULT",FONT_S,PURPLE,(left.x+S(20),left.y+S(220)))
        draw_wrapped_text(cfg.get('domain_desc','Sem efeito especial.'),FONT_S,WHITE,pygame.Rect(left.x+S(20),left.y+S(254),left.w-S(40),S(90)),3,False,3)
        use,tip=CHARACTER_GUIDE.get(name,("Use o ataque principal e a habilidade especial para controlar a wave.","Teste o kit na Zona AFK para descobrir o melhor posicionamento."))
        draw_text("COMO USAR",FONT_S,CYAN,(left.x+S(20),left.y+S(352)))
        draw_wrapped_text(use,FONT_S,WHITE,pygame.Rect(left.x+S(20),left.y+S(386),left.w-S(40),S(76)),3,False,3)
        draw_text("DICA",FONT_S,YELLOW,(left.x+S(20),left.y+S(470)))
        draw_wrapped_text(tip,FONT_S,WHITE,pygame.Rect(left.x+S(20),left.y+S(504),left.w-S(40),S(68)),3,False,3)
        draw_text("COMO DESBLOQUEAR",FONT_S,ORANGE,(left.x+S(20),left.y+S(580)))
        draw_wrapped_text(character_unlock_text(name),FONT_S,WHITE,pygame.Rect(left.x+S(20),left.y+S(612),left.w-S(40),S(34)),2,False,2)
        draw_text("TAGS VISIVEIS",FONT_M,WHITE,(right.x+S(20),right.y+S(18)))
        tags="  •  ".join(sorted(CHARACTER_TAGS.get(name,set()))) or "SEM TAGS"
        draw_wrapped_text(tags,FONT_S,DIVINE_BLUE,pygame.Rect(right.x+S(20),right.y+S(66),right.w-S(40),S(80)),3,False,3)
        draw_text("STATUS BASE",FONT_M,WHITE,(right.x+S(20),right.y+S(165)))
        draw_text(f"HP {cfg['hp']}   DANO {cfg['damage']:.1f}   VEL {cfg['speed']}",FONT_S,WHITE,(right.x+S(20),right.y+S(210)))
        draw_text(f"STA {cfg['stamina']}   REGEN {cfg['regen']:.1f}/s",FONT_S,GRAY,(right.x+S(20),right.y+S(248)))
        ratings=CHARACTER_BASE_RATINGS.get(name,{})
        yy=right.y+S(310)
        for key in EVOLUTION_STAT_KEYS:
            val=int(ratings.get(key,5)); label=EVOLUTION_STAT_LABELS[key]; draw_text(label,FONT_S,WHITE,(right.x+S(20),yy)); bx=right.x+S(190); bw=right.w-S(230); pygame.draw.rect(SCREEN,DARK,(bx,yy+S(5),bw,S(18)),border_radius=S(7)); pygame.draw.rect(SCREEN,color,(bx,yy+S(5),bw*clamp(val/10,0,1),S(18)),border_radius=S(7)); draw_text(str(val),FONT_S,GRAY,(right.right-S(28),yy),True); yy+=S(48)
        # Acoes rapidas da ficha.
        ay=H-CONTROL_SAFE_Y-S(66); aw=S(280); ah=S(54); ag=S(16); total=aw*3+ag*2; ax=W/2-total/2
        play=pygame.Rect(ax,ay,aw,ah); afk=pygame.Rect(ax+aw+ag,ay,aw,ah); train=pygame.Rect(ax+(aw+ag)*2,ay,aw,ah)
        for r,label,col in ((play,"JOGAR",RED),(afk,"ZONA AFK",CYAN),(train,"TREINAR",ORANGE)):
            pygame.draw.rect(SCREEN,col if unlocked else PANEL2,r,border_radius=S(15)); draw_text(label,FONT_S,DARK if unlocked and col in (CYAN,ORANGE) else WHITE,r.center,True)
        self.character_catalog_play_rect=play; self.character_catalog_afk_rect=afk; self.character_catalog_train_rect=train
        back=pygame.Rect(CONTROL_SAFE_X,H-CONTROL_SAFE_Y-S(66),S(210),S(54)); pygame.draw.rect(SCREEN,PANEL2,back,border_radius=S(13)); draw_text("< LISTA",FONT_S,WHITE,back.center,True); self.character_catalog_detail_back_rect=back
    ...
    def draw_character_select(self):
        ...
        # Catalogo V15+: ate cinco personagens visiveis de uma vez para escalar melhor com o elenco.
        if len(order) == 1:
            offsets=(0,); factors=(0.50,)
        elif len(order) == 2:
            offsets=(0,1); factors=(0.34,0.66)
        elif len(order) == 3:
            offsets=(-1,0,1); factors=(0.18,0.50,0.82)
        elif len(order) == 4:
            offsets=(-1,0,1,2); factors=(0.10,0.37,0.63,0.90)
        else:
            offsets=(-2,-1,0,1,2); factors=(0.09,0.285,0.50,0.715,0.91)
        centers = [area.x+area.w*f for f in factors]
        idxs = [((self.character_carousel_index+off)%len(order)) for off in offsets]
        for slot, idx in enumerate(idxs):
            name = order[idx]
            cfg = CHARACTER_CONFIG[name]
            center_slot = offsets[slot] == 0
            cw = int(area.w*(0.31 if center_slot else (0.22 if len(order)<=3 else 0.17)))
            ch = int(area.h*(0.76 if center_slot else 0.50))
        ...
        draw_text(f"{self.character_carousel_index+1} / {len(order)}", FONT_S, GRAY, (area.centerx, area.bottom+S(35)), True)
        ...
        actual_damage = cfg["damage"] * (1 + 0.025 * permanent_effective_level("damage_level")) * (1 + 0.03 * prof.get("strength",0)) * max(.10,1.0+passive_preview.get("damage",0.0))
        actual_speed = cfg["speed"] * (1 + min(0.30,0.0075*permanent_effective_level("speed_level"))) * (1 + 0.015 * prof.get("speed",0)) * max(.10,1.0+passive_preview.get("move",0.0))
        actual_hp = 1 if selected in ("Sans", "Glonk 100% Power") else (cfg["hp"] + permanent_effective_level("vitality_level")*5 + prof.get("hp",0)*6) * max(.10,1.0+passive_preview.get("hp",0.0))
        hp_to_sta = prof.get("hp",0)*3 if selected == "Sans" else (prof.get("hp",0)*2 if selected == "Glonk 100% Power" else 0)
        actual_sta = cfg["stamina"] + permanent_effective_level("vitality_level")*3 + permanent_effective_level("stamina_level")*6 + prof.get("stamina",0)*5 + hp_to_sta
        actual_regen = cfg["regen"] * (1 + 0.02 * permanent_effective_level("regen_level")) * (1 + 0.015 * prof.get("stamina",0))
        parry_window = 0.16 + 0.006 * prof.get("parry",0)
        resist_pct = 100*(1-(max(0.65,1-0.01*permanent_effective_level("armor_level"))*max(0.625,1-0.015*prof.get("resistance",0))))
        ...
        for stat in EVOLUTION_STAT_KEYS:
            ...
            y = row_y
            label = "BLASTER" if selected=="Sans" and stat=="parry" else ("ADAPTAR" if selected==MAHORAGA_MODULE_KEY and stat=="parry" else EVOLUTION_STAT_LABELS[stat])
    ...
    def start_passive_spin(self, from_auto=False, ignore_protection=False):
        ...
        other_key=pp["equipped"][1-self.passive_roll_slot]
        final_key=passive_roll_key(char)
        # Exatamente a mesma passiva do outro slot nao entra no resultado final.
        # Mantem as probabilidades de raridade praticamente iguais sem apagar a passiva antiga.
        for _ in range(24):
            if final_key != other_key:
                break
            final_key=passive_roll_key(char)
        prof["points"]-=1
    ...
    def finish_passive_spin(self):
        ...
        if not ok:
            evolution_profile(self.passive_roll_character)["points"] += 1
            self.passive_roll_spinning=False; self.passive_roll_result=None
            self.passive_roll_message="DUPLICADA: PASSIVA MANTIDA E PONTO DEVOLVIDO"
            save_data(SAVE); AUDIO.play("hurt",0.4,100)
            return
    ...
    def draw_shop(self):
        ...
        draw_text(f"MOEDAS: {format_big_money(SAVE['coins'])}  |  LV.60 | PRECO +14%/LV | RETORNO DECRESCENTE", FONT_M, ORANGE, (W/2, S(112)), True)
        per_page = 6
        ...
        for i, (key, name, desc, base) in enumerate(subset):
            ...
            cost = permanent_upgrade_cost(base, lvl)
            col, row = i%cols, i//cols
            r = pygame.Rect(SAFE+col*(card_w+gap), top+row*(card_h+gap), card_w, card_h)
            pygame.draw.rect(SCREEN, PANEL, r, border_radius=S(18))
            ...
            draw_text(f"MAX LV.{PERMANENT_UPGRADE_CAP}" if capped else f"{format_big_money(cost)} $", FONT_S, WHITE, buy.center, True)
    ...
    def draw_settings(self):
        ...
        panel=pygame.Rect(W/2-S(600),S(205),S(1200),S(575))
        ...
        cut_on=bool(SAVE.get("cutscenes_enabled",True))
        cr=pygame.Rect(left,S(640),track_w,S(62))
        pygame.draw.rect(SCREEN,GREEN if cut_on else RED,cr,border_radius=S(14))
        draw_text("CUTSCENES: "+("ATIVADAS" if cut_on else "DESATIVADAS"),FONT_S,DARK if cut_on else WHITE,cr.center,True)
        self.settings_cutscene_rect=cr
        draw_text("DESATIVAR REDUZ EFEITOS/CENAS PESADAS",FONT_S,GRAY,(W/2,S(728)),True)
    ...
    def settings_down(self,pos):
        ...
        if hasattr(self,"settings_cutscene_rect") and self.settings_cutscene_rect.collidepoint(pos):
            SAVE["cutscenes_enabled"]=not bool(SAVE.get("cutscenes_enabled",True)); self.settings_dirty=True; AUDIO.play("click",0.65); return
    ...
    def draw_cheats(self):
        ...
        mode_names={"wave":"ONDA INICIAL","evo":"PONTOS DE EVOLUCAO","enemy_hp":"HP DA SALA","enemy_sta":"ESTAMINA DA SALA"}
        ...
        numeric_mode = self.cheat_money_mode or self.cheat_value_mode in ("wave","evo","enemy_hp","enemy_sta")
        ...
        elif self.cheat_value_mode == "enemy_hp":
            val=int(SAVE.get("cheat_enemy_start_hp",-1)); draw_text("HP DA SALA: "+("PADRAO" if val<0 else str(val)),FONT_S,RED,(W/2,S(270)),True)
        elif self.cheat_value_mode == "enemy_sta":
            val=int(SAVE.get("cheat_enemy_start_stamina",-1)); draw_text("ESTAMINA DA SALA: "+("PADRAO" if val<0 else str(val)),FONT_S,CYAN,(W/2,S(270)),True)
        ...
        if SAVE.get("cheat_dev",False):
            labels=[
                ...
                ("enemy_hp","HP DA SALA",RED),("enemy_sta","STA DA SALA",CYAN),
                ("reset_room","RESET SALA",ORANGE),
                ("immortal","IMORTAL: "+("ON" if SAVE.get("cheat_immortal") else "OFF"),GREEN if SAVE.get("cheat_immortal") else PANEL2),
                ("stamina","ESTAMINA INF: "+("ON" if SAVE.get("cheat_infinite_stamina") else "OFF"),GREEN if SAVE.get("cheat_infinite_stamina") else PANEL2),
                ("zero_cd","ZERO COOLDOWN: "+("ON" if SAVE.get("cheat_zero_cooldown") else "OFF"),GREEN if SAVE.get("cheat_zero_cooldown") else PANEL2),
    ...
    def cheat_max_permanent_upgrades(self):
        """Coloca todos os upgrades permanentes no limite atual (LV.60)."""
    ...
    def submit_cheat(self):
        if self.cheat_value_mode in ("wave","evo","enemy_hp","enemy_sta"):
            ...
            if self.cheat_value_mode=="wave":
                ...
                SAVE["cheat_enemy_start_hp"]=amount; self.cheat_message=f"HP DA SALA: {amount}"
            else:
                SAVE["cheat_enemy_start_stamina"]=amount; self.cheat_message=f"ESTAMINA DA SALA: {amount}"
    ...
    def draw_lira_domain_choice(self):
        ...
        draw_text(f"{len(self.lira_domain_selected)}/{3 if getattr(self.player,'lira_nothing_published',False) else 2}",FONT_M,WHITE,(W/2,H-CONTROL_SAFE_Y-S(38)),True)

    def draw_mahoraga_reward(self):
        SCREEN.fill((18,16,8))
        draw_text("RECOMPENSA UNICA — MAHORAGA",FONT_L,YELLOW,(W/2,S(55)),True)
        draw_text("RESGATAVEL UMA UNICA VEZ",FONT_M,RED,(W/2,S(105)),True)
        draw_text("DESBLOQUEADO: MAHORAGA MODULO   |   +100.000 MOEDAS   |   +150 PONTOS DE EVOLUCAO",FONT_S,WHITE,(W/2,S(150)),True)
        draw_text("ESCOLHA 1 ARMA DIVINA — A ESCOLHA E DEFINITIVA",FONT_M,DIVINE_BLUE,(W/2,S(195)),True)
        weapons=self.mahoraga_reward_weapons(); per=4; pages=max(1,math.ceil(len(weapons)/per)); self.mahoraga_reward_page%=pages
        subset=weapons[self.mahoraga_reward_page*per:(self.mahoraga_reward_page+1)*per]
        self.mahoraga_reward_rects=[]; gap=S(18); top=S(245); bottom=H-CONTROL_SAFE_Y-S(145); cw=int((W-CONTROL_SAFE_X*2-gap*(per-1))/per); ch=max(S(330),bottom-top)
        crafted=set(SAVE.get("crafted_weapons",[]))
        for i,w in enumerate(subset):
            r=pygame.Rect(CONTROL_SAFE_X+i*(cw+gap),top,cw,ch); owned=w["key"] in crafted; selected=self.mahoraga_reward_selected==w["key"]
            edge=GREEN if owned else (WHITE if selected else DIVINE_BLUE)
            pygame.draw.rect(SCREEN,PANEL,r,border_radius=S(22)); pygame.draw.rect(SCREEN,edge,r,max(2,S(6 if selected else 4)),border_radius=S(22))
            draw_text(w["character"].upper(),FONT_S,YELLOW,(r.centerx,r.y+S(30)),True)
            draw_wrapped_text(w["name"],FONT_M,DIVINE_BLUE,pygame.Rect(r.x+S(12),r.y+S(65),r.w-S(24),S(100)),3,True,3)
            draw_wrapped_text(w.get("desc",""),FONT_S,WHITE,pygame.Rect(r.x+S(12),r.y+S(180),r.w-S(24),r.h-S(245)),3,True,5)
            draw_text("JA POSSUI" if owned else ("SELECIONADA" if selected else "TOQUE PARA ESCOLHER"),FONT_S,edge,(r.centerx,r.bottom-S(35)),True)
            self.mahoraga_reward_rects.append((r,w["key"],owned))
        by=H-CONTROL_SAFE_Y-S(82)
        self.mahoraga_reward_prev_rect=pygame.Rect(CONTROL_SAFE_X,by,S(220),S(58)); self.mahoraga_reward_next_rect=pygame.Rect(W-CONTROL_SAFE_X-S(220),by,S(220),S(58)); self.mahoraga_reward_confirm_rect=pygame.Rect(W/2-S(280),by,S(560),S(58))
        for r,label in ((self.mahoraga_reward_prev_rect,"< ANTERIOR"),(self.mahoraga_reward_next_rect,"PROXIMA >")):
            pygame.draw.rect(SCREEN,PANEL2,r,border_radius=S(14)); draw_text(label,FONT_S,WHITE,r.center,True)
        ready=self.mahoraga_reward_selected is not None
        pygame.draw.rect(SCREEN,GREEN if ready else PANEL2,self.mahoraga_reward_confirm_rect,border_radius=S(14)); draw_text("CONFIRMAR ARMA DIVINA" if ready else "ESCOLHA UMA ARMA",FONT_S,DARK if ready else GRAY,self.mahoraga_reward_confirm_rect.center,True)
        draw_text(f"PAGINA {self.mahoraga_reward_page+1}/{pages}",FONT_S,GRAY,(W/2,by-S(28)),True)
    ...
    def draw(self):
        if self.state == "menu":
        ...
        elif self.state == "character_catalog_list":
            self.draw_character_catalog_list()
        elif self.state == "character_catalog_detail":
            self.draw_character_catalog_detail()
        elif self.state == "difficulty_select":
            self.draw_difficulty_select()
        elif self.state == "easy_trial_warning":
            self.draw_easy_trial_warning()
        elif self.state == "curse_choice":
            self.draw_curse_choice()
        elif self.state == "mode_complete":
            self.draw_mode_complete()
        elif self.state == "mahoraga_reward":
            self.draw_mahoraga_reward()
            ...
            self.draw_death()
        # Diagnostico permanente de desempenho: canto superior direito.
        fps = int(round(CLOCK.get_fps()))
        fps_txt = f"FPS {fps}"
        tw, th = FONT_S.size(fps_txt)
        box = pygame.Rect(W-CONTROL_SAFE_X-tw-S(20), S(10), tw+S(16), th+S(8))
        pygame.draw.rect(SCREEN, (12,12,16), box, border_radius=S(7))
        pygame.draw.rect(SCREEN, GREEN if fps>=50 else (YELLOW if fps>=30 else RED), box, max(1,S(2)), border_radius=S(7))
        draw_text(fps_txt, FONT_S, WHITE, (box.x+S(8),box.y+S(4)))

    def draw_mode_select(self):
        SCREEN.fill(BG)
        draw_text("MODOS DE JOGO",FONT_L,WHITE,(W/2,S(55)),True)
        draw_text("MODOS NORMAIS À ESQUERDA | PROVAÇÃO E JULGAMENTO À DIREITA",FONT_S,CYAN,(W/2,S(105)),True)
        self.game_mode_rects=[]
        normal_keys=[k for k in GAME_MODE_ORDER if k not in ("mahoraga_trial","sans_trial")]
        left=pygame.Rect(CONTROL_SAFE_X,S(140),int(W*0.64)-CONTROL_SAFE_X,H-S(255))
        gap=S(12); cols=3; card_w=int((left.w-gap*(cols-1))/cols); card_h=S(165)
        for i,key in enumerate(normal_keys):
            row=i//cols; col=i%cols; r=pygame.Rect(left.x+col*(card_w+gap),left.y+row*(card_h+gap),card_w,card_h)
            cfg=GAME_MODES[key]; unlocked=self.mode_unlocked(key); color=cfg["color"] if unlocked else GRAY
            ...
            draw_text(label,FONT_S,color,(r.centerx,r.y+S(26)),True)
            desc=cfg["desc"] if unlocked else self.mode_lock_text(key)
            draw_wrapped_text(desc,FONT_S,WHITE if unlocked else GRAY,pygame.Rect(r.x+S(12),r.y+S(52),r.w-S(24),S(76)),3,True,3)
            if unlocked:
                metric="KOs" if key=="arena_survival" else "WAVE"
                draw_text(f"RECORDE {metric}: {self.mode_record(key)}",FONT_S,GRAY,(r.centerx,r.bottom-S(25)),True)
            self.game_mode_rects.append((r,key,unlocked))

        # Desafios especiais: Provacao do Mahoraga e Julgamento do Sans compartilham a coluna direita.
        special_x=int(W*0.68); special_w=W-special_x-CONTROL_SAFE_X; special_top=S(140); special_bottom=H-S(255); sgap=S(14)
        special_h=int((special_bottom-special_top-sgap)/2)

        cfg=GAME_MODES["mahoraga_trial"]; unlocked=self.mode_unlocked("mahoraga_trial")
        mr=pygame.Rect(special_x,special_top,special_w,special_h)
        pygame.draw.rect(SCREEN,(27,25,14),mr,border_radius=S(22)); pygame.draw.rect(SCREEN,YELLOW,mr,max(2,S(5)),border_radius=S(22))
        draw_text("PROVAÇÃO",FONT_S,ORANGE,(mr.centerx,mr.y+S(24)),True)
        c=pygame.Vector2(mr.x+S(86),mr.y+S(103)); wr=S(45)
        pygame.draw.circle(SCREEN,YELLOW,(int(c.x),int(c.y)),wr,max(2,S(5)))
        for i in range(8):
            d=vec_from_angle(i*math.tau/8); pygame.draw.line(SCREEN,YELLOW,c+d*S(12),c+d*wr,max(2,S(4)))
        draw_text("MAHORAGA",FONT_M,YELLOW,(mr.x+S(165),mr.y+S(72)))
        draw_wrapped_text("Escudos em 80/50/20%, Parry reativo e adaptacao crescente.",FONT_S,WHITE,pygame.Rect(mr.x+S(165),mr.y+S(108),mr.w-S(185),S(60)),3,False,2)
        warn=pygame.Rect(mr.x+S(14),mr.y+S(170),mr.w-S(28),S(104)); pygame.draw.rect(SCREEN,(72,24,18),warn,border_radius=S(12)); pygame.draw.rect(SCREEN,RED,warn,max(2,S(3)),border_radius=S(12))
        draw_wrapped_text("Ao receber dano dentro do domínio, a roda de Mahoraga gira, aumentando sua resistência e seu dano. Durante a expansão, ele também dispara cortes em área, incluindo o Corte do Mundo. Quanto mais giros completar, mais forte ele fica.",FONT_S,YELLOW,warn.inflate(-S(12),-S(8)),5,True,2)
        draw_text(f"VITORIAS NORMAL+: {self.mode_record('mahoraga_trial')} | TOQUE PARA ENTRAR",FONT_S,WHITE,(mr.centerx,mr.bottom-S(24)),True)
        self.game_mode_rects.append((mr,"mahoraga_trial",unlocked))

        cfg=GAME_MODES["sans_trial"]; unlocked=self.mode_unlocked("sans_trial")
        sr=pygame.Rect(special_x,special_top+special_h+sgap,special_w,special_h)
        pygame.draw.rect(SCREEN,(12,22,30),sr,border_radius=S(22)); pygame.draw.rect(SCREEN,CYAN,sr,max(2,S(5)),border_radius=S(22))
        draw_text("JULGAMENTO",FONT_S,CYAN,(sr.centerx,sr.y+S(24)),True)
        simg=sans_player_surface("down",0,S(112))
        if simg is not None: SCREEN.blit(simg,simg.get_rect(center=(sr.x+S(82),sr.y+S(105))))
        draw_text("SANS",FONT_L,WHITE,(sr.x+S(155),sr.y+S(72)))
        draw_wrapped_text("5x Estamina. Sem regeneracao normal. Alcance-o atraves de ossos, Blasters e ataques de alma.",FONT_S,WHITE,pygame.Rect(sr.x+S(155),sr.y+S(110),sr.w-S(175),S(75)),3,False,2)
        draw_text("<50%: VERDADEIRA BATALHA",FONT_S,RED,(sr.centerx,sr.y+S(205)),True)
        draw_text("NORMAL+ — RECOMPENSA: SANS + 150 EVO + 1 MOEDA",FONT_S,YELLOW,(sr.centerx,sr.y+S(238)),True)
        draw_text(f"VITORIAS NORMAL+: {self.mode_record('sans_trial')} | TOQUE PARA ENTRAR",FONT_S,WHITE,(sr.centerx,sr.bottom-S(24)),True)
        self.game_mode_rects.append((sr,"sans_trial",unlocked))
    ...
    def draw_easy_trial_warning(self):
        # Mantem o contexto da dificuldade ao fundo e abre uma confirmacao bem explicita.
        self.draw_difficulty_select()
        shade=pygame.Surface((W,H),pygame.SRCALPHA); shade.fill((0,0,0,190)); SCREEN.blit(shade,(0,0))
        box=pygame.Rect(W/2-S(560),H/2-S(250),S(1120),S(500))
        pygame.draw.rect(SCREEN,(24,18,14),box,border_radius=S(26))
        pygame.draw.rect(SCREEN,ORANGE,box,max(2,S(5)),border_radius=S(26))
        draw_text("ATENÇÃO",FONT_L,ORANGE,(box.centerx,box.y+S(62)),True)
        msg=("Vish vai jogar no modo fácil? Achei que tava criando jogadores e não moças. "
             "Mas parece que alguém tá com medo né?.... Independente disso, o modo fácil não oferece "
             "recompensas, é só pra molhar a boca, deseja continuar?")
        draw_wrapped_text(msg,FONT_M,WHITE,pygame.Rect(box.x+S(55),box.y+S(125),box.w-S(110),S(185)),5,True,4)
        yes=pygame.Rect(box.x+S(70),box.bottom-S(115),S(420),S(75))
        no=pygame.Rect(box.right-S(490),box.bottom-S(115),S(420),S(75))
        pygame.draw.rect(SCREEN,RED,yes,border_radius=S(16)); pygame.draw.rect(SCREEN,PANEL2,no,border_radius=S(16))
        draw_text("SIM, CONTINUAR",FONT_M,WHITE,yes.center,True); draw_text("VOLTAR",FONT_M,WHITE,no.center,True)
        self.easy_trial_warning_confirm_rect=yes; self.easy_trial_warning_cancel_rect=no
    ...
    def draw_difficulty_select(self):
        ...
        if self.game_mode in ("mahoraga_trial","sans_trial"):
            draw_text("SOMENTE NORMAL OU SUPERIOR REGISTRA VITORIA E ENTREGA RECOMPENSAS",FONT_S,ORANGE,(W/2,S(157)),True)
        self.difficulty_rects = []
        gap = S(18)
        margin = CONTROL_SAFE_X
        card_w = int((W - margin*2 - gap*4) / 5)
        card_h = min(S(570), H-S(285))
        y = S(195) if self.game_mode in ("mahoraga_trial","sans_trial") else S(185)
        for i,key in enumerate(DIFFICULTY_ORDER):
            ...
            special_easy=(self.game_mode in ("mahoraga_trial","sans_trial") and key=="easy")
            reward_label="RECOMPENSAS 0%" if special_easy else f"RECOMPENSAS {int(cfg['reward']*100)}%"
            stats=[
                f"HP x{cfg['hp']:.2f}", f"FORCA x{cfg['force']:.2f}", f"DANO x{cfg['damage']:.2f}", f"VEL x{cfg['speed']:.2f}",
                f"INTELIGENCIA x{cfg['ai']:.2f}", f"QUANTIDADE x{cfg['count']:.2f}", reward_label
            ]
            yy=r.y+S(225)
            for line in stats:
            ...
            if special_easy:
                warn=pygame.Rect(r.x+S(12),r.bottom-S(125),r.w-S(24),S(64))
                pygame.draw.rect(SCREEN,(74,30,18),warn,border_radius=S(12))
                pygame.draw.rect(SCREEN,ORANGE,warn,max(1,S(2)),border_radius=S(12))
                draw_wrapped_text("SEM VITORIA REGISTRADA / SEM RECOMPENSAS",FONT_S,ORANGE,warn,2,True,2)
    ...
    def click_ui(self, pos):
        if self.state == "menu":
            if hasattr(self,"menu_characters_rect") and self.menu_characters_rect.collidepoint(pos):
                AUDIO.play("click",0.65); self.character_catalog_page=0; self.state="character_catalog_list"
            elif hasattr(self, "menu_settings_rect") and self.menu_settings_rect.collidepoint(pos):
            ...
            names=self.character_catalog_names(); pages=max(1,math.ceil(len(names)/7))
            if self.character_catalog_back_rect.collidepoint(pos): self.state="menu"
            elif self.character_catalog_prev_rect.collidepoint(pos): self.character_catalog_page=(self.character_catalog_page-1)%pages
            elif self.character_catalog_next_rect.collidepoint(pos): self.character_catalog_page=(self.character_catalog_page+1)%pages
            else:
                for r,name in self.character_catalog_rects:
                    if r.collidepoint(pos): self.character_catalog_selected=name; self.state="character_catalog_detail"; AUDIO.play("click",0.6); break
        elif self.state == "character_catalog_detail":
            name=self.character_catalog_selected
            unlocked=self.character_unlocked(name)
            if self.character_catalog_detail_back_rect.collidepoint(pos): self.state="character_catalog_list"
            elif self.character_catalog_train_rect.collidepoint(pos):
                self.bestiary_page=0; self.bestiary_return_state="character_catalog_detail"; self.state="bestiary"; AUDIO.play("click",0.6)
            elif self.character_catalog_afk_rect.collidepoint(pos):
                if unlocked: self.selected_character=name; self.start_afk(name)
                else: self.unlock_notice=character_unlock_text(name); self.unlock_notice_timer=2.5; AUDIO.play("hurt",0.4,100)
            elif self.character_catalog_play_rect.collidepoint(pos):
                if unlocked:
                    self.selected_character=name; self.catalog_launch_character=name; self.open_mode_select()
                else: self.unlock_notice=character_unlock_text(name); self.unlock_notice_timer=2.5; AUDIO.play("hurt",0.4,100)
            ...
            if self.mode_select_back_rect.collidepoint(pos): self.state="menu"; AUDIO.play("click",0.55)
                ...
                for r,key,unlocked in self.game_mode_rects:
                    if r.collidepoint(pos):
                        ...
                        catalog_char=getattr(self,"catalog_launch_character",None)
                        if key=="rogue":
                            self.catalog_launch_character=None
                        ...
                        elif catalog_char and self.character_unlocked(catalog_char):
                            self.catalog_launch_character=None; self.selected_character=catalog_char; self.open_difficulty_select(catalog_char)
                        else:
                            self.catalog_launch_character=None; self.open_character_select("arena")
                        break
            ...
            if hasattr(self,"difficulty_back_rect") and self.difficulty_back_rect.collidepoint(pos):
                ...
                for r,key in getattr(self,"difficulty_rects",[]):
                    if r.collidepoint(pos):
                        ...
                        self.difficulty=key
                        AUDIO.play("click",0.75)
                        if key=="easy" and self.game_mode in ("mahoraga_trial","sans_trial"):
                            self.easy_trial_warning_character=chosen
                            self.state="easy_trial_warning"
                        else:
                            self.reset_run(chosen,key)
                        break

        elif self.state == "easy_trial_warning":
            if self.easy_trial_warning_confirm_rect.collidepoint(pos):
                chosen=self.easy_trial_warning_character or self.pending_character or self.selected_character
                AUDIO.play("click",0.75); self.difficulty="easy"; self.reset_run(chosen,"easy")
            elif self.easy_trial_warning_cancel_rect.collidepoint(pos):
                AUDIO.play("click",0.55); self.state="difficulty_select"
            ...
            weapons=self.mahoraga_reward_weapons(); pages=max(1,math.ceil(len(weapons)/4))
            if self.mahoraga_reward_prev_rect.collidepoint(pos): self.mahoraga_reward_page=(self.mahoraga_reward_page-1)%pages; AUDIO.play("click",0.5)
            elif self.mahoraga_reward_next_rect.collidepoint(pos): self.mahoraga_reward_page=(self.mahoraga_reward_page+1)%pages; AUDIO.play("click",0.5)
            elif self.mahoraga_reward_confirm_rect.collidepoint(pos) and self.mahoraga_reward_selected:
                self.claim_mahoraga_divine_reward(self.mahoraga_reward_selected)
            else:
                for r,key,owned in self.mahoraga_reward_rects:
                    if r.collidepoint(pos) and not owned:
                        self.mahoraga_reward_selected=key; AUDIO.play("click",0.7); break
            ...
            if self.bestiary_back_rect.collidepoint(pos):
                AUDIO.play("click", 0.65); self.state = getattr(self,"bestiary_return_state","menu"); self.bestiary_return_state="menu"
            ...
            if self.cheat_back_rect.collidepoint(pos): self.state = "menu"
                ...
                if key in ("wave","evo","enemy_hp","enemy_sta"):
                    self.cheat_value_mode=key; self.cheat_money_mode=False; self.cheat_money_buffer=""; self.cheat_buffer=""
                elif key=="reset_room":
                    SAVE["cheat_enemy_start_hp"]=-1; SAVE["cheat_enemy_start_stamina"]=-1; save_data(SAVE)
                    self.cheat_message="STATUS DA SALA: PADRAO"
                elif key=="immortal": SAVE["cheat_immortal"]=not SAVE.get("cheat_immortal",False); save_data(SAVE)
                elif key=="stamina": SAVE["cheat_infinite_stamina"]=not SAVE.get("cheat_infinite_stamina",False); save_data(SAVE)
                elif key=="zero_cd": SAVE["cheat_zero_cooldown"]=not SAVE.get("cheat_zero_cooldown",False); save_data(SAVE)
...
def main():
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    if game.state == "menu":
                    ...
                    elif game.state == "easy_trial_warning":
                        ...
                        game.state = "menu"
                    elif game.state == "mahoraga_reward":
                        pass
                    elif game.state == "character_select":
                        game.state = "mode_select" if game.selection_target=="arena" else "menu"
                    elif game.state == "character_catalog_detail":
                        game.state = "character_catalog_list"
                    elif game.state == "character_catalog_list":
                        game.state = "menu"
                ...
                if game.state == "playing" and not getattr(game, "mahoraga_cutscene_active", False) and not getattr(game, "sukuna_cutscene_active", False):
                    if event.key == pygame.K_SPACE:
                        ...
                        if game.player.character == "Sans":
                            if game.bad_time_active: game.player.use_sans_soul(game)
                            else: game.player.fire_sans_blaster(game, auto=False)
                        elif game.player.character == MAHORAGA_MODULE_KEY:
                            game.player.start_mahoraga_adapt(game)
        ...
        try:
            ...
            if getattr(game,"game_mode","")=="sans_trial":
                _write_sans_trial_crash("main_loop",exc)
                game.sans_trial_error=repr(exc)
                game.sans_boss_bones=[]; game.sans_boss_blasters=[]; game.enemies=[]; game.sans_trial_boss=None
                game.game_mode="normal"; game.pending_game_mode="normal"; game.state="mode_select"
                game.unlock_notice="SANS TRIAL ERROU: veja sans_trial_crash.log"; game.unlock_notice_timer=10.0
                try:
                    game.draw()
                except Exception:
                    SCREEN.fill(BG)
                    draw_text("ERRO SANS TRIAL SALVO NO LOG",FONT_M,RED,(W/2,H/2),True)
            else:
                raise
        present_frame()
