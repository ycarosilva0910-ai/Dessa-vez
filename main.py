...
class AudioManager:
    ...
    def sync_music(self, game):
        ...
        if getattr(game, "mahoraga_cutscene_active", False):
            try:
...
# ============================================================
CHARACTER_ORDER = ["Ana", "Kevyn", "Ycaro", "Kayk", "Pedro", "Ruan", "Vasco", LIRA_KEY, HANK_KEY, "Pai do Kayk", "Strikada Egoísta", "Glonk 100% Power", "Vinicius 13", "Glonk"]
BETA_CHARACTER_ORDER = ["Sans", "Potential Man"]
...
FORGE_WEAPONS = [
    ...
    # PREVIAS DIVINAS - Sans/Potential continuam Beta; Strikada/Glonk 100% ja sao personagens normais,
    # mas suas Divinas ainda ficam como previa ate receberem integracao propria.
...
WHATS_NEW_BY_VERSION = {"V13": WHATS_NEW_V13, "V14": WHATS_NEW_V14}
...
class RuanServant:
    ...
    def __init__(self, pos, source_enemy, player, guardian=False):
        ...
        self.hp = self.max_hp
        self.damage = max(9.0, player.base_damage * (2.20 if guardian else 0.58)) * player.ruan_servant_damage_mult * player.passive_ally_damage_mult()
        if guardian:
...
class ViniciusTurret:
    ...
    def __init__(self,pos,kind,player,mega=False):
        ...
        self.radius=S(72 if mega else 34); self.max_hp=(760 if mega else 125)+player.max_hp*(0.55 if mega else 0.16); self.hp=self.max_hp; self.damage=max(5.0,player.base_damage*(1.45 if mega else 0.72))*player.passive_ally_damage_mult(); self.cd=random.uniform(0.15,0.8)
    def take_damage(self,amount,game):
        if self.dead:return
        amount*=max(0.0,1.0-getattr(self.owner_player,"passive_ally_damage_reduction",lambda:0.0)())
        self.hp-=amount
...
class Enemy:
    ...
    def update(self, dt, game):
        ...
        lira_power=self.lira_outgoing_mult(game)
        self.shoot_cd -= dt
        self.touch_cd -= dt
...
class DivineDog:
    ...
    def __init__(self, pos, player, side=1):
        ...
        self.max_hp = 105.0
        self.hp = self.max_hp
        self.damage = max(8.0, player.base_damage * 0.62) * player.passive_ally_damage_mult()
        self.speed = S(205)
        self.attack_cd = 0.0
        self.dead = False
        self.guardian = False
...
class Player:
    ...
    def vasco_roll_attack(self, game):
        ...
        if result == "win":
            self.vasco_bet_level = min(100.0, self.vasco_bet_level + 10.0)
            msg = f"GANHOU +{int(self.vasco_bet_level-old)}%"
            color = VASCO_NEON
        elif result == "lose":
    ...
    def attack(self, game):
        ...
        # Potential Man beta: toque no ATK mira automaticamente no inimigo mais proximo e usa Sapos.
        ...
        if self.character == LIRA_KEY:
            ...
            pr=Projectile(self.pos+d*S(38),d*S(900 if intellect_buff else 720),self.base_damage*0.65*self.projectile_bonus,"player",S(12),LIRA_COLOR,3.6 if intellect_buff else 2.2,self.projectile_pierce)
            pr.kind="lira_censor"; pr.origin=pygame.Vector2(self.pos); game.projectiles.append(pr)
        ...
        if self.character == HANK_KEY:
            ...
            pr = Projectile(self.pos+d*(self.radius+shell_r+S(8)), d*S(720), self.base_damage*1.20*self.projectile_bonus, "player", shell_r, (210,210,210), 3.2, 0)
            pr.kind="hank_shell"; pr.origin=pygame.Vector2(self.pos); pr.hank_explosion_radius=S(155); pr.hank_explosion_damage=self.base_damage*1.05*self.projectile_bonus
            game.projectiles.append(pr)
        ...
        if self.character == "Glonk 100% Power":
            ...
            rng = S(310)
            hit_any = False
            cos_limit=math.cos(math.radians(72))
            for e in list(game.enemies):
                if e.dead:
                    continue
                to_e = e.pos-self.pos
            ...
            if game.domain_active and self.hank_cannon_planted and game.hank_cannon_pos is not None and self.hank_artillery_cd<=0:
                ...
                if targets:
                    ...
                    if d.length_squared()>0:
                        ...
                        pr=Projectile(origin+d*(shell_r+S(28)),d*S(690),self.base_damage*1.15*self.projectile_bonus,"player",shell_r,(210,210,210),3.2,0)
                        pr.kind="hank_shell"; pr.origin=pygame.Vector2(origin); pr.hank_domain_cannon=True; pr.hank_explosion_radius=S(155); pr.hank_explosion_damage=self.base_damage*1.00*self.projectile_bonus
                        game.projectiles.append(pr); AUDIO.play("hank_cannon",0.90,100); game.shake=max(game.shake,S(8))
                    self.hank_artillery_cd=2.0
    ...
    def _update_rival_domain(self, dt, game):
        if self.domain_active:
            ...
            if c=="Ana" and self.domain_tick<=0:
                ...
                game.player.take_damage(max(10.0,game.player.max_hp*0.07),game); self.domain_tick=0.82
...
class Game:
    ...
    def draw_mahoraga_cutscene(self):
        ...
        if t >= 8.0:
            ...
            # Pequena onomatopeia do uivo perto do primeiro par, nao como mensagem central.
            if 8.0 <= t < 9.2:
                draw_text("AUUU...",FONT_S,GRAY,(W/2,H*0.78),True)
        ...
        if self.mahoraga_cutscene_revealed and self.mahoraga is not None:
            ...
            # Roda do Dharma dedicada da cutscene; parada ate 15s e gira no CLANK.
            wc=pygame.Vector2(mp.x,mp.y-self.mahoraga.radius-S(60)); wr=S(62)
            turn=0.0
            if t >= self.mahoraga_cutscene_clank_t:
                turn=min(1.0,(t-self.mahoraga_cutscene_clank_t)/0.42)*(math.pi/4)
            ...
            for i in range(8):
                d=vec_from_angle(i*math.tau/8+turn)
                a=wc+d*S(12); b=wc+d*wr
                pygame.draw.line(SCREEN,YELLOW,(int(a.x),int(a.y)),(int(b.x),int(b.y)),max(2,S(4)))
            if t >= self.mahoraga_cutscene_clank_t:
    ...
    def trigger_hank_explosion(self, pos, damage, radius, exclude=None, artillery=False):
    ...
    def add_domain_charge(self, amount):
        # V14: depois de ativar, a barra fica TRAVADA em 0 ate o Dominio terminar.
        # Acertos, kills, parries e outras fontes normais nao podem pre-carregar o proximo Dominio.
        if self.domain_active:
            return
    ...
    def handle_touch_down(self, pos, finger_id=None):
        if self.state == "playing" and getattr(self, "mahoraga_cutscene_active", False):
            return
        if self.state == "playing":
    ...
    def update_playing(self, dt):
        ...
        if self.attack_held and self.player.character not in ("Sans", "Potential Man", HANK_KEY) and not self.player.has_divine_attack_hold():
            self.player.attack(self)
        ...
        for pr in self.projectiles:
            ...
            if not projectile_arena_wall_collision(pr):
                if getattr(pr,"kind","")=="hank_shell":
                    self.trigger_hank_explosion(pr.pos,getattr(pr,"hank_explosion_damage",pr.damage*.85),getattr(pr,"hank_explosion_radius",S(155)))
                continue
                ...
                for e in list(self.enemies):
                    ...
                    if pr.pos.distance_to(e.pos) <= pr.radius + e.radius:
                        ...
                        if source=="lira_censor" and not e.dead and not isinstance(e,Boss):
                            e.apply_lira_censor(random.choice(["strength","speed","resistance","intelligence"]),0.20,self)
    ...
    def draw_controls(self):
        ...
        dom_label = "BAD" if self.player.character == "Sans" else ("MAHO" if self.player.character == "Potential Man" else ("AUTO" if self.player.character=="Vinicius 13" else ("ART" if self.player.character==HANK_KEY else "DOM")))
    ...
    def draw_hank_aim_preview(self, offset):
        """Pre-visualizacao do corredor do tiro pesado do Hank.

        Enquanto o joystick de mira esta pressionado, mostra um retangulo branco
        translucido da boca do canhao ate a parede da arena. O disparo so ocorre
        no release; dentro do Dominio Hank usa facas, entao o corredor nao aparece.
        """
        ...
        end_world = ray_to_arena_edge(origin_world, d)
        origin = origin_world + offset
        end = end_world + offset
        normal = pygame.Vector2(-d.y, d.x)
        # Corredor um pouco maior que a bala gigante (~80% do raio do Hank).
        half_w = max(S(22), int(self.player.radius*0.95))
        pts = [origin+normal*half_w, end+normal*half_w, end-normal*half_w, origin-normal*half_w]
        overlay = pygame.Surface((W,H), pygame.SRCALPHA)
        ipts=[(int(q.x),int(q.y)) for q in pts]
        pygame.draw.polygon(overlay,(255,255,255,42),ipts)
        pygame.draw.polygon(overlay,(255,255,255,105),ipts,max(1,S(2)))
        # Linha central ajuda a enxergar o eixo exato sem deixar a tela poluida.
        pygame.draw.line(overlay,(255,255,255,80),(int(origin.x),int(origin.y)),(int(end.x),int(end.y)),max(1,S(2)))
    ...
    def draw_character_select(self):
        ...
        for slot, idx in enumerate(idxs):
            ...
            card_name = "LIRA SOLIS" if name == LIRA_KEY else ("HANK" if name == HANK_KEY else name.upper())
            long_name = False
        ...
        selected_title = ("VASCO, O APOSTADOR INCANSAVEL" if selected == "Vasco" else
                          ("LIRA SOLIS, A C#NSUR#" if selected == LIRA_KEY else
                           ("HANK J. WIMBLETON" if selected == HANK_KEY else selected.upper())))
    ...
    def draw_whats_new(self):
        SCREEN.fill(BG)
        version = self.whats_new_version if self.whats_new_version in WHATS_NEW_BY_VERSION else "V14"
        ...
        self.whats_new_v13_rect=v13; self.whats_new_v14_rect=v14
        ...
        for i,line in enumerate(entries):
            ...
            if r.bottom>=viewport.top and r.top<=viewport.bottom:
                edge=DIVINE_BLUE if version=="V14" else CYAN
        ...
        if max_scroll>0:
            ...
            pygame.draw.rect(SCREEN,DIVINE_BLUE if version=="V14" else CYAN,(track.x,knob_y,track.w,knob_h),border_radius=S(3))
    ...
    def choose_lira_domain_stat(self,key):
        ...
        if len(self.lira_domain_selected)>=2: return
        self.lira_domain_selected.append(key); AUDIO.play("click",0.55)
        if len(self.lira_domain_selected)>=2: self.finalize_lira_domain_choice()

    def finalize_lira_domain_choice(self):
        if self.player.character!=LIRA_KEY or not self.domain_active or len(self.lira_domain_selected)!=2: return
    ...
    def draw_lira_domain_choice(self):
        ...
        draw_text("ESCOLHA 2 CENSURAS",FONT_M,LIRA_ACCENT,(W/2,S(158)),True)
    ...
    def click_ui(self, pos):
        if self.state == "menu":
            if hasattr(self, "menu_settings_rect") and self.menu_settings_rect.collidepoint(pos):
                ...
                AUDIO.play("click", 0.65); self.whats_new_version="V14"; self.whats_new_scroll=0.0; self.state = "whats_new"
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if game.state == "playing" and not getattr(game, "mahoraga_cutscene_active", False):
