...
# ============================================================
# DERROTE O GORDO DO PAI DO KAYK - V11 LAB + MISSOES
...
ACHIEVEMENTS = [
    ("first_blood", "PRIMEIRO SANGUE", "Derrote seu primeiro inimigo."),
    ("domain_user", "ABRA AS PORTAS", "Ative uma Expansao de Dominio."),
    ("parry_master", "MESTRE DO PARRY", "Faca 5 Parries na mesma run."),
    ("untouchable", "INTOCAVEL", "Faca 5 Perfect Dodges na mesma run."),
    ("clash_winner", "DOMINIO SOBERANO", "Venca um Choque de Dominios."),
    ("pineapple_refund", "ABACAXI ECONOMICO", "Recupere HP/STA com um abacaxi que voltou sem acertar ninguem."),
    ("friend_squad", "PODER DA AMIZADE", "Tenha 5 amigos do Ruan vivos ao mesmo tempo."),
    ("father_unlock", "PAI PRESENTE", "Desbloqueie o Pai do Kayk."),
]

CHARACTER_MISSIONS = {
    "Ana": [
        ("ana_kills", "REALIDADE HOSTIL", "Derrote 30 inimigos com Ana", "kills", 30, 80),
        ("ana_domains", "SEM SAIDA", "Ative 3 Dominios com Ana", "domains", 3, 100),
        ("ana_wave", "ANOMALIA ESTAVEL", "Alcance a onda 6 com Ana", "best_wave", 6, 140),
    ],
    "Kevyn": [
        ("kevyn_kills", "LAMINA PACIENTE", "Derrote 35 inimigos com Kevyn", "kills", 35, 85),
        ("kevyn_domains", "FORA DO TEMPO", "Ative 3 Dominios com Kevyn", "domains", 3, 105),
        ("kevyn_wave", "MURALHA VIVA", "Alcance a onda 7 com Kevyn", "best_wave", 7, 150),
    ],
    "Ycaro": [
        ("ycaro_kills", "CACA ABERTA", "Derrote 40 inimigos com Ycaro", "kills", 40, 90),
        ("ycaro_domains", "CAMPO DO ZUMBI", "Ative 3 Dominios com Ycaro", "domains", 3, 110),
        ("ycaro_wave", "NAO PASSA", "Alcance a onda 7 com Ycaro", "best_wave", 7, 155),
    ],
    "Kayk": [
        ("kayk_kills", "DOIS GATILHOS", "Derrote 45 inimigos com Kayk", "kills", 45, 100),
        ("kayk_domains", "NECROPOLE", "Ative 4 Dominios com Kayk", "domains", 4, 125),
        ("kayk_wave", "MIL ALMAS", "Alcance a onda 8 com Kayk", "best_wave", 8, 175),
    ],
    "Pedro": [
        ("pedro_kills", "COLHEITA", "Derrote 35 inimigos com Pedro", "kills", 35, 90),
        ("pedro_domains", "X DA COLHEITA", "Ative 3 Dominios com Pedro", "domains", 3, 110),
        ("pedro_wave", "ABACAXI SUPREMO", "Alcance a onda 7 com Pedro", "best_wave", 7, 160),
    ],
    "Ruan": [
        ("ruan_kills", "RECRUTADOR", "Derrote 35 inimigos com Ruan", "kills", 35, 95),
        ("ruan_domains", "REINO DOS AMIGOS", "Ative 3 Dominios com Ruan", "domains", 3, 115),
        ("ruan_wave", "AMIZADE ETERNA", "Alcance a onda 7 com Ruan", "best_wave", 7, 165),
    ],
    "Pai do Kayk": [
        ("father_kills", "AUTORIDADE", "Derrote 50 inimigos com o Pai do Kayk", "kills", 50, 120),
        ("father_domains", "SENTENCA", "Ative 3 Dominios com o Pai do Kayk", "domains", 3, 135),
        ("father_wave", "CHEFE DA CASA", "Alcance a onda 9 com o Pai do Kayk", "best_wave", 9, 200),
    ],
    "Glonk": [
        ("glonk_wave", "GLONK GAMING", "Tenha coragem de iniciar uma run com Glonk", "best_wave", 1, 80),
    ],
}

WHATS_NEW = [
    "Dominios agora ficam FIXOS no ponto de ativacao e sao 50% maiores.",
    "Ana: cooldown normal do cajado ajustado para 5 segundos.",
    "Kevyn: regeneracao passiva de 2 HP por segundo.",
    "Pequenas orbes de vida surgem aleatoriamente pela arena.",
    "Zona AFK ganhou spawn de inimigos e bosses para testar Choque de Dominios.",
    "Buffs agora precisam ser selecionados e CONFIRMADOS.",
    "Ruan: amigos perdem vida com o tempo e protegem o jogador.",
    "Pedro: abacaxis causam dano na volta; errar tudo devolve 2 HP e 2 STA.",
    "Kayk no Dominio: +1 bala por rajada e tiros mais rapidos.",
    "Novas abas: Conquistas, Missoes, Novidades e Cheats.",
    "Loja Permanente ampliada com varios upgrades novos.",
]
...
class Drop:
    COLORS = {"heal": GREEN, "ambient_heal": GREEN, "stamina": CYAN, "shield": BLUE, "haste": YELLOW, "coin": ORANGE}

    def __init__(self, pos, kind):
        self.pos = pygame.Vector2(pos)
        self.kind = kind
        self.radius = S(12 if kind == "ambient_heal" else (20 if kind in ("heal", "stamina") else 18))
        self.t = 0
    ...
    def draw(self, offset):
        ...
        if self.kind in ("heal", "ambient_heal", "stamina"):
        ...
        label = {"heal": "+", "ambient_heal": "+", "stamina": "E", "shield": "S", "haste": ">", "coin": "$"}[self.kind]
...
class Enemy:
    ...
    def die(self, game):
        ...
        reward = int(self.coin_value * game.coin_multiplier * (1.0 + 0.05 * SAVE.get("coin_level", 0)))
            ...
            if self.domain_timer <= 0:
                ...
                if self.variant == "father" and inside and self.domain_tick <= 0:
                    ...
                    for i in range(4):
                        ...
                        game.projectiles.append(Projectile(self.domain_center, d*S(355), 6+self.wave*0.35, "enemy", S(9), CYAN, 2.5))
...
class Player:
    def __init__(self, game, character="Ycaro"):
        ...
        self.hp = self.max_hp
        self.max_stamina = cfg["stamina"] + SAVE["vitality_level"] * 6 + SAVE.get("stamina_level", 0) * 10
        self.stamina = self.max_stamina
        self.stamina_regen = cfg["regen"] * (1 + 0.06 * SAVE.get("regen_level", 0))
    ...
    def attack(self, game):
        ...
        if self.character == "Ana":
            ...
            # V11: por pedido dos jogadores, cooldown normal da Ana = 5s; no Dominio continua 1s.
            self.attack_cd = 1.0 if in_dom else 5.0
    ...
    def expand_domain(self, game):
        ...
        game.unlock_achievement("domain_user")
        game.add_mission_progress("domains", 1)
...
class Game:
    ...
    def get_enemy_target(self, pos):
        """Escolhe quem os inimigos vao perseguir. Amigos/guardiao podem proteger o jogador."""
    ...
    def update_domain_clash(self, dt):
        ...
        if boss.dead:
            ...
            self.unlock_achievement("clash_winner")
            return
        if not self.domain_active or not boss.domain_active:
            self.domain_clash_active = False
            return
    ...
    def mission_stats_for(self, character):
        ...
        stats = all_stats.setdefault(character, {"kills": 0, "domains": 0, "best_wave": 0})
        for key in ("kills", "domains", "best_wave"):
    ...
    def spawn_ruan_servant(self, enemy):
        ...
        if sum(1 for x in self.servants if not x.dead and not x.guardian) >= 5:
            self.unlock_achievement("friend_squad")
    ...
    def update_playing(self, dt):
        ...
        for pr in self.projectiles:
            ...
            if not pr.update(proj_dt):
                if isinstance(pr, BoomerangProjectile) and pr.returned_to_player and not pr.hit_any:
                    self.player.heal(2, self)
                    self.player.recover_stamina(2, self)
                    self.damage_texts.append(DamageText("+2 HP / +2 STA", pygame.Vector2(self.player.pos), GREEN, 0.8))
                    self.unlock_achievement("pineapple_refund")
                continue
        ...
        for d in self.drops:
            ...
            if dist <= d.radius + self.player.radius + S(8):
                ...
                if self.player.orb_magnet and d.kind in ("heal","ambient_heal","stamina"):
        ...
        if self.parries >= 5:
            self.unlock_achievement("parry_master")
        if self.perfect_dodges >= 5:
    ...
    def update(self, dt):
        ...
        elif self.state in ("menu", "character_select", "shop", "upgrade", "death", "glonk_death", "upgrade_catalog", "bestiary", "control_editor", "run_upgrades", "whats_new", "achievements", "missions", "cheats"):
            self.particles = [p for p in self.particles if p.update(dt)]
            self.damage_texts = [d for d in self.damage_texts if d.update(dt)]
    ...
    def draw_menu(self):
        ...
        news = pygame.Rect(SAFE, H-S(96), tab_w, tab_h)
        ach = pygame.Rect(SAFE, H-S(150), tab_w, tab_h)
        missions = pygame.Rect(W-SAFE-tab_w, H-S(150), tab_w, tab_h)
        cheats = pygame.Rect(W-SAFE-tab_w, H-S(96), tab_w, tab_h)
        ...
        draw_text("WASD | ESPACO ataque | SHIFT dash | E parry | Q dominio | P pause", FONT_S, GRAY, (W/2, H*0.94), True)
    ...
    def draw_character_select(self):
        ...
        names = ["Ana", "Kevyn", "Ycaro", "Kayk", "Pedro", "Ruan", "Pai do Kayk", "Glonk"]
        cols, rows = 4, 2
        ...
        for i, name in enumerate(names):
            ...
            cheat_all = SAVE.get("cheat_all_chars", False)
            if cheat_all:
                unlocked = True
            elif name == "Kayk":
                unlocked = SAVE.get("kayk_unlocked", False)
            elif name == "Pai do Kayk":
            ...
            if unlocked:
                ...
                if name == "Kayk":
                    lock = "DERROTE O PAI DO KAYK"
                else:
                    lock = f"VITORIAS CONTRA O PAI: {father_progress}/{len(FATHER_UNLOCK_CHARACTERS)}"
    ...
    def draw_shop(self):
        ...
        for i, (key, name, desc, base) in enumerate(subset):
            ...
            pygame.draw.rect(SCREEN, ORANGE if SAVE['coins'] >= cost else PANEL2, buy, border_radius=S(16))
            draw_text(f"{cost} $", FONT_S, WHITE, buy.center, True)
            self.shop_rects.append((buy, key, cost))
        back = pygame.Rect(SAFE, H-S(72), S(210), S(50))
        prev = pygame.Rect(W/2-S(260), H-S(72), S(210), S(50))
        nxt = pygame.Rect(W/2+S(50), H-S(72), S(210), S(50))
        for r,label in [(back,"VOLTAR"),(prev,"< ANTERIOR"),(nxt,"PROXIMA >")]:
            pygame.draw.rect(SCREEN, PANEL2, r, border_radius=S(14)); draw_text(label,FONT_S,WHITE,r.center,True)
    ...
    def draw_upgrade(self):
        ...
        confirm = pygame.Rect(W/2-S(260), H-S(135), S(520), S(82))
    ...
    def draw_whats_new(self):
        SCREEN.fill(BG)
        draw_text("NOVIDADES - V11", FONT_L, WHITE, (W/2, S(58)), True)
        draw_text("O QUE MUDOU NESTA VERSAO", FONT_S, CYAN, (W/2, S(105)), True)
        cols, gap = 2, S(18)
        top, card_h = S(145), S(128)
        card_w = int((W-SAFE*2-gap)/2)
        for i, line in enumerate(WHATS_NEW):
            col, row = i%cols, i//cols
            r = pygame.Rect(SAFE+col*(card_w+gap), top+row*(card_h+S(10)), card_w, card_h)
            pygame.draw.rect(SCREEN, PANEL, r, border_radius=S(14))
            pygame.draw.rect(SCREEN, CYAN, r, width=max(1,S(2)), border_radius=S(14))
            draw_text("+", FONT_M, YELLOW, (r.x+S(18), r.y+S(18)))
            draw_wrapped_text(line, FONT_S, WHITE, pygame.Rect(r.x+S(55), r.y+S(18), r.w-S(75), r.h-S(24)), 2, False, 3)
        back = pygame.Rect(SAFE, H-S(68), S(210), S(46))
    ...
    def draw_achievements(self):
        SCREEN.fill(BG)
        unlocked = set(SAVE.get("unlocked_achievements", []))
        draw_text("CONQUISTAS", FONT_L, WHITE, (W/2, S(58)), True)
        draw_text(f"{len(unlocked)}/{len(ACHIEVEMENTS)} DESBLOQUEADAS", FONT_S, YELLOW, (W/2, S(108)), True)
        cols, gap = 2, S(18)
        top, card_h = S(150), S(165)
        card_w = int((W-SAFE*2-gap)/2)
        for i,(key,name,desc) in enumerate(ACHIEVEMENTS):
            col,row=i%cols,i//cols
            r=pygame.Rect(SAFE+col*(card_w+gap), top+row*(card_h+gap), card_w, card_h)
            ok=key in unlocked
            pygame.draw.rect(SCREEN,PANEL,r,border_radius=S(16)); pygame.draw.rect(SCREEN,GREEN if ok else GRAY,r,width=max(2,S(3)),border_radius=S(16))
            draw_text(("[OK] " if ok else "[??] ")+name,FONT_M,GREEN if ok else GRAY,(r.x+S(16),r.y+S(16)))
            draw_wrapped_text(desc,FONT_S,WHITE if ok else GRAY,pygame.Rect(r.x+S(16),r.y+S(68),r.w-S(32),S(75)),2,False,2)
        back=pygame.Rect(SAFE,H-S(70),S(210),S(48)); pygame.draw.rect(SCREEN,PANEL2,back,border_radius=S(13)); draw_text("VOLTAR",FONT_S,WHITE,back.center,True)
        self.achievements_back_rect=back
    ...
    def draw_missions(self):
        ...
        for mission_id,name,desc,stat_key,goal,reward in missions:
            ...
            y += S(215)
        back=pygame.Rect(SAFE,H-S(72),S(210),S(50)); prev=pygame.Rect(W/2-S(300),H-S(72),S(240),S(50)); nxt=pygame.Rect(W/2+S(60),H-S(72),S(240),S(50))
    ...
    def draw_cheats(self):
        SCREEN.fill(BG)
        draw_text("AREA DE CHEAT",FONT_L,WHITE,(W/2,S(60)),True)
        draw_text("DIGITE O CODIGO DE TESTE",FONT_S,GRAY,(W/2,S(112)),True)
        box=pygame.Rect(W/2-S(260),S(155),S(520),S(85)); pygame.draw.rect(SCREEN,PANEL,box,border_radius=S(18)); pygame.draw.rect(SCREEN,PURPLE,box,width=max(2,S(4)),border_radius=S(18))
        shown="*"*len(self.cheat_buffer)
        draw_text(shown or "____",FONT_L,WHITE,box.center,True)
        if SAVE.get("cheat_all_chars",False): draw_text("CHEAT ATIVO: TODOS OS PERSONAGENS LIBERADOS",FONT_S,GREEN,(W/2,S(270)),True)
        elif self.cheat_message: draw_text(self.cheat_message,FONT_S,RED,(W/2,S(270)),True)
        ...
        clear=pygame.Rect(W/2-S(360),H-S(185),S(300),S(70)); confirm=pygame.Rect(W/2+S(60),H-S(185),S(300),S(70)); back=pygame.Rect(SAFE,H-S(72),S(210),S(50))
        ...
        draw_text("APAGAR",FONT_S,WHITE,clear.center,True); draw_text("CONFIRMAR",FONT_S,DARK,confirm.center,True); draw_text("VOLTAR",FONT_S,WHITE,back.center,True)
    ...
    def submit_cheat(self):
        if self.cheat_buffer == "0910":
            ...
            save_data(SAVE)
            self.cheat_message = "CODIGO ACEITO"
            self.unlock_notice = "CHEAT 0910: TODOS OS PERSONAGENS LIBERADOS"
    ...
    def click_ui(self, pos):
        if self.state == "menu":
            if hasattr(self, "menu_sfx_rect") and self.menu_sfx_rect.collidepoint(pos):
                ...
                AUDIO.play("click", 0.65); self.state = "whats_new"
            elif self.menu_achievements_rect.collidepoint(pos):
                AUDIO.play("click", 0.65); self.state = "achievements"
                ...
                AUDIO.play("click", 0.65); self.cheat_buffer = ""; self.cheat_message = ""; self.state = "cheats"
            ...
            if self.shop_back_rect.collidepoint(pos):
                ...
                for r, key, cost in self.shop_rects:
                    if r.collidepoint(pos) and SAVE["coins"] >= cost:
                        ...
                        SAVE["coins"] -= cost
                        SAVE[key] = SAVE.get(key, 0) + 1
                        save_data(SAVE)
                        break

        elif self.state == "whats_new":
            if self.whats_new_back_rect.collidepoint(pos): self.state = "menu"
            ...
            elif self.cheat_clear_rect.collidepoint(pos): self.cheat_buffer = self.cheat_buffer[:-1]
            elif self.cheat_confirm_rect.collidepoint(pos): self.submit_cheat()
            else:
                for r,digit in self.cheat_digit_rects:
                    if r.collidepoint(pos) and len(self.cheat_buffer) < 8:
            ...
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_ESCAPE:
                    if game.state == "menu":
                        ...
                        game.state = "paused"
                    elif game.state in ("upgrade_catalog", "bestiary", "control_editor", "character_select", "shop", "glonk_death", "death", "whats_new", "achievements", "missions", "cheats"):
                ...
                if game.state == "cheats":
                    if pygame.K_0 <= event.key <= pygame.K_9 and len(game.cheat_buffer) < 8:
                        game.cheat_buffer += chr(event.key)
                    elif event.key in (pygame.K_BACKSPACE, pygame.K_DELETE):
                        game.cheat_buffer = game.cheat_buffer[:-1]
