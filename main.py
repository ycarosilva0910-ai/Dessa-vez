...
# ============================================================
# DERROTE O GORDO DO PAI DO KAYK - V13 PERSONAGENS + EVOLUCAO
...
BESTIARY_ENTRIES = [
    ...
    {"name":"ARCEBISPOS DOS PECADOS", "type":"Boss especial", "hp":"Alto", "speed":"Variavel", "damage":"Assinatura reduzida do Pecado", "desc":"Aparecem a cada 50 ondas intermediarias e antecipam a mecanica do proximo Pecado."},
    ...
    {"name":"GULA", "type":"Pecado", "hp":"Extremo + memoria", "speed":"Alta", "damage":"Devorar", "desc":"Na wave 500. Um contato real pode devorar a run e fortalece reencontros futuros."},
...
FORGE_WEAPONS = [
    ...
    {"key":"kevyn_seven_faces","character":"Kevyn","name":"LAMINA CELESTIAL DAS SETE FACES","rarity":"divine","coins":0,
     "cost":DIVINE_COST.copy(),"desc":"Cortes a distancia. Segurar ATK 1s puxa inimigos; PARRY 1s repele; DOM 3s libera uma explosao roxa absoluta."},
    ...
    {"key":"ruan_monarch_cloak","character":"Ruan","name":"MANTO DO MONARCA","rarity":"divine","coins":0,
     "cost":DIVINE_COST.copy(),"desc":"No botao DASH: toque curto usa o Dash e invoca 1 Amigo se nao houver nenhum; segurar 1s teleporta o dobro da distancia do Dash e tambem garante o Amigo. Amigos podem soltar orbes de +5 HP."},
    {"key":"pedro_hunting_time","character":"Pedro","name":"TEMPO DE CACA","rarity":"divine","coins":0,
     "cost":DIVINE_COST.copy(),"desc":"+100% alcance e velocidade. Abacaxis sao teleguiados e sempre recuperam HP/Estamina. Segurar ATK 1s planta um abacaxi que fere inimigos e cura 10 HP ao ser recolhido."},
...
WHATS_NEW = [
    ...
    'PECADOS: a cada 50 ondas aparece um Arcebispo e a cada 100 ondas um Pecado verdadeiro; o ciclo reinicia apos a onda 700 mais forte.',
    'BOSSES: Gula continua na onda 500 e cada Pecado verdadeiro entrega seu Pedaco exclusivo ao ser derrotado.',
    ...
    'PEDRO DIVINO: Tempo de Caca recebeu +100% de alcance e +100% de velocidade nos abacaxis.',
...
def evolution_drop_chance(wave):
    # 22% no inicio, subindo gradualmente ate 50% nas waves mais altas.
    return min(0.50, 0.22 + max(0, int(wave)) * 0.00056)
...
def sin_encounter_for_wave(wave):
    """50=Arcebispo da Ira, 100=Ira, ... 650=Arcebispo da Luxuria, 700=Luxuria; depois reinicia."""
    if wave < 50 or wave % 50 != 0:
        return None
    slot = ((wave - 50) // 50) % 14
    sin_index = slot // 2
    ...
    def update(self, dt, game):
        ...
        if self.sin_key == "wrath" and self.sin_tick <= 0:
            ...
            for i in range(shots):
                ...
                game.projectiles.append(Projectile(self.pos,d*S(330), (8+game.wave*0.22)*self.boss_damage_scale*strength,"enemy",S(9),RED,2.5))
            self.sin_tick = 1.8 if self.archbishop else 1.15
            ...
            self.sin_aux = 4.6 if self.archbishop else 3.6
            ...
            if to.length_squared()>0:
                ...
                for off in (-0.24,0,0.24):
                    game.projectiles.append(Projectile(self.pos,vec_from_angle(a+off)*S(390),(9+game.wave*0.20)*self.boss_damage_scale*strength,"enemy",S(10),GREEN,3.0))
            self.speed=max(self.speed, game.player.base_speed*(0.72 if self.archbishop else 0.95))
            self.sin_tick=1.55 if self.archbishop else 1.05
            ...
            self.sin_aux=3.2 if self.archbishop else 2.1
            ...
            for i in range(6 if self.archbishop else 10):
                ...
                game.projectiles.append(Projectile(self.pos,d*S(205),(7+game.wave*0.16)*self.boss_damage_scale*strength,"enemy",S(13),BLUE,4.0))
            self.sin_tick=2.4 if self.archbishop else 1.7
            ...
            if to.length_squared()>0:
                ...
                for off in (-0.42,-0.21,0,0.21,0.42):
                    game.projectiles.append(Projectile(self.pos,vec_from_angle(a+off)*S(360),(8+game.wave*0.18)*self.boss_damage_scale*strength,"enemy",S(9),PINK,3.0))
            self.sin_aux=3.4 if self.archbishop else 2.25
...
class Player:
    ...
    def has_divine_attack_hold(self):
        return self.divine_kevyn or self.divine_ana or self.divine_pedro
    ...
    def _divine_ruan_friend_if_empty(self, game, spawn_pos=None):
        ...
        if targets:
            ...
            # Fonte neutra: garante que o Manto funcione ate em arena vazia/AFK.
    ...
    def finish_dash_hold(self, game):
        if (self.divine_ruan or self.divine_kayk) and not self.divine_dash_triggered:
    ...
    def update(self, dt, game, move_dir):
        ...
        if self.has_divine_attack_hold() and getattr(game,"attack_held",False):
            ...
            if self.divine_kevyn and self.divine_attack_hold>=1.0 and not self.divine_attack_triggered:
                self.divine_attack_triggered=True; self.divine_pull(game)
            elif self.divine_pedro and self.divine_attack_hold>=1.0 and not self.divine_attack_triggered:
                self.divine_attack_triggered=True; self.plant_divine_pineapple(game)
            elif self.divine_ana and self.divine_attack_hold>=self.longinus_next_threshold:
        ...
        if (self.divine_ruan or self.divine_kayk) and getattr(game,"dash_held",False):
...
class Game:
    ...
    def roll_evolution_point(self):
        if self.mode != "arena" or is_beta_character(self.player.character):
            return False
        chance = evolution_drop_chance(self.wave)
        ...
        save_data(SAVE)
        self.evolution_notice = f"+1 PONTO DE EVOLUCAO PARA {self.player.character.upper()}!"
    ...
    def roll_forge_drop(self, enemy):
        ...
        if is_boss:
            ...
            return
        chance = 0.035 + (0.045 if getattr(enemy,"elite",None) else 0.0) + min(0.04,self.wave*0.00008)
        if random.random() > chance:
            return
        roll=random.random()
    ...
    def end_run(self):
        ...
        if not is_beta_character(self.player.character):
    ...
    def handle_touch_down(self, pos, finger_id=None):
        if self.state == "playing":
            ...
            if self.buttons["attack"].collidepoint(pos):
                ...
                if self.player.divine_ruan or self.player.divine_kayk:
    ...
    def update(self, dt):
        ...
        elif self.state in ("menu", "character_select", "shop", "upgrade", "death", "glonk_death", "upgrade_catalog", "bestiary", "forge", "items", "weapons", "control_editor", "run_upgrades", "whats_new", "achievements", "missions", "cheats", "vinicius_mega"):
    ...
    def draw_hud(self):
        ...
        if p.divine_ana:
            ...
            else: special_text="MANTO: DASH=AMIGO | SEGURE 1.0s"
        elif p.divine_kevyn:
            special_text="SETE FACES: ATK 1s | PARRY 1s | DOM 3s"
        elif p.divine_pedro:
            special_text="TEMPO DE CACA: SEGURE ATK 1.0s"
    ...
    def draw_character_select(self):
        ...
        if beta_selected:
            ...
            chance = int(evolution_drop_chance(max(1,SAVE.get('best_wave',1)))*100)
            draw_text(f"Ponto por onda: ~{chance}% na sua faixa (22% a 50%)", FONT_S, GRAY, (detail.x+S(22),detail.y+S(103)))
    ...
    def draw_cheats(self):
        ...
        if SAVE.get("cheat_dev",False):
            ...
            ew,eh,eg=S(250),S(52),S(12); total=ew*5+eg*4; ex=W/2-total/2; ey=S(540)
            for i,(key,label,color) in enumerate(labels):
                rr=pygame.Rect(ex+i*(ew+eg),ey,ew,eh); pygame.draw.rect(SCREEN,color,rr,border_radius=S(12)); draw_text(label,FONT_S,DARK if color in (ORANGE,CYAN,GREEN) else WHITE,rr.center,True); self.cheat_extra_rects.append((rr,key))
                    ...
                    if not spent:
                        ...
                        if not changed and self.character_play_rect.collidepoint(pos):
                            ...
                            if self.character_unlocked(name):
                                ...
                                if self.selection_target == "afk": self.start_afk(name)
                                else: self.reset_run(name)
            ...
            if event.type == pygame.QUIT:
                ...
                if game.state == "playing":
                    if event.key == pygame.K_SPACE:
                        ...
                        if game.player.divine_ruan or game.player.divine_kayk:
