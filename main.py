...
# ============================================================
# DERROTE O GORDO DO PAI DO KAYK - V12 RELEASE + JOYSTICK + GULA
...
def high_wave_scaling(wave):
    """Escala adicional para impedir que builds de waves altissimas trivializem o jogo."""
    if wave <= 50:
        return 1.0, 1.0, 1.0
    tier = (wave - 50) / 50.0
    hp_mult = 1.0 + 0.35 * tier + 0.080 * tier * tier
    damage_mult = 1.0 + 0.18 * tier + 0.035 * tier * tier
    speed_mult = min(1.60, 1.0 + 0.04 * tier)
    return hp_mult, damage_mult, speed_mult
...
BESTIARY_ENTRIES = [
    ...
    {"name":"A GULA", "type":"Boss Especial", "hp":"Escala brutal + memoria de derrotas", "speed":"Aumenta a cada encontro", "damage":"Qualquer golpe que atravesse sua defesa encerra a run", "desc":"Surge na onda 500. Cada vez que devora um jogador, fica permanentemente mais forte nas proximas tentativas."},
]
...
ACHIEVEMENTS = [
    ...
    ("domain_500", "RITUALISTA", "Ative 500 Expansoes de Dominio no total."),
    ("wave_100", "TRES DIGITOS", "Alcance a onda 100."),
    ("wave_250", "ISSO JA E PESSOAL", "Alcance a onda 250."),
    ("wave_500", "ONDE TERMINA?", "Alcance a onda 500 e encontre A Gula."),
    ...
    ("pineapple_25", "ABACAXI ECONOMICO", "Recupere HP/STA 25 vezes com abacaxis que voltaram sem acertar ninguem."),
    ("friend_12", "PODER DA AMIZADE", "Tenha 12 amigos do Ruan vivos ao mesmo tempo."),
    ("father_unlock", "PAI PRESENTE", "Desbloqueie o Pai do Kayk derrotando-o com todos os personagens exigidos."),
    ("gula_defeated", "A FOME PASSOU", "Derrote A Gula na onda 500."),
]
...
WHATS_NEW = [
    "Joystick analogico virtual substitui o direcional antigo.",
    "Controles e botoes importantes agora respeitam margens seguras de tela.",
    "Kevyn acerta inimigos mesmo quando esta totalmente colado neles.",
    "Inimigos escalam muito mais depois da onda 50 e ficam brutais nas waves altas.",
    "A Gula aparece na onda 500 e fica mais forte cada vez que devora uma run.",
    "Golpe do Pai do Kayk agora mostra claramente o raio atingido.",
    "Missoes e conquistas foram refeitas para jogadores de alto nivel.",
    "Cheat 2026 libera o painel DINHEIRO para definir a quantidade de moedas.",
    "Backup e restauracao por codigo continuam ativos com clipboard nativo Android.",
    "Dominios fixos, Zona AFK, buffs confirmados e melhorias da V11 continuam incluidos.",
]
...
class Enemy:
    ...
    def __init__(self, pos, kind, wave, elite=None):
        ...
        base_hp = (26 + wave * 7) * hp_scale
        base_speed = S(95 + wave * 2.2) * speed_scale
        self.contact_damage = (7 + wave * 0.7) * damage_scale
    ...
    def update(self, dt, game):
        ...
        if self.kind == "chaser" or self.kind == "tank":
            ...
            if self.shoot_cd <= 0:
                ...
                game.projectiles.append(Projectile(self.pos, direction * speed, (7 + game.wave * 0.55) * self.high_wave_damage_scale, "enemy"))
            ...
            if self.shoot_cd <= 0:
                for off in (-0.12, 0.12):
                    ...
                    game.projectiles.append(Projectile(self.pos, vec_from_angle(a) * S(330), (5 + game.wave * 0.45) * self.high_wave_damage_scale, "enemy", S(8), YELLOW))
                self.shoot_cd = 1.8
...
class Boss(Enemy):
    def __init__(self, pos, wave):
        ...
        self.max_hp = (700 + wave * 95) * hp_scale
        self.hp = self.max_hp
        self.speed = S(75 + wave) * speed_scale
        self.coin_value = 55
        self.wave = wave
    ...
    def update(self, dt, game):
        ...
        if self.domain_active:
            ...
            if self.domain_timer <= 0:
                ...
                if self.variant == "father" and inside and self.domain_tick <= 0:
                    game.player.take_damage((6 + self.wave*0.22) * self.boss_damage_scale, game)
                    ...
                    if game.player.stamina > 0:
                        ...
                        game.player.take_damage((5 + self.wave*0.15) * self.boss_damage_scale, game)
                        self.domain_tick = 0.8
                elif self.variant == "sentinel" and inside and self.domain_tick <= 0:
                    for i in range(4):
                        d = vec_from_angle(i*math.pi/2)
                        game.projectiles.append(Projectile(self.domain_center, d*S(355), (6+self.wave*0.35) * self.boss_damage_scale, "enemy", S(9), CYAN, 2.5))
        ...
        if self.burst_cd <= 0:
            ...
            for i in range(shots):
                ...
                game.projectiles.append(Projectile(self.pos, vec_from_angle(a) * S(285 if phase2 else 235), (8 + game.wave * 0.5)*buff*self.boss_damage_scale, "enemy", S(11), PINK))
        ...
        if dist < self.radius + target_entity.radius and self.touch_cd <= 0:
            if self.variant == "gula" and target_entity is game.player:
                ...
                target_entity.take_damage((14 + game.wave * 0.8)*buff*self.boss_damage_scale, game)
            self.touch_cd = 0.65
...
class GulaBoss(Boss):
    """Boss especial da onda 500. Lembra quantas vezes ja devorou o jogador."""
...
class Player:
    ...
    def expand_domain(self, game):
        ...
        if SAVE["total_domains"] >= 500:
            game.unlock_achievement("domain_500")
        save_data(SAVE)
...
class Game:
    ...
    def spawn_wave(self):
        ...
        if self.wave % 5 == 0:
            if self.wave == 500:
                boss = GulaBoss((W/2, S(350)), self.wave)
                self.unlock_notice = f"A GULA LEMBRA DE VOCE | FOME {boss.hunger_level}"
            ...
            adds = min(6 if self.wave >= 100 else 3, max(1, self.wave // 80 + 2))
            for _ in range(adds):
            ...
            count_cap = 36 if self.wave >= 150 else (32 if self.wave >= 75 else 28)
            count = min(4 + self.wave * 2, count_cap)
            kinds = ["chaser", "shooter"]
            if self.wave >= 2:
                kinds.append("kiter")
            if self.wave >= 3:
                kinds.append("tank")
            elite_chance = min(0.08 + self.wave * 0.015, 0.70 if self.wave >= 150 else 0.50 if self.wave >= 75 else 0.35)
            for _ in range(count):
    ...
    def record_clash_win(self):
        ...
        save_data(SAVE)
        if SAVE["clash_wins"] >= 10:
            self.unlock_achievement("clash_10")
    ...
    def on_enemy_killed(self, enemy):
        ...
        if self.mode == "arena":
            ...
            if SAVE.get("total_kills", 0) >= 1000:
                self.unlock_achievement("kill_1000")
        if self.domain_clash_active:
        ...
        for pr in self.projectiles:
            ...
            if not pr.update(proj_dt):
                if isinstance(pr, BoomerangProjectile) and pr.returned_to_player and not pr.hit_any:
                    ...
                    if SAVE["pineapple_refunds"] >= 25:
                        self.unlock_achievement("pineapple_25")
                    save_data(SAVE)
                continue
    ...
    def draw_playing(self):
        ...
        if self.wave_banner > 0 and self.mode == "arena":
            draw_text("A GULA" if self.wave == 500 else ("BOSS" if self.wave % 5 == 0 else f"ONDA {self.wave}"), FONT_XL, RED if self.wave == 500 else WHITE, (W/2, H*0.36), True)
    ...
    def draw_menu(self):
        ...
        # Backup fica centralizado entre os toggles de audio e os recordes.
        backup = pygame.Rect(W/2-tab_w/2, int(H*0.745), tab_w, tab_h)
        pygame.draw.rect(SCREEN, GREEN, backup, border_radius=S(12))
        draw_text("BACKUP / RESTAURAR", FONT_S, DARK, backup.center, True)
        self.menu_backup_rect = backup
    ...
    def draw_character_select(self):
        ...
        back = pygame.Rect(CONTROL_SAFE_X, H-CONTROL_SAFE_Y-S(48), S(210), S(48))
    ...
    def draw_whats_new(self):
        SCREEN.fill(BG)
        draw_text("NOVIDADES - V12", FONT_L, WHITE, (W/2, S(58)), True)
    ...
    def draw_achievements(self):
        SCREEN.fill(BG)
        unlocked = set(SAVE.get("unlocked_achievements", []))
        known_keys = {a[0] for a in ACHIEVEMENTS}
        unlocked_now = unlocked & known_keys
        per_page = 6
        pages = max(1, math.ceil(len(ACHIEVEMENTS)/per_page))
        self.achievement_page = int(clamp(self.achievement_page, 0, pages-1))
        ...
        draw_text(f"{len(unlocked_now)}/{len(ACHIEVEMENTS)} DESBLOQUEADAS  |  PAGINA {self.achievement_page+1}/{pages}", FONT_S, YELLOW, (W/2, S(108)), True)
        cols, gap = 2, S(18)
        top, card_h = S(150), S(165)
        card_w = int((W-CONTROL_SAFE_X*2-gap)/2)
        for i,(key,name,desc) in enumerate(subset):
            col,row=i%cols,i//cols
            r=pygame.Rect(CONTROL_SAFE_X+col*(card_w+gap), top+row*(card_h+gap), card_w, card_h)
            ok=key in unlocked
        ...
        yb = H-CONTROL_SAFE_Y-S(50)
        back=pygame.Rect(CONTROL_SAFE_X,yb,S(210),S(48))
        prev=pygame.Rect(W/2-S(260),yb,S(210),S(48))
        nxt=pygame.Rect(W/2+S(50),yb,S(210),S(48))
        for r,label in [(back,"VOLTAR"),(prev,"< ANTERIOR"),(nxt,"PROXIMA >")]:
            pygame.draw.rect(SCREEN,PANEL2,r,border_radius=S(13)); draw_text(label,FONT_S,WHITE,r.center,True)
        self.achievements_back_rect=back
        self.achievements_prev_rect=prev
        self.achievements_next_rect=nxt
    ...
    def draw_cheats(self):
        ...
        title = "CHEAT: DINHEIRO" if self.cheat_money_mode else "AREA DE CHEAT"
        subtitle = "DIGITE QUANTO DINHEIRO VOCE QUER" if self.cheat_money_mode else "DIGITE O CODIGO DE TESTE"
        ...
        buffer = self.cheat_money_buffer if self.cheat_money_mode else self.cheat_buffer
        shown = buffer if self.cheat_money_mode else "*"*len(buffer)
        draw_text(shown or ("0" if self.cheat_money_mode else "____"),FONT_L,WHITE,box.center,True)
        if self.cheat_money_mode:
            ...
            draw_text("0910 ATIVO: TODOS OS PERSONAGENS",FONT_S,GREEN,(W/2,S(270)),True)
        ...
        draw_text("APAGAR",FONT_S,WHITE,clear.center,True); draw_text("DEFINIR" if self.cheat_money_mode else "CONFIRMAR",FONT_S,DARK,confirm.center,True); draw_text("VOLTAR",FONT_S,WHITE,back.center,True)
    ...
    def submit_cheat(self):
        ...
        if self.cheat_buffer == "0910":
            ...
            save_data(SAVE)
            self.cheat_message = "CODIGO 0910 ACEITO"
            ...
            save_data(SAVE)
            self.cheat_message = "CODIGO 2026 ACEITO: DINHEIRO LIBERADO"
            AUDIO.play("unlock", 1.0)
        else:
    ...
    def click_ui(self, pos):
        if self.state == "menu":
            if hasattr(self, "menu_sfx_rect") and self.menu_sfx_rect.collidepoint(pos):
                ...
                AUDIO.play("click", 0.65); self.cheat_buffer = ""; self.cheat_money_buffer = ""; self.cheat_money_mode = False; self.cheat_message = ""; self.state = "cheats"
            ...
            if self.cheat_back_rect.collidepoint(pos): self.state = "menu"
                ...
                self.cheat_money_mode = not self.cheat_money_mode
                self.cheat_buffer = ""
                self.cheat_money_buffer = ""
                self.cheat_message = ""
            elif self.cheat_clear_rect.collidepoint(pos):
                if self.cheat_money_mode: self.cheat_money_buffer = self.cheat_money_buffer[:-1]
                ...
                for r,digit in self.cheat_digit_rects:
                    if r.collidepoint(pos):
                        if self.cheat_money_mode and len(self.cheat_money_buffer) < 9:
                            self.cheat_money_buffer += digit
                        elif not self.cheat_money_mode and len(self.cheat_buffer) < 8:
