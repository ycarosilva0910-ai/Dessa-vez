...
# ============================================================
# DERROTE O GORDO DO ?AI DO KAYK - V7 PERSONAGENS + PAUSE
...
pygame.display.set_caption("Derrote o Gordo do ?ai do Kayk")
        ...
        for enemy in list(game.enemies):
            ...
            if dist <= radius + enemy.radius:
                ...
                if random.random() < game.player.crit_chance:
                    dmg *= 2
                    game.damage_texts.append(DamageText("CRIT!", enemy.pos.copy(), YELLOW))
                enemy.damage(dmg, game, self.pos)
                if random.random() < self.double_chance:
                    enemy.damage(dmg, game, self.pos)
                    game.damage_texts.append(DamageText("ERRO x2", enemy.pos.copy(), PINK))
                hit_any = True
...
class Enemy:
    ...
    def damage(self, amount, game, source_pos=None):
        ...
        self.hp -= amount
        self.flash = 0.12
        game.damage_texts.append(DamageText(str(int(amount)), self.pos.copy(), WHITE))
...
class TrainingDummy(Enemy):
    ...
    def die(self, game):
        ...
        game.damage_texts.append(DamageText("DUMMY RESET", self.pos.copy(), CYAN))
...
class Player:
    ...
    def recover_stamina(self, amount, game=None):
        ...
        if game and self.stamina > old:
            game.damage_texts.append(DamageText("+" + str(int(self.stamina-old)) + " STA", self.pos.copy(), CYAN))
    ...
    def heal(self, amount, game=None):
        ...
        if game and self.hp > old:
            game.damage_texts.append(DamageText("+" + str(int(self.hp-old)) + " HP", self.pos.copy(), GREEN))
    ...
    def take_damage(self, amount, game):
        ...
        if self.character == "Kevyn" and self.in_domain(game):
            game.damage_texts.append(DamageText("INTOCAVEL", self.pos.copy(), BLUE))
            return False
        if self.invuln > 0 or self.dash_time > 0:
            return False
        if self.shield > 0:
            self.shield -= 1
            game.damage_texts.append(DamageText("BLOCK", self.pos.copy(), BLUE))
            game.shake = max(game.shake, S(5))
            self.invuln = 0.35
            return False
        ...
        if self.eternal_combo:
            ...
            game.combo = 0
            game.combo_timer = 0
        game.damage_texts.append(DamageText("-" + str(int(amount)) + " HP", self.pos.copy(), RED))
        ...
        if self.hp <= 0:
            if not self.revive_used and (self.second_bar or self.one_last_game):
                ...
                game.damage_texts.append(DamageText("SEGUNDA CHANCE!", self.pos.copy(), YELLOW, 1.1))
    ...
    def attack(self, game):
        ...
        if self.character == "Kevyn":
            ...
            if marked and self.spend_stamina(max(1, 2-self.kevyn_sword_cost_reduction)):
                ...
                game.damage_texts.append(DamageText("EXECUCAO!", target.pos.copy(), YELLOW))
                ...
                if 0 < dist <= rng + e.radius:
                    ...
                    if d.dot(self.facing) >= angle_limit:
                        ...
                        if random.random() < self.crit_chance:
                            dmg *= 2
                            game.damage_texts.append(DamageText("CRIT!", e.pos.copy(), YELLOW))
        ...
        if self.character == "Ana":
            ...
            game.damage_texts.append(DamageText("MINI-ANAS!", self.pos.copy(), PINK))
            return
    ...
    def dash(self, game, move_dir):
        ...
        if near:
            ...
            game.damage_texts.append(DamageText("PERFECT DODGE!", self.pos.copy(), CYAN))
            game.add_domain_charge(5)
            if self.broken_time:
                game.enemy_slow_timer = max(game.enemy_slow_timer, 2.2)
        if self.ghost_dash:
            game.delayed_blasts.append({"timer":0.70, "pos":self.pos.copy(), "radius":S(150), "damage":self.base_damage*1.2, "color":CYAN})
    ...
    def expand_domain(self, game):
        ...
        game.domain_center = self.pos.copy()
...
class Game:
    ...
    def start_afk(self, character=None):
        ...
        self.damage_texts.append(DamageText("ZONA AFK", self.player.pos.copy(), CYAN, 1.2))
    ...
    def end_run(self):
        ...
        if self.mode == "afk":
            ...
            self.healing_locked = False
            self.damage_texts.append(DamageText("RESET HP", self.player.pos.copy(), GREEN))
            return
        if self.state == "death":
            return
        self.state = "death"
    ...
    def on_enemy_killed(self, enemy):
        ...
        if enemy.blood_marked:
            ...
            self.damage_texts.append(DamageText("PRESA CONSUMIDA", enemy.pos.copy(), GREEN))
        if p.chain_execution and self.current_damage_kind == "execute":
            ...
            if options:
                ...
                self.damage_texts.append(DamageText("MARCADO", target.pos.copy(), RED))
    ...
    def check_synergies(self):
        ...
        for req, name in combos:
            if req.issubset(self.owned_upgrades) and name not in self.synergies:
                ...
                self.damage_texts.append(DamageText("SINERGIA!", self.player.pos.copy(), YELLOW, 1.2))
        ...
        if self.state == "playing":
            ...
            if self.mode == "afk":
                ...
                if self.afk_buttons["DOM"].collidepoint(pos):
                    ...
                    self.player.hp = self.player.max_hp
                    self.player.stamina = self.player.max_stamina
                    self.damage_texts.append(DamageText("DOMINIO 100%", self.player.pos.copy(), PURPLE))
                    return
    ...
    def update_playing(self, dt):
        ...
        if self.domain_active:
            ...
            if self.domain_timer <= 0:
                self.domain_active = False
                self.domain_timer = 0
                self.damage_texts.append(DamageText("DOMINIO ENCERRADO", self.player.pos.copy(), GRAY))
        ...
        if self.domain_active and self.player.character == "Kayk" and self.player.in_domain(self):
            ...
            if self.kayk_domain_fire_cd <= 0:
                ...
                for target in targets[:max_targets]:
                    ...
                    self.damage_texts.append(DamageText("ALMA!", target.pos.copy(), PURPLE))
        ...
        for pr in self.projectiles:
            ...
            if pr.owner == "enemy":
                if self.player.parry_time > 0 and pr.pos.distance_to(self.player.pos) <= S(95):
                    ...
                    self.damage_texts.append(DamageText("PARRY!", self.player.pos.copy(), YELLOW))
                ...
                for e in list(self.enemies):
                    ...
                    if pr.pos.distance_to(e.pos) <= pr.radius + e.radius:
                        ...
                        if random.random() < self.player.crit_chance:
                            dmg *= 2
                            self.damage_texts.append(DamageText("CRIT!", e.pos.copy(), YELLOW))
                        ...
                        if random.random() < double_chance:
                            e.damage(dmg, self, pr.pos)
                            self.damage_texts.append(DamageText("ERRO x2", e.pos.copy(), PINK))
                        self.current_damage_kind = None
                        pr.hit_ids.add(e.id)
            ...
            if dist <= d.radius + self.player.radius + S(8):
                if d.kind == "heal":
                    if self.player.insatiable_hunger:
                        self.damage_texts.append(DamageText("FOME", self.player.pos.copy(), RED))
                    else:
                ...
                elif d.kind == "shield": self.player.shield += 1
                elif d.kind == "haste": self.player.speed_buff = 8
                elif d.kind == "coin": self.run_coins += int(8 * self.coin_multiplier)
                ...
                if self.player.orb_magnet and d.kind in ("heal","stamina"):
                    ...
                    if self.orb_chain_count >= 3:
                        ...
                        self.current_damage_kind = old
                        self.damage_texts.append(DamageText("COLAPSO DE ORBES!", self.player.pos.copy(), PURPLE))
                continue
    ...
    def draw_menu(self):
        ...
        draw_text(f"MOEDAS: {SAVE['coins']}   |   MELHOR ONDA: {SAVE['best_wave']}   |   RECORDE KOs: {SAVE['best_kills']}", FONT_S, GRAY, (W/2, H*0.74), True)
        draw_text("HP + Estamina | Builds, sinergias e Expansao de Dominio", FONT_S, CYAN, (W/2, H*0.82), True)
        if SAVE.get("kayk_unlocked", False):
            draw_text("KAYK DESBLOQUEADO", FONT_S, PURPLE, (W/2, H*0.865), True)
        draw_text("Teclado: WASD | ESPACO atacar | SHIFT dash | E parry | Q dominio", FONT_S, GRAY, (W/2, H*0.91), True)
    ...
    def pause_game(self):
        ...
        self.state = "paused"
        bw, bh = S(520), S(92)
        self.pause_resume_rect = pygame.Rect(W/2-bw/2, H*0.45, bw, bh)
        self.pause_menu_rect = pygame.Rect(W/2-bw/2, H*0.57, bw, bh)
        self.attack_held = False
        for key in self.move_touch:
            self.move_touch[key] = False
    ...
    def draw_paused(self):
        ...
        bw, bh = S(520), S(92)
        resume = pygame.Rect(W/2-bw/2, H*0.45, bw, bh)
        menu = pygame.Rect(W/2-bw/2, H*0.57, bw, bh)
    ...
    def click_ui(self, pos):
        if self.state == "menu":
            if self.menu_start_rect.collidepoint(pos):
                self.open_character_select("arena")
            elif self.menu_afk_rect.collidepoint(pos):
                self.open_character_select("afk")
